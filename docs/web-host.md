...
Evidence: [STATUS-COMPLETE-PHASES.md](../STATUS-COMPLETE-PHASES.md#phase-w--web-host-software-tier).
...
the core worker + `test:web-hyperdrive-browser` (256t install reports
`fetchPath: hyperdrive` via live gateway bulk fetch; DHT relay remains an
experimental fallback because `@hyperswarm/dht-relay` lookup relay is still
broken against `hyperdht` 6.21 in local CI). WebSerial RNode stretch landed
