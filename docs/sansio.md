...
> / `LeafTransport.sendPacket` adapt it. **`Link.updateKeepalive`** routes through
> `stepLinkWatchdog` `link/rtt-measured` (no duplicated keepalive/stale formula). Remaining
