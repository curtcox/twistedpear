...
| Host-owned WebRTC signaling and data channel; SDP/ICE never cross the broker | [`webrtc-route.ts`](../packages/peer-discovery/src/webrtc-route.ts) | Reuse for the top plane. Data channel remains for control/TPD2; media tracks via `mediaTransceivers` + `attachTrack` / `onRemoteTrack`, and `createWebRtcMediaTrackPlaneOpener`. |
