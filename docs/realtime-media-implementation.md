...
| 4 · Media path | Authenticated Reticulum/WebRTC route subscriptions, bounded `TPM1` offer/frame envelopes, TPD2 timing, receive router and two-host in-memory timed-frame loopback; concrete `PlaneStreamEgressFactory` openers for WebRTC / Pears-bulk / Reticulum (via the peer-route bridge), a derived-only CAS snapshot opener, a live Pears Hyperdrive append path for latency-tolerant rungs (desktop/mobile), and a host WebRTC *media-track* opener (`createWebRtcMediaTrackPlaneOpener`) preferred over the data-channel path when the host supplies a capture track; `WebRtcRouteController` negotiates audio/video transceivers and exposes `attachTrack` / `onRemoteTrack`; web host SDP includes sendrecv audio/video m-lines | `npm run formal:stream`; `npm run test:local-multipeer` carries derived and PCM `TPD2` frames between peers and echoes them back; peer-discovery and miniapp-runtime unit coverage for track attach and data-channel fallback | Incomplete: shipping hosts do not yet feed `getOutboundTrack` into the plane table for a live call; desktop↔desktop audio calls are unrecorded |
...
4. Wire shipping hosts to supply `getOutboundTrack` into `createHostPlaneOpeners({ webrtcMedia })`
   (controller + web SDP already negotiate tracks); then run desktop↔desktop and
   desktop↔simulator audio calls. Host `PlaneStreamEgressFactory` already opens
   WebRTC/Pears/Reticulum via the peer-route bridge, CAS for derived snapshots, and a
   live Pears Hyperdrive append path for latency-tolerant rungs; SPEC-STREAM admits
   `cas-snapshot` when no live plane has supply.
