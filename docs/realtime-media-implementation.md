...
| 5 · Codecs and duplex | Encoding-aware demand, codec effect boundary, browser/desktop raw RGBA/PCM capture, WebCodecs mono Opus, browser/desktop voice-processing constraints; desktop/web harness `media-opus-duplex` / `media-opus-play`; native mobile worklet admits Opus via `SimulatedMediaCodecDriver` and exposes `media-opus-duplex` / `media-opus-play` over the test-agent (PCM speaker play until a real Opus decoder ships); native worklet wires `openWebRtcMediaPlane` + WebRTC peer candidates/establish/data beside Reticulum, with host IPC handlers that fail closed until `RTCPeerConnection`/`getUserMedia` are provided (e.g. react-native-webrtc `registerGlobals`) | `npm run test:sim-media-ladder`; `npm run test:webrtc-gui-call` and `test:webrtc-gui-call:web` record `callsOpusDuplex` (encode→decode→speaker + cross-peer TPD2 Opus play) beside `callsWebRtc` track bytes and voice-duplex AEC constraints | Complete for desktop↔desktop and web↔web Opus duplex + AEC-constrained track attach; native mobile has simulated Opus codec-boundary + harness cmds and `openWebRtcMediaPlane` IPC parity (2026-08-01); Incomplete: real mobile Opus decode, RN WebRTC globals for honest native track bytes, desktop↔simulator audio |
...
3. Native mobile: add `react-native-webrtc` (and `registerGlobals`) so host IPC can
   produce honest track bytes; real Opus decode remains open. Simulated Opus codec
   boundary, `media-opus-duplex` harness cmds, and `openWebRtcMediaPlane` worklet/host
   wiring are in place. Desktop Opus duplex remains `test:webrtc-gui-call`
