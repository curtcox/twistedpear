...
4. Record desktop↔desktop or web↔web media bytes after invite accept (web and desktop
   hosts now feed `openWebRtcMediaPlane` / `getUserMedia`+`addTrack` into the plane table;
   desktop also negotiates WebRTC beside Reticulum). Host `PlaneStreamEgressFactory`
