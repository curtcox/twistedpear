...
4. Bind WebRTC *media tracks* (beyond the authenticated data-channel opener); then run
   desktop↔desktop and desktop↔simulator audio calls. Host `PlaneStreamEgressFactory`
   already opens WebRTC/Pears/Reticulum via the peer-route bridge, CAS for derived
   snapshots, and a live Pears Hyperdrive append path for latency-tolerant rungs;
   SPEC-STREAM admits `cas-snapshot` when no live plane has supply.
