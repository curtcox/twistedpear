...
| 5 · Codecs and duplex | Encoding-aware demand, codec effect boundary, browser/desktop raw RGBA/PCM capture, WebCodecs mono Opus, browser/desktop voice-processing constraints; desktop/web harness `media-opus-duplex` / `media-opus-play`; native mobile worklet admits Opus via `SimulatedMediaCodecDriver` and exposes `media-opus-duplex` / `media-opus-play` over the test-agent (PCM speaker play until a real Opus decoder ships); native worklet wires `openWebRtcMediaPlane` + WebRTC peer candidates/establish/data beside Reticulum plus harness `peer-pair-*` / `renderer-ping` (same contract as web/desktop); host depends on `react-native-webrtc` + `@config-plugins/react-native-webrtc` (Expo 52) with `registerGlobals` in `index.js`; local iOS prebuild pods include `react-native-webrtc` 124.0.4 / JitsiWebRTC and a Debug sim install of TwistedPearHarness succeeded (2026-08-01); `npm run test:webrtc-gui-call:ios` drives hub+desktop+ios | `npm run test:sim-media-ladder`; `npm run test:webrtc-gui-call` and `test:webrtc-gui-call:web` record `callsOpusDuplex` (encode→decode→speaker + cross-peer TPD2 Opus play) beside `callsWebRtc` track bytes and voice-duplex AEC constraints | Complete for desktop↔desktop and web↔web Opus duplex + AEC-constrained track attach; native mobile has simulated Opus + `openWebRtcMediaPlane` IPC + RN WebRTC + peer-pair harness cmds + iOS sim rebuild/install (2026-08-01); Incomplete: recorded native WebRTC track-byte proof (`test:webrtc-gui-call:ios`), real mobile Opus decode, desktop↔simulator audio |
...
3. Native mobile: record honest WebRTC track bytes on the rebuilt iOS sim app; real
   Opus decode remains open. Simulated Opus, `media-opus-duplex`, `openWebRtcMediaPlane`
   IPC, RN WebRTC + `registerGlobals`, and an iOS Debug sim install are in place.
   Desktop Opus duplex remains `test:webrtc-gui-call` `callsOpusDuplex`; Android
   peer-audio has voice-duplex AEC/NS/AGC recording beside iOS.
