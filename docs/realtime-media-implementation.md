...
3. Native mobile real Opus decode (and optional iOS-originated track attach). Desktop↔iOS
   WebRTC track bytes are recorded by `npm run test:webrtc-gui-call:ios` (1484 outbound RTP
   bytes + voice-duplex AEC, 2026-08-01). Desktop Opus duplex remains
   `test:webrtc-gui-call` `callsOpusDuplex`; Android peer-audio has voice-duplex AEC/NS/AGC
   recording beside iOS.
