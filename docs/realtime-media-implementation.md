...
3. Optional iOS-originated WebRTC track attach, and Android Opus duplex / PeerAudio speaker
   once an emulator + adb are available. Desktop↔iOS Opus duplex and track bytes are
   recorded by `npm run test:webrtc-gui-call:ios` (`callsOpusDuplex` webcodecs→bundled-opus
   with peerPlayed, plus `callsWebRtc` 451 outbound RTP bytes + voice-duplex AEC,
