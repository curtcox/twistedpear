...
|---|---|---|---|
| `freenet:contract` | Available on desktop/headless when an external or user-supervised v0.2.112 node is configured. Mobile hosts expose simulator-verified remote-node grant chrome; web stays unsupported under Option A. Off by default; every `put` and `update` requires a host confirmation because publication is global and irreversible. | [10](10-apps-that-use-freenet.md) | [integration plan](../docs/freenet-integration-plan.md), [app-execution ADR](../docs/adr-freenet-app-execution.md) |
...
| Resolving a 256t identifier | The host requests a missing locator and connected holders re-announce the signed record. A valid identifier remains unresolvable when no reachable holder knows it. | [6](06-apps-that-move-files.md), [8](08-apps-that-build-apps.md) | [docs/256t-distribution.md](../docs/256t-distribution.md) |
| QR scanning | Mobile and desktop host chrome can scan; desktop requires Chromium `BarcodeDetector` support and camera permission. Paste remains the fallback. | [6](06-apps-that-move-files.md#photo-drop) | [LIMITATIONS.md §7](../LIMITATIONS.md) |
| Workspace files | 256 KiB per file, 4 MiB and 512 files per app. Editor changes are delta-based; the remaining ceiling is a host safety quota. | [6](06-apps-that-move-files.md) | [LIMITATIONS.md §7](../LIMITATIONS.md) |
...
| Embeddings and vector search | `ai.embed` accepts at most 64 inputs of 16,384 characters and 4,096 dimensions; `ai.search` ranks at most 63 app-supplied documents per call and stores no index. Requires a separately configured embedding model and `ai:embed` grant. | [7](07-apps-that-use-a-model.md#ask-the-handbook) | [docs/miniapp-sdk.md](../docs/miniapp-sdk.md) |
| Mini-app announce discovery | The SDK/broker and local snapshot service work, but shipped desktop, mobile, and web host wrappers do not connect `AnnounceService` to Reticulum. Cookbook tests inject peer events into one service; separate hosts do not exchange them. | [5](05-apps-that-find-each-other.md#how-discovery-works) | [Mini-app runtime](../docs/miniapp-runtime.md), [Reticulum announces](https://reticulum.network/manual/understanding.html#public-key-announcements) |
| `announce.subscribe()` | Resolves once with a snapshot of announces buffered by the host adapter — not a live push stream. Nothing arrives while closed, and later announces are only seen by re-polling. Desktop uses Reticulum; hosts without transport effects retain the local fallback. | [5](05-apps-that-find-each-other.md#how-discovery-works) | [docs/miniapp-sdk.md](../docs/miniapp-sdk.md) |
