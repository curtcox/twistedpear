...
describe("desktop peer chrome", () => {
  it("routes full manual codes and peer confirmation through trusted request/reply messages", async () => {
    ...
    const chrome = createDesktopPeerChrome({
      ...
      async requestReply(message) {
        seen.push(message);
        if (message.type === "peer-manual-present" && message.expectsResponse)
          return { accepted: true, code: "answer-code" };
    ...
    const answers = [];
    for await (const code of chrome.manual.offer(
      { id: "session", kind: "manual" },
      "offer-code",
      { timeoutMs: 1_000 },
    ))
      answers.push(code);
      ...
      async requestReply(message) {
        ...
        return {
          availability: {
            ...
            reason: "Camera permission is required",
          },
        };
      },
    });
    await expect(chrome.qr.availability()).resolves.toMatchObject({
      state: "permission-required",
    });
  });
  ...
  it("reports audio availability without requesting microphone permission", async () => {
    ...
    await expect(chrome.audio.availability()).resolves.toMatchObject({
      state: "permission-required",
    });
  });
  ...
  it("routes ntfy secret exchange through trusted chrome with the configured server disclosure", async () => {
    ...
    const chrome = createDesktopPeerChrome({
      ...
      async requestReply(message) {
        ...
        if (message.type === "peer-ntfy-enter")
          return { accepted: true, code: "TPN1-secret" };
        return { accepted: true };
      },
    });
    await expect(chrome.ntfy.availability()).resolves.toMatchObject({
      state: "available",
    });
    await expect(
      chrome.ntfy.presentCode({ id: "offer", kind: "ntfy" }, "TPN1-offer", {
        timeoutMs: 1_000,
      }),
    ).resolves.toBeUndefined();
    await expect(
      chrome.ntfy.requestCode(
        { id: "join", kind: "ntfy" },
        { service: "chat", timeoutMs: 1_000 },
      ),
    ).resolves.toBe("TPN1-secret");
    expect(seen.map(({ type }) => type)).toEqual([
      "peer-ntfy-availability",
      "peer-ntfy-present",
      "peer-ntfy-enter",
    ]);
    expect(seen[1]).toMatchObject({
      server: "https://rendezvous.example/",
      code: "TPN1-offer",
    });
  });
  ...
  it("keeps confirmation and permission prompts in host chrome outside the mini-app widget root", () => {
    ...
    expect(html.indexOf('id="host-modal-overlay"')).toBeGreaterThan(
      html.indexOf('id="widget-root"'),
    );
  ...
  it("keeps Peer Link on the public broker API and exposes host diagnostics", () => {
    ...
    const manifest = JSON.parse(
      readFileSync("apps/peer-link/app.manifest.json", "utf8"),
    );
    expect(source).toContain('from "@twistedpear/miniapp-sdk"');
    expect(source).toContain("peers.diagnostics()");
    expect(source).not.toMatch(
      /peer-discovery|BarcodeDetector|getUserMedia|RTCPeerConnection|Bluetooth|ntfy/i,
    );
