#!/usr/bin/env node
/**
 * Long-running publisher peer that announces the Handbook for Android emulator UI tests.
 */
...
const metaPath = join(dirname(fileURLToPath(import.meta.url)), "handbook-fixture-meta.json");
...
async function main() {
  ...
  const entry = catalog.ingest({
    destinationHash: meta.destinationName ?? "emulator-handbook-peer",
  ...
  console.log(
    `[android-emulator/handbook-peer] handbook ${entry.appId} v${entry.version} on ${LEAF_HOST}:${LEAF_PORT}`
  );
