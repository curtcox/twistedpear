...
export function runIosInterfacePolicy() {
  ...
  if (ranked.map((entry) => entry.kind).join(",") !== "auto,tcp,ble") {
    throw new Error(
      `unexpected interface rank order: ${ranked.map((entry) => entry.kind).join(",")}`,
    );
  }
  ...
  if (preferred?.name !== "harness-tcp") {
    throw new Error(
      `expected harness-tcp when auto is offline, got ${preferred?.name ?? "null"}`,
    );
  }
  ...
  if (blePreferred?.name !== "harness-ble") {
    throw new Error(
      `expected harness-ble as last online interface, got ${blePreferred?.name ?? "null"}`,
    );
  }
  ...
  console.log(
    "[ios-sim/interface-policy] AutoInterface > TCP > BLE ranking verified for harness interface names",
  );
}
