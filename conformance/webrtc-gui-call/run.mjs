 ...
 * Android offer: LOCAL_MULTIPEER_REQUIRED=1 node conformance/webrtc-gui-call/run.mjs --peers=hub,android,desktop
 *
...
function parseWanted() {
  ...
  const raw =
    arg?.slice("--peers=".length) ?? fromEnv ?? "hub,desktop,desktop2";
  return raw
    .split(",")
    .map((id) => id.trim())
    .filter(Boolean);
...
function proofFileName(leftId, rightId) {
  if (leftId.startsWith("web") || rightId.startsWith("web"))
    return "webrtc-gui-call-web-proof.json";
  if (
    leftId === "ios" ||
    rightId === "ios" ||
    leftId === "android" ||
...
await runMain(async () => {
  ...
  assert(
    wanted.length >= 3 && wanted[0] === "hub",
    "peers must start with hub and include two GUI peers",
  );
  ...
  try {
    ...
    for (const id of [leftId, rightId]) {
      ...
      assert(
        ping.ok === true,
        `${id} renderer ping failed: ${ping.error ?? "unknown"}`,
      );
    ...
    if (opusDuplex) {
      ...
      const duplexTimeoutMs =
        ...
        leftId === "android"
          ? 120_000
          : 30_000;
      const leftDuplex = await control.command(
        leftId,
        "media-opus-duplex",
        {},
        duplexTimeoutMs,
      ...
      assert(
        leftDuplex.voiceDuplex === true,
        `${leftId} Opus duplex missing voiceDuplex`,
      );
      assert(
        typeof leftDuplex.opusBytes === "number" && leftDuplex.opusBytes > 0,
        `${leftId} Opus encode empty`,
      );
      assert(leftDuplex.played === true, `${leftId} local Opus play failed`);
      step(
        `${leftId} Opus ${leftDuplex.pcmBytes}→${leftDuplex.opusBytes}→${leftDuplex.decodedBytes} bytes, played (${leftDuplex.implementation})`,
      );

      const rightDuplex = await control.command(
        rightId,
        "media-opus-duplex",
        {},
        duplexTimeoutMs,
      ...
      step(
        `${rightId} Opus duplex local play ok (${rightDuplex.implementation})`,
      );

      section("Cross-peer Opus speaker play");
      const crossAt = Date.now();
      assert(
        typeof leftDuplex.frameHex === "string" &&
          leftDuplex.frameHex.length > 72,
        `${leftId} missing Opus TPD2 frame`,
      );
      ...
      step(
        `${leftId} → ${rightId} Opus TPD2 played (${played.bytes} bytes, ${crossElapsedMs}ms)`,
      );
      callsOpusDuplex = {
        from: leftId,
        to: rightId,
    ...
    try {
      for (const id of [leftId, rightId]) {
        ...
        assert(
          typeof info.lxmfAddress === "string",
          `${id} lost its peer-agent attachment`,
        );
      ...
      const sent = await control.sendInvite(leftId, rightLxmf, "line-check", [
        "microphone",
      ]);
      assert(
        typeof sent.inviteId === "string",
        `${leftId} did not send a session invite`,
      );
      ...
      while (Date.now() < deadline) {
        try {
          ...
          step(
            `${rightId} invite-state poll: ${error instanceof Error ? error.message : String(error)}`,
          );
      ...
      assert(
        ...
        `${rightId} did not accept the invite`,
      );
      step(`${rightId} accepted invite ${raised.id}`);
      inviteId = raised.id;

      section("Pair WebRTC");
      await control.command(leftId, "peer-pair-start", {
        role: "offer",
        appId: "line-check",
      });
      await control.command(rightId, "peer-pair-start", {
        role: "listen",
        appId: "line-check",
      });

      const offer = await control.command(
        leftId,
        "peer-pair-code-out",
        {},
        PAIR_TIMEOUT_MS,
      ...
      assert(
        typeof offer.code === "string" && offer.code.length > 0,
        "offer peer did not publish a manual code",
      );
      step(`${leftId} published offer code (${offer.code.length} chars)`);
      await control.command(rightId, "peer-pair-code-in", {
        code: offer.code,
        sessionId: offer.sessionId,
      });

      const answer = await control.command(
        rightId,
        "peer-pair-code-out",
        {},
        PAIR_TIMEOUT_MS,
      ...
      assert(
        typeof answer.code === "string" && answer.code.length > 0,
        "listen peer did not publish an answer code",
      );
      step(`${rightId} published answer code (${answer.code.length} chars)`);
      await control.command(leftId, "peer-pair-code-in", {
        code: answer.code,
        sessionId: answer.sessionId,
      });

      left = await control.command(
        leftId,
        "peer-pair-wait",
        {},
        PAIR_TIMEOUT_MS,
      );
      right = await control.command(
        rightId,
        "peer-pair-wait",
        {},
        PAIR_TIMEOUT_MS,
      ...
      assert(
        left.dataPlane === "webrtc",
        `${leftId} dataPlane is ${left.dataPlane}, expected webrtc`,
      );
      assert(
        right.dataPlane === "webrtc",
        `${rightId} dataPlane is ${right.dataPlane}, expected webrtc`,
      );
      step(
        `paired ${leftId}↔${rightId} over webrtc (${left.displayLabel} / ${right.displayLabel})`,
      );
      ...
      const attachTrack = async (fromId, handleId, toId, toLxmf) => {
        ...
        assert(
          ...
          `${fromId} WebRTC media attach failed: ${attached.error ?? "unknown"}`,
        );
        assert(
          attached.plane === "webrtc-track",
          `${fromId} unexpected plane ${attached.plane}`,
        );
        assert(
        ...
        step(
          `${fromId} attached microphone track (${attached.bytesSent} bytesSent, ${tookMs}ms)`,
        );
      ...
      step(
        `invite/WebRTC path failed after Opus evidence: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    const proof = {
      ...
      ...(callsOpusDuplex === null
        ? {}
        : { callsOpusDuplex: [callsOpusDuplex] }),
    };
    ...
    if (webRtcEntries !== null) {
      for (const entry of webRtcEntries) {
        step(
          `callsWebRtc ${entry.from}→${entry.to} bytes=${entry.bytes} plane=${entry.plane}`,
        );
      }
    }
    if (callsOpusDuplex !== null) {
      step(
        `callsOpusDuplex opusBytes=${callsOpusDuplex.opusBytes} peerPlayed=true`,
      );
    }
    ...
    if (webRtcEntries === null && callsOpusDuplex === null) {
      assert(
        false,
        "neither WebRTC track bytes nor Opus duplex evidence was recorded",
      );
    }
    ...
    if (webRtcEntries === null && !(mobilePeer && callsOpusDuplex !== null)) {
      assert(
        false,
        "Opus duplex recorded, but invite/WebRTC path did not complete",
      );
    }
  } finally {
    await control?.close();
