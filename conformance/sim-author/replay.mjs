...
const path = resolve(requestedPath ?? "conformance/sim-author/fixtures/model-authored-duplicate.json");
...
if (requestedPath === undefined) {
  const provenance = JSON.parse(readFileSync(resolve("conformance/sim-author/model-authored-provenance.json"), "utf8"));
  const retained = provenance.accepted?.some((entry) => JSON.stringify(entry.proposal) === JSON.stringify(bundle.proposal));
  const execution = provenance.executions?.find((entry) => JSON.stringify(entry.proposal) === JSON.stringify(bundle.proposal));
  if (!retained || execution?.finding?.oracle !== bundle.expectedOracle ||
