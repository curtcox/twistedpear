...
if (command.length === 0)
  throw new Error("usage: npm run sim:author -- -- <model-command> [args...]");

const context = {
  objective:
    process.env.SIM_ATTACK_OBJECTIVE ??
    "Find authority replay, revocation, duplication, and containment failures",
  allowedPowers: ["drop", "delay", "reorder", "duplicate", "inject"],
  nodes: (process.env.SIM_ATTACK_NODES ?? "app,host,peer,relay")
    .split(",")
    .filter(Boolean),
  channels: (process.env.SIM_ATTACK_CHANNELS ?? "grant,broker,link,key-share")
    .split(",")
    .filter(Boolean),
  maxProposals: 16,
};
...
const result = await authorAttackStrategies(
  (prompt) => runModelCommand(command, prompt),
  context,
);
const executions = executeAuthoredStrategies(
  result.accepted,
  integerEnv("SIM_ATTACK_SEED", 44),
);
const output = {
  ...
  provenance: {
    ...
    command,
    generatedAt:
      process.env.SIM_ATTACK_GENERATED_AT ?? new Date().toISOString(),
...
if (destination === undefined) process.stdout.write(body);
  ...
  console.error(
    `wrote ${result.accepted.length} executed strategies to ${destination}`,
  );
}
...
if (fixtureDestination !== undefined) {
  const retained =
    executions.find((entry) => entry.minimized !== null) ?? executions[0];
  if (retained === undefined)
    throw new Error("model produced no accepted strategy to retain");
  const fixture = {
    ...
    history:
      retained.minimized === null
        ? null
        : JSON.parse(serializeHistory(retained.minimized)),
  };
  mkdirSync(dirname(resolve(fixtureDestination)), { recursive: true });
  writeFileSync(
    fixtureDestination,
    `${JSON.stringify(fixture, bytesReplacer, 2)}\n`,
  );
}
...
function runModelCommand([executable, ...args], prompt) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill();
      reject(new Error("strategy model timed out"));
    }, 120_000);
