...
const DISCOVERY_TIMEOUT_MS = timeoutFromEnv(
  "LOCAL_MULTIPEER_DISCOVERY_MS",
  90_000,
);
...
function requestedPeers() {
  ...
  if (flag !== undefined) {
    return [
      ...new Set(flag.slice("--peers=".length).split(",").filter(Boolean)),
    ];
  }
  if (smoke) {
    return ["hub", "node2"];
  ...
  if (attach) {
    ...
    assert(
      recorded.length > 0,
      "no peers are running; start some with `npm run peers -- up ...`",
    );
    return recorded;
  }
  return ["hub", "node2"];
  ...
  try {
    for (const id of ordered) {
      ...
      try {
        recordPeer(
          id,
          await adapter.up({ log: (line) => step(line), build: false }),
        );
        started.push(id);
        step(`${id} started`);
      } catch (error) {
...
await runMain(async () => {
  ...
  assert(
    wanted.length >= 2,
    `need at least two peers, got: ${wanted.join(", ") || "none"}`,
  );
  ...
  try {
    ...
    const attachResults = await Promise.all(
      expected.map(async (id) => {
        try {
          return {
            id,
            agent: await control.waitForAgent(id, ATTACH_TIMEOUT_MS),
          };
        } catch (error) {
          if (GUI_PEER_IDS.includes(id) && !required) {
            return { id, error };
    ...
    for (const result of attachResults) {
      ...
      step(
        `${id} attached (${agent.platform}, lxmf ${agent.lxmfAddress.slice(0, 12)}…)`,
      );
      attached.push(id);
    }
    assert(
      attached.length >= 2,
      `only ${attached.length} peer(s) attached: ${attached.join(", ")}`,
    );
    ...
    step(
      `${flooder}→${observer} announce-burst sent=${burst.sent} failed=${burst.failed}`,
    );
    ...
    const absentPeer =
      observerStatus.dropCensus.byPeer?.["never-seen-destination"];
    assert(
      ...
      for (const to of attached) {
        ...
        await waitUntil(
          ...
          async () => {
            ...
            return inbox.some(
              (entry) => entry.nonce === nonce && entry.kind === "probe",
            );
          },
          MESSAGE_TIMEOUT_MS,
        );
    ...
    for (const from of attached) {
      for (const to of attached) {
        ...
        await waitUntil(
          ...
          async () =>
            (await control.linkState(to)).readiness.some(
        ...
        await waitUntil(
          ...
          async () =>
            (await control.linkState(from)).readiness.some(
              (entry) => entry.kind === "response",
            ),
          MESSAGE_TIMEOUT_MS,
        );
        const answer = (await control.linkState(from)).readiness.findLast(
          (entry) => entry.kind === "response",
        );
        ...
        const reparsed = parseMediaReadiness(
          decodeLinkControl(
            encodeReadinessEnvelope(READINESS_REQUEST_ID, answer.readiness),
          ).payload,
        );
        assert(
          reparsed !== null,
          `${to} answered with a readiness body that does not validate`,
        );
        assert(
          answer.readiness.consentPosture !== "closed" &&
            answer.readiness.expiresAt > Date.now(),
          `${to} answered with an expired or closed readiness posture`,
        );
        const elapsedMs = Date.now() - at;
        step(
          `${from} → ${to} readiness ${answer.readiness.downlinkBucket}/${answer.readiness.consentPosture} (${elapsedMs}ms)`,
        );
    ...
    for (const from of attached) {
      for (const to of attached) {
        if (from === to) continue;
        const { probeId, budgetBytes } = await control.linkProbe(
          from,
          addresses.get(to),
          PROBE_BUDGET_BYTES,
        );
        assert(
          typeof probeId === "string" && budgetBytes === PROBE_BUDGET_BYTES,
          `${from} did not accept the probe budget`,
        );
        await waitUntil(
          `${from} never measured a probe reply from ${to}`,
          async () =>
            (await control.linkState(from)).probes.some(
              (entry) => entry.id === probeId && entry.rttMs !== null,
            ),
          MESSAGE_TIMEOUT_MS,
        );
        const measured = (await control.linkState(from)).probes.find(
          (entry) => entry.id === probeId,
        );
        assert(
          measured.rttMs > 0,
          `${from} recorded a non-positive probe RTT to ${to}`,
        );
        step(
          `${from} → ${to} probe ${budgetBytes} bytes, rtt ${measured.rttMs}ms`,
        );
        results.probes.push({
          from,
          to,
    ...
    for (const from of attached) {
      for (const to of attached) {
        if (from === to) continue;
        const at = Date.now();
        const sent = await control.sendInvite(
          from,
          addresses.get(to),
          "line-check",
          ["microphone"],
        ...
        assert(
          typeof sent.inviteId === "string",
          `${from} did not accept the invite request`,
        );
        await waitUntil(
          `${to} never raised ${from}'s session invite`,
          async () =>
            (await control.inviteState(to)).some(
              (entry) =>
                entry.kind === "raised" && entry.id.endsWith(sent.inviteId),
            ),
          MESSAGE_TIMEOUT_MS,
        );
        const raised = (await control.inviteState(to)).findLast(
          (entry) =>
            entry.kind === "raised" && entry.id.endsWith(sent.inviteId),
        );
        ...
        assert(
          raised.peerLabel.length > 0 &&
            !raised.peerLabel.includes(sent.inviteId),
          `${to} took the invited peer's label from the sender`,
        );
        assert(
          raised.appId === "line-check",
          `${to} raised an invite for the wrong app`,
        );
        assert(
          raised.expiresAt > Date.now(),
          `${to} raised an already-expired invite`,
        );
        const elapsedMs = Date.now() - at;
        step(
          `${from} → ${to} invite ${raised.requestedClasses.join("+")} as "${raised.peerLabel}" (${elapsedMs}ms)`,
        );
        results.invites.push({
          ...
          id: raised.id,
          appId: raised.appId,
          requestedClasses: raised.requestedClasses,
          elapsedMs,
        });
      }
    }
    ...
    for (const invite of results.invites) {
      ...
      assert(
        ...
        `${invite.to} did not accept ${invite.from}'s invite`,
      );
      assert(
        typeof accepted.peerDestinationHash === "string" &&
          accepted.peerDestinationHash.length > 0,
        `${invite.to} accepted without a peer destination`,
      );
      ...
      await waitUntil(
        ...
        async () =>
          (await control.callInbox(invite.from)).some(
            (entry) =>
              entry.nonce === nonce &&
              entry.kind === "payload" &&
              entry.payloadHex === payloadHex,
          ),
        MESSAGE_TIMEOUT_MS,
      );
      await waitUntil(
        ...
        async () =>
          (await control.callInbox(invite.to)).some(
            (entry) =>
              entry.nonce === nonce &&
              entry.kind === "echo" &&
              entry.payloadHex === payloadHex,
      ...
      const elapsedMs = Date.now() - at;
      step(
        `${invite.to} accepted → ${invite.from} call media (${frame.length} bytes, ${elapsedMs}ms)`,
      );
      results.calls.push({
        from: invite.to,
        to: invite.from,
    ...
    for (const from of attached) {
      for (const to of attached) {
        ...
        const payloads = [
          {
            kind: "derived",
            bytes: encodeMediaFrame(
              `media-derived-${suffix}`,
              encodeDeviceStreamFrame({
                version: 2,
                sampleKind: 5,
                sessionToken: 1,
                ...
                payload: new TextEncoder().encode(
                  '{"kind":"microphone","tier":"derived","voiceActive":true}',
                ),
              }),
            ),
          },
          {
            ...
            bytes: encodeMediaFrame(
              ...
              encodeDeviceStreamFrame({
                ...
                captureAtUs: Date.now() * 1_000,
                clockId: 2,
                payload: new Uint8Array(new Float32Array(320).buffer),
              }),
            ),
          },
        ];
        for (const payload of payloads) {
          ...
          await waitUntil(
            `${to} never received ${payload.kind} realtime payload from ${from}`,
            async () =>
              (await control.realtimeInbox(to)).some(
                (entry) =>
                  entry.nonce === nonce &&
                  entry.kind === "payload" &&
                  entry.payloadHex === payloadHex,
          ...
          await waitUntil(
            `${from} never received ${to}'s ${payload.kind} realtime echo`,
            async () =>
              (await control.realtimeInbox(from)).some(
                (entry) =>
                  entry.nonce === nonce &&
                  entry.kind === "echo" &&
                  entry.payloadHex === payloadHex,
          ...
          const elapsedMs = Date.now() - at;
          step(
            `${from} → ${to} → ${from} ${payload.kind} carrier (${payload.bytes.length} bytes, ${elapsedMs}ms)`,
          );
          results.realtime.push({
            from,
            to,
            kind: payload.kind,
    ...
    if (required) {
      ...
      for (const id of wanted.filter((peerId) =>
        GUI_PEER_IDS.includes(peerId),
      )) {
        assert(
          attached.includes(id),
          `required GUI peer ${id} was not attached`,
        );
        assert(
        ...
        step(
          `${id} recorded in readiness, probes, invites, calls, and realtime`,
        );
      }
    }
