...

  {
    ...
    const actual =
      ...
      !store.entries("board").some((entry) => entry.payload === "forged")
        ? "BLOCKED"
        : "UNCONTROLLED";
    expectVerdict("HP-FORGE", "BLOCKED", actual);
    counts[actual] += 1;
  }

  {
    ...
    const actual =
      ...
      store.view("board").get("slot")?.payload === "held"
        ? "BLOCKED"
        : "UNCONTROLLED";
    expectVerdict("HP-CROSS-TOMB", "BLOCKED", actual);
    counts[actual] += 1;
  }

  {
    ...
    const actual =
      result.rejected[0]?.reason === "unoffered-author"
        ? "BLOCKED"
        : "UNCONTROLLED";
    expectVerdict("HP-UNOFFERED", "BLOCKED", actual);
    counts[actual] += 1;
  }
...

  {
    ...
    const actual =
      compiled.powers[0] === "author-flood" && refused
        ? "CONTAINED"
        : "UNCONTROLLED";
    expectVerdict("HP-FLOOD-MODEL", "CONTAINED", actual);
    counts[actual] += 1;
  }
