#!/usr/bin/env node
/**
 * W4 Playwright: WebSerial RNode interface online via simulated navigator.serial.
 */
...
function runBuild() {
  const build = spawnSync("node", ["conformance/web-rnode/build.mjs"], {
    cwd: repoRoot,
    stdio: "inherit"
  });
  if (build.status !== 0) {
...
async function runPlaywright(pageUrl) {
  ...
  try {
    ...
    try {
      await page.waitForFunction(() => globalThis.__WEB_RNODE__?.status === "done", undefined, {
        timeout: 60_000
      });
    } catch (error) {
      let snapshot = null;
...
try {
  runBuild();
  staticServer = await startStaticServer(rnodeRoot);
  const pageUrl = `http://127.0.0.1:${staticServer.port}/`;
  const result = await runPlaywright(pageUrl);
  console.log(`web-rnode: ${JSON.stringify(result)}`);
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`web-rnode: failed — ${message}`);
  process.exit(1);
} finally {
