...
function runBenchmark() {
  ...
  return {
    ...
    results: parsed.results.bare.map(({ name, opsPerSec }) => ({
      name,
      opsPerSec,
    })),
  };
...
async function main() {
  ...
  if (compare) {
    ...
    if (regressions.length > 0) {
      console.error(
        "Bare sodium benchmark regressions (>50% slower than baseline):",
      );
  ...
  writeFileSync(
    baselinePath,
    `${JSON.stringify({ ...current, recordedAt: new Date().toISOString().slice(0, 10), host: "ci-reference" }, null, 2)}\n`,
  );
