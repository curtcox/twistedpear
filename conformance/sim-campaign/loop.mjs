...
const loopConfig = JSON.parse(
  readFileSync("conformance/sim-campaign/loop-config.json", "utf8"),
);
...
try {
  ...
  if (!readFileSync(first).equals(readFileSync(second)))
    throw new Error("fixed replay was not byte-identical");
  const fixedReplaySha256 = createHash("sha256")
    .update(readFileSync(first))
    .digest("hex");
  if (fixedReplaySha256 !== loopConfig.fixedReplaySha256) {
    throw new Error(
      `fixed replay baseline moved: expected ${loopConfig.fixedReplaySha256}, received ${fixedReplaySha256}`,
    );
  }
  run("sim:report");
...
function run(script, trailing = []) {
  const result = spawnSync(npm, ["run", script, ...trailing], {
    cwd: process.cwd(),
    env: process.env,
    stdio: "inherit",
  });
