...
const recorder = new FileHistoryRecorder(reproducers, (path, contents) =>
  writeFileSync(path, `${contents}\n`),
);
const registry = createProductionScenarioRegistry({
  cells,
  defectIds: new Set(canaryIds),
  recorder,
});
if (registry.supportedCells.length !== cells.length)
  throw new Error("scenario registry does not cover every scheduled cell");

const options = {
  cells,
  seeds: { from: seedFrom, to: seedTo },
  scenario: registry.create,
  parallelism: 8,
};
const first = await runCampaign(options);
const second = await runCampaign(options);
if (serializeCampaignReport(first) !== serializeCampaignReport(second))
  throw new Error("campaign rerun was not byte-identical");

const split = Math.floor((seedFrom + seedTo) / 2);
const captured = (minimum, maximum) =>
  new Set(
    first.canaryFindings
      .filter((finding) => finding.seed >= minimum && finding.seed <= maximum)
      .map((finding) => finding.cell),
  );
const completeness = estimateCompleteness({
  canaryIds,
  firstCapture: captured(seedFrom, split),
  secondCapture: captured(split + 1, seedTo),
});
const baseline = JSON.parse(
  readFileSync("conformance/sim-baselines/containment.json", "utf8"),
);
const regressions = [
  ...containmentRegressions(first.containment, baseline.containment),
];
if (completeness.floor < baseline.canaryFloorMin)
  regressions.push(
    `canary floor ${completeness.floor} is below ${baseline.canaryFloorMin}`,
  );
if (first.findings.length > 0)
  regressions.push(
    `${first.findings.length} genuine oracle violation(s) found`,
  );
const report = {
  ...first,
  difficulty: {
...
console.log(
  `simulation campaign: ${first.scenariosRun} real scenarios, ${first.canaryFindings.length} canary findings, ${first.findings.length} genuine findings, floor ${completeness.floor.toFixed(3)}`,
);
if (regressions.length > 0)
  throw new Error(`simulation campaign regression:\n${regressions.join("\n")}`);
