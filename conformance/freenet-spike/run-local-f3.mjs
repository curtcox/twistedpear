...
const binary =
  ...
  (existsSync("/Applications/Freenet.app/Contents/MacOS/freenet-bin")
    ...
    : "freenet");
const wsPort = Number(process.env.FREENET_F3_WS_PORT ?? 17619);
const networkPort = Number(process.env.FREENET_F3_NETWORK_PORT ?? 31448);

assert(
  Number.isSafeInteger(wsPort) && wsPort >= 1 && wsPort <= 65535,
  `Invalid Freenet F3 ws port: ${wsPort}`
);
assert(
  Number.isSafeInteger(networkPort) && networkPort >= 1 && networkPort <= 65535,
  `Invalid Freenet F3 network port: ${networkPort}`
);

const root = mkdtempSync(join(tmpdir(), "twistedpear-freenet-f3-"));
...
async function main() {
  section("Freenet F3 isolated single-node proof");
  isolateHome();
  ...
  section("Offline-A / retrieve-B store proof");
  const proof = spawn(
    process.execPath,
    [join(import.meta.dirname, "prove-f3-propagation.mjs")],
    {
      env: {
        ...process.env,
        FREENET_NODE_URL: `ws://127.0.0.1:${wsPort}/v1/contract/command`,
        FREENET_F3_LABEL: "local-isolated"
      },
      stdio: "inherit"
    }
  );
  const exitCode = await new Promise((resolve, reject) => {
    ...
    proof.once("exit", (code, signal) => {
      if (signal !== null) reject(new Error(`F3 proof exited via ${signal}`));
      else resolve(code ?? 1);
    });
  });
  if (exitCode !== 0) {
    throw new Error(`F3 proof exited with status ${exitCode}`);
  }
}
...
await runMain(async () => {
  try {
    ...
    await stopNodes();
    if (keepState) {
      console.error(`Preserved Freenet F3 state at ${root}`);
    } else {
