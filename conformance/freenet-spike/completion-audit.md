...
| S8 privacy posture | complete for F1 | `docs/security-review.md`; mobile remote-node grant chrome + contract/packet-tunnel/propagation Bare worklet backends + Maestro probes (simulator software path) |
...
| F5 capability/UI | capability + desktop + mobile grant chrome | `freenet:contract` + HOST_API 0.11.0; desktop Settings; mobile remote-node disclosure/refusal/revoke/session + Bare worklet contract, packet-tunnel (`FreenetInterface`), and propagation-mirror backends (Maestro); web off per Option A; S4 BareKit measurements still probe-ready |
...
mobile remote-node grant/session chrome (including Bare worklet contract,
packet-tunnel, and propagation-mirror backends plus Maestro probes), and a
