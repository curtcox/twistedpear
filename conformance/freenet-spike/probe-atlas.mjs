...
const nodeUrl =
  process.env.FREENET_NODE_URL ?? "ws://127.0.0.1:7509/v1/contract/command";
const indexId =
  process.env.ATLAS_INDEX_ID ?? "CJUR37WSMxV7C1yhrr3xSgjnrJT5yuvQGFNcgvSnsvg";
...
try {
  ...
  if (!prefixText.includes("key_auth")) {
    throw new Error(
      "Atlas state does not match the pinned IndexState CBOR shape",
    );
  }
  const evidence = {
    schemaVersion: 1,
    ...
    stateSha256Hex: createHash("sha256").update(record.state).digest("hex"),
    keyRoundTripMatches: Buffer.from(record.key).equals(Buffer.from(key)),
