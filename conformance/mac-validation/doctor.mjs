...
check("macOS", true, () => {
  if (process.platform !== "darwin")
    throw new Error(`platform is ${process.platform}`);
  ...
  () => {
    const out = run(
      ...
      [
        "compose",
        "-f",
        join(repoRoot, "conformance/docker/docker-compose.yml"),
        "images",
        "--quiet",
      ],
      { timeout: 20000 },
...
check(
  ...
  () => {
    ...
    const version = run(join(home, "bin/java"), ["--version"], {
      timeout: 15000,
    });
    if (!/\b17\./.test(version))
      throw new Error(
        `Java 17 candidate resolved to ${home} (${version.split("\n")[0]})`,
      );
    return home;
  },
...
check(
  ...
  () => {
    ...
    if (!avds.includes("Pixel_8_API_34"))
      throw new Error(`AVDs: ${avds.join(", ") || "none"}`);
