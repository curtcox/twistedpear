...
def rns_packet_corpus() -> dict:
    ...
    def announce_entry(name: str, ratchet: bytes = b"") -> dict:
        signed_data = (
            destination_hash + alice.get_public_key() + name_hash + random_hash + ratchet + app_data
        )
...
def lxmf_message_corpus(identity_corpus: dict) -> dict:
    ...
    identities = {
        entry["name"]: load_identity_from_vector(entry) for entry in identity_corpus["identities"]
    }
