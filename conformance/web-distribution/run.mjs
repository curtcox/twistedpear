...
async function startPublisherGateway() {
  ...
  const nameHash = bytesToHex(
    provider
      .sha256(new TextEncoder().encode(verified.manifest.name))
      .slice(0, 8),
  );
  ...
  const reannounceTimer = setInterval(() => {
    void appDestination
      .announce({ appData: encodeAppAnnounceData(summary) })
      .catch(() => {});
    void casDestination
  ...
  try {
    ...
    try {
      await page.waitForFunction(
        () => globalThis.__WEB_DISTRIBUTION__?.status === "done",
        undefined,
        {
          timeout: 90_000,
        },
      ...
      const snapshot = await page.evaluate(
        () => globalThis.__WEB_DISTRIBUTION__ ?? null,
      );
      throw new Error(
    ...
    if (result?.status !== "done") {
      throw new Error(
        `web distribution spike incomplete: ${JSON.stringify(result)}`,
      );
    }
...
try {
  ...
  console.log(
    `web-distribution: ${JSON.stringify({ appId: result.appId, version: result.version })}`,
  );
} catch (error) {
