...
export class BrowserUiDriver {
  static async connect({ id, cdpPort }) {
    ...
    if (page === undefined)
      throw new Error(`${id}: CDP connected without a page`);
  ...
  command(command, payload = {}) {
    if (this.id !== "web")
      throw new Error("In-page commands are only available on the web peer");
    return this.page.evaluate(
      ({ command, payload }) =>
        globalThis.__TP_CROSS_DEVICE__.command(command, payload),
      { command, payload },
    );
  }
  ...
  async approveInstall() {
    if (this.id === "desktop") {
      const modal = this.page.locator("#host-modal");
      await modal
        .getByText("storage:kv", { exact: false })
    ...
    await this.page
      .getByTestId("host-confirmation-modal")
      .waitFor({ timeout: 60_000 });
