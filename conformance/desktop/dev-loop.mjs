#!/usr/bin/env node
/**
 * Desktop worklet dev-loop slice (Phase 6 M4): tp dev hot reload via dev channel + worklet host.
 */
...
export async function runDesktopDevLoop() {
  const started = Date.now();
  const workDir = mkdtempSync(join(tmpdir(), "tp-desktop-dev-loop-"));
  const beeDir = mkdtempSync(join(tmpdir(), "tp-desktop-dev-bee-"));
  const outbound = [];
