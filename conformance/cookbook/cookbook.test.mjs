...
import {
  fireTraceEvent,
  roundTripSession,
} from "../../packages/miniapp-test/dist/index.js";
import { createHost, fakeT256 } from "./fixtures.mjs";
...
// TRACE-3: every app records a scripted session and replays to an identical
// widget-patch stream. The script presses buttons the app itself renders, in
// document order, so it works for the whole corpus without per-app authoring.
// Only names that appear once are used: a shape trace records the event name,
// never the node id, so an ambiguous name would make replay pick for itself.
function eventNameCounts(node, counts = new Map()) {
  const event = node.props?.event;
  if (node.type === "button" && typeof event === "string") {
    counts.set(event, (counts.get(event) ?? 0) + 1);
  }
  for (const child of node.children ?? []) eventNameCounts(child, counts);
  return counts;
}

function nextScriptedInput(tree, fired) {
  if (tree === null) return null;
  for (const [event, count] of eventNameCounts(tree.root)) {
    if (count === 1 && !fired.has(event)) return event;
  }
  return null;
}

async function scriptCookbookSession(session, limit = 3) {
  const fired = new Set();
  for (let index = 0; index < limit; index += 1) {
    const event = nextScriptedInput(session.host.snapshot().widgetTree, fired);
    if (event === null) return;
    fired.add(event);
    await fireTraceEvent(session, event);
  }
}

function cookbookLaunchManifest(name) {
  const manifest = JSON.parse(
    readFileSync(join(appsRoot, name, "app.manifest.json"), "utf8"),
  );
  return {
    ...
    bundle: new Uint8Array(readFileSync(join(appsRoot, name, manifest.entry))),
  };
}
...
describe.each(appNames)("cookbook app %s", (name) => {
  ...
  it("records a scripted session and replays it identically", async () => {
    const report = await roundTripSession({
      ...cookbookLaunchManifest(name),
      host: async (context) => (await createHost(name, context)).host,
      script: (session) => scriptCookbookSession(session),
    });
    // The tape and the patch stream are the assertions. `at` is not: it counts
    // host clock reads, and async completion order can shift that by a tick
    // without changing what the app did. `clockDrift` reports it instead.
    expect(report.divergences, `${name} replayed tape`).toEqual([]);
    expect(report.patchesMatch, `${name} widget-patch stream`).toBe(true);
    expect(report.patches.length, `${name} rendered nothing`).toBeGreaterThan(
      0,
    );
  }, 30_000);
});
