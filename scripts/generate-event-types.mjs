...
export function generateEventTypes(schemaPath = SCHEMA_PATH) {
  ...
  const typeOf = (node) => {
    ...
    if (node.enum !== undefined)
      return node.enum.map((item) => JSON.stringify(item)).join(" | ");
    ...
    throw new Error(
      `cannot map schema node to TypeScript: ${JSON.stringify(node)}`,
    );
  };
  ...
  const propertyLine = (name, node, required) => {
    ...
    if (node["x-presence"] === "explicit-undefined")
      return `readonly ${name}: ${type} | undefined`;
