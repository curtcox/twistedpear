...
function strikeRow(text, label, evidencePath) {
  ...
  if (index < 0)
    throw new Error(`could not find open register row for ${label}`);
...
export function record({
  ...
  if (!["started", "passed", "failed"].includes(status))
    throw new Error("status must be started, passed, or failed");
  if (!log && status !== "started")
    throw new Error("--log is required for passed or failed evidence");
  const generatedLog = join(
    root,
    "release/evidence-logs",
    `${safeId(id)}-started.log`,
  );
  if (!log)
    replaceAtomic(
      generatedLog,
  ...
  if (!existsSync(absoluteLog))
    throw new Error(`evidence log does not exist: ${absoluteLog}`);
  const logText = readFileSync(absoluteLog, "utf8");
  if (
    status === "passed" &&
    /\[mac-validation\] exit: (?!0\s*$)/m.test(logText)
  )
    throw new Error("refusing to record a failed validation log as passed");
  const filename = `${safeId(id)}.json`;
  const evidenceFile = join(root, "release/evidence", filename);
  const recordValue = {
    schema: "twistedpear.release-evidence-v1",
    id,
    status,
    at,
  ...
  if (!complete.includes("## Release evidence log"))
    complete +=
      "\n## Release evidence log\n\n| ID | Completed | Evidence | Note |\n|---|---|---|---|\n";
  if (!complete.includes(`| ${id} |`))
    complete += `| ${id} | ${at} | [record](release/evidence/${filename}) · [log](${recordValue.log}) | ${note.replaceAll("|", "\\|")} |\n`;
...
function main(argv = process.argv.slice(2)) {
  ...
  if (!id)
    throw new Error(
      "usage: release:record <id> --status <started|passed|failed> [--log <path>] [--note text]",
    );
  const value = (flag) => {
