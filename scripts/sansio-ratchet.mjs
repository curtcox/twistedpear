...
function main() {
  ...
  const excepted = new Set(
    ratchet.exceptions.map((e) => (typeof e === "string" ? e : e.file)),
  );
  ...
  const growth = [...excepted].filter(
    (f) =>
      !violating.has(f) === false && !report.filesWithViolations.includes(f),
  );
  ...
  if (gitShow.status === 0 && gitShow.stdout) {
    try {
      previousExceptions = new Set(
        JSON.parse(gitShow.stdout).exceptions.map((e) =>
  ...
  if (added.length > 0) {
    failed = true;
    console.error(
      "Sans-IO ratchet: exception list grew (only shrinkage allowed):",
    );
  ...
  if (stale.length > 0) {
    console.warn(
      "Sans-IO ratchet: stale exceptions (file no longer violates — remove from list):",
    );
