...
export function readJson(file, fallback = NO_FALLBACK) {
  try {
    ...
    if (fallback !== NO_FALLBACK && /** @type {NodeJS.ErrnoException} */ (error).code === "ENOENT") {
      return fallback;
    }
    throw error;
  }
...
export function compareDiagnosticSet({
  ...
  const baseline = readJson(baselineFile, { version: 1, description, entries: [] });
  const previousEntries = new Set(baseline.entries ?? []);
  const currentEntries = [...new Set(current)].sort();
  const additions = currentEntries.filter((entry) => !previousEntries.has(entry));
  const stale = [...previousEntries].filter((entry) => !currentEntries.includes(entry)).sort();
  ...
  if (write) {
    ...
    writeJson(baselineFile, { version: 1, description, entries: currentEntries });
...
export function printDiagnosticResult(label, result) {
  ...
  for (const entry of result.baselineGrowth ?? []) console.error(`  baseline + ${entry}`);
  if ((result.stale ?? []).length > 0) {
    console.warn(`${label}: ${(result.stale ?? []).length} stale baseline finding(s); run the baseline command to tighten it.`);
  }
  const failed = (result.additions ?? []).length > 0 || (result.baselineGrowth ?? []).length > 0;
  console.log(
