...
const parserExamples = {
  ...
  colon: { kind: "colon" },
  comma: { kind: "comma" },
  "array-open": { kind: "array-open" },
  "array-close": { kind: "array-close" },
...
function parserEvent(state, eventClass) {
  ...
  const keys = {
    "expect-appId-key": "appId",
    "expect-publisher-key": "publisherPublicKey",
    "expect-granted-key": "granted",
    "expect-updated-key": "updatedAt",
  };
...
function makeParserVector() {
  const nearMisses = [
    {
      name: "duplicate-key",
      text: '{"appId":"a","appId":"b","publisherPublicKey":"p","granted":[],"updatedAt":1}',
    },
    {
      name: "reordered-fields",
      text: '{"publisherPublicKey":"p","appId":"a","granted":[],"updatedAt":1}',
    },
    {
      name: "whitespace",
      text: '{ "appId":"a","publisherPublicKey":"p","granted":[],"updatedAt":1}',
    },
    {
      name: "trailing-byte",
      text: '{"appId":"a","publisherPublicKey":"p","granted":[],"updatedAt":1}x',
    },
    {
      name: "non-canonical-number",
      text: '{"appId":"a","publisherPublicKey":"p","granted":[],"updatedAt":1.0}',
    },
  ].map((entry) => ({ ...entry, expected: "reject" }));
  return {
    schema: "twistedpear.parser-transition-v1",
    machine: "grant-parser",
    generatedBy: "scripts/vectors-generate-grant.mjs",
    states: grantParserMachine.states,
    ...
    cells: enumerateCells(grantParserMachine).map((cell) => ({
      ...
      event: parserEvent(cell.state, cell.eventClass),
      legal: cell.rows.length > 0,
    })),
    canonical:
      '{"appId":"a","publisherPublicKey":"p","granted":["read"],"updatedAt":1}',
    nearMisses,
  };
}
...
const vectors = [
  ...
  [
    ...
    makeVector(
      ...
      escrowExamples,
      (phase) => ({
        ...initialEscrowState(2),
        phase,
        amount: phase === "pending" ? 0 : 100,
      }),
      stepEscrow,
    ),
  ],
  [
    "recovery-quorum.json",
    makeVector(
      "recovery-quorum",
      recoveryQuorumMachine,
      recoveryExamples,
      (phase) => ({
        ...initialRecoveryQuorumState(2),
        phase,
        shares: phase === "idle" ? [] : ["a", "b"],
      }),
      stepRecoveryQuorum,
    ),
  ],
