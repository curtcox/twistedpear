...
for (const entry of policy.entries ?? []) {
  if (
    ...
    console.error(
      `Expired advisory exception ${entry.id} (${entry.expires}): ${entry.reason}`,
    );
    failed = true;
  }
}
...
if (full || baseline) {
  ...
  if (audit.error || audit.message) {
    console.error(
      `npm audit failed: ${audit.message ?? audit.error?.summary ?? "unknown error"}`,
    );
    process.exit(1);
  }
  ...
  if (baseline) {
    if (
      ...
      console.error(
        "Refusing to establish a non-empty advisory allowlist without --allow-regressions.",
      );
      process.exit(1);
    }
    ...
    writeJson(FILE, {
      version: 1,
      description:
        "Temporary high/critical npm advisory exceptions. Each exception requires a reason and expires; this list may not grow on a PR.",
      entries: vulnerable.map((item) => ({
        ...item,
        expires: expiry,
        reason:
          "Existing dependency graph when the advisory gate was established; upgrade or remove before expiry.",
      })),
    });
    console.log(
      `Advisory gate: wrote ${vulnerable.length} temporary exceptions.`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `Advisory gate: ${vulnerable.length} high/critical vulnerable packages, ${allowed.size} temporary exceptions.`,
  );
} else {
  console.log(
    `Advisory policy: ${(policy.entries ?? []).length} valid, unexpired temporary exceptions.`,
  );
}
