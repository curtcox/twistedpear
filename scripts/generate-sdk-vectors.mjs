...
const outputPath = join(here, "..", "specs", "spec-sdk", "vectors", "calls.json");
...
const app = (name, declared, granted = declared, register = true) => ({ name, declared, granted, register });
...
const INPUTS = [
  {
    ...
    steps: [
      ...
      { call: call("identity", "sign", "identity", { payload: { $textBytes: "payload" } }) }
    ]
  },
  {
  ...
  },
  {
    ...
    steps: [
      {
        call: call("ui", "render", undefined, { tree: { root: { id: "root", type: "carousel" } } }),
  ...
  },
  {
    ...
    steps: [
      { call: call("storage.kv", "set", "storage:kv", { key: "greeting", value: { $textBytes: "hello" } }) },
  ...
  },
  {
    ...
    steps: [
      {
        call: call("storage.kv", "set", "storage:kv", { key: "big", value: { $textBytes: "y".repeat(200) } }),
        expectCode: "BROKER_ERROR",
  ...
  },
  {
    ...
    steps: [
      ...
      { call: call("storage.bee", "put", "storage:hyperbee", { key: "post:1", value: { $textBytes: "first" } }) },
      { call: call("storage.bee", "get", "storage:hyperbee", { key: "post:1" }) },
      { call: call("storage.bee", "list", "storage:hyperbee", { options: { limit: 10 } }) }
    ]
  },
  {
    ...
    steps: [
      {
        call: call("storage.bee", "put", "storage:hyperbee", { key: "big", value: { $textBytes: "z".repeat(200) } }),
        expectCode: "BROKER_ERROR",
        messageIncludes: "quota"
      }
    ]
  },
  {
    ...
    steps: [
      { call: call("lxmf", "send", "lxmf:send", { to: "vector-app", subject: "hi", body: "note to self" }) },
  ...
  },
  {
    ...
    steps: [
      { call: call("announce", "publish", "announce:publish", { appData: { $textBytes: "post-1" }, namespace: "board" }) },
      { call: call("announce", "subscribe", "announce:subscribe", { namespace: "board" }) }
    ]
  },
  {
  ...
  },
  {
    ...
    steps: [
      { call: call("workspace", "write", "workspace", { path: "src/main.js", content: "console.log(1)" }) },
  ...
  },
  {
    ...
    steps: [
      ...
      { call: call("share.cas", "get", "share:cas", { t256: "3231c75f2b33d1a7cc41e8eb7618273f2311a3f43149a59979f869b87ed583ad" }) }
    ]
  },
  {
    name: "resource: fetch within budget",
    steps: [{ call: call("resource", "fetch", "resource:fetch", { resourceId: "offer:demo", budgetBytes: 4096 }) }]
  },
  {
    name: "resource: budget exceeded",
    steps: [
      {
        call: call("resource", "fetch", "resource:fetch", { resourceId: "offer:demo", budgetBytes: 4 }),
        expectCode: "BROKER_ERROR",
  ...
  },
  {
    ...
    steps: [
      {
        call: call("resource", "fetch", "resource:fetch", { resourceId: "offer:missing" }),
        expectCode: "BROKER_ERROR",
  ...
  },
  {
    ...
    app: app("rogue-app", ["identity", "network:raw"], ["identity", "network:raw"], false),
    steps: [
      { call: call("identity", "destinationHash", "identity"), expectCode: "UNKNOWN_CAPABILITY" }
    ]
  },
  {
    ...
    steps: [
      { call: call("storage.kv", "get", "storage:kv", { key: "x" }), expectCode: "UNDECLARED_CAPABILITY" }
    ]
  },
  {
    ...
    steps: [
      { call: call("storage.kv", "get", "storage:kv", { key: "x" }), expectCode: "CAPABILITY_DENIED" }
    ]
  },
  {
    name: "capability: request names the wrong capability",
    steps: [
      { call: call("storage.kv", "get", "identity", { key: "x" }), expectCode: "CAPABILITY_MISMATCH" }
    ]
  },
  {
    ...
    steps: [
      { call: call("lxmf", "send", "lxmf:send", { to: "x", subject: "s", body: "b" }), expectCode: "CAPABILITY_DENIED" },
      { call: call("announce", "publish", "announce:publish"), expectCode: "CAPABILITY_DENIED" },
      { call: call("presence", "snapshot", "presence"), expectCode: "CAPABILITY_DENIED" },
      { call: call("workspace", "list", "workspace"), expectCode: "CAPABILITY_DENIED" },
      { call: call("share.cas", "put", "share:cas", { content: "x" }), expectCode: "CAPABILITY_DENIED" },
      { call: call("resource", "fetch", "resource:fetch", { resourceId: "offer:demo" }), expectCode: "CAPABILITY_DENIED" }
    ]
  },
  {
    name: "broker: unknown method",
    steps: [{ call: call("identity", "teleport", "identity"), expectCode: "UNKNOWN_METHOD" }]
  },
  {
    name: "broker: rate limited",
    setup: { maxMessagesPerSecond: 4 },
    steps: [
      { call: call("identity", "destinationHash", "identity"), repeat: 6, expectCode: "RATE_LIMITED" }
    ]
  },
  {
    ...
    steps: [
      {
        call: call("storage.kv", "set", "storage:kv", { key: "big", note: { $asciiString: 300000 } }),
    ...
    steps: [
      {
        call: call("apps", "package", "apps:package", {
          projectPrefix: "proj",
          manifest: { name: "demo-app", version: "1.0.0", entry: "bundle.js", capabilities: [] }
        }),
  ...
  },
  {
    ...
    steps: [
      {
        call: call("ai", "chat", "ai:chat", { messages: [{ role: "user", content: "hi" }] }),
  ...
  },
  {
    ...
    host: "ai-configured",
    steps: [{ call: call("ai", "chat", "ai:chat", { messages: [{ role: "user", content: "hi" }] }) }]
  }
];
...
for (const input of INPUTS) {
  ...
  if (input.setup?.maxMessagesPerSecond !== undefined) {
    host.setResourceLimits(vectorApp.name, { maxMessagesPerSecond: input.setup.maxMessagesPerSecond });
  }
  ...
  for (const step of input.steps) {
    ...
    if (response.ok) {
      if (step.expectCode !== undefined) {
        throw new Error(`${input.name}: expected ${step.expectCode}, call succeeded`);
      }
      expect = { ok: true };
      if (match === "exact") expect.result = normalizeValue(response.result) ?? null;
      if (match === "keys") {
      ...
      if (step.expectCode === undefined) {
        throw new Error(`${input.name}: unexpected failure ${code}: ${response.error?.message}`);
      }
      if (code !== step.expectCode) {
        throw new Error(`${input.name}: expected ${step.expectCode}, got ${code}: ${response.error?.message}`);
      }
      if (step.messageIncludes !== undefined && !response.error.message.toLowerCase().includes(step.messageIncludes.toLowerCase())) {
        throw new Error(`${input.name}: message "${response.error.message}" lacks "${step.messageIncludes}"`);
      }
      seenCodes.add(code);
      expect = {
        ok: false,
        code,
        ...(step.messageIncludes === undefined ? {} : { messageIncludes: step.messageIncludes })
      };
    }
    steps.push({
      call: step.call,
...
if (missing.length > 0) {
  throw new Error(`error taxonomy not fully covered, missing: ${missing.join(", ")}`);
}
...
console.log(`wrote ${vectors.length} vectors (${seenCodes.size} taxonomy codes) -> ${outputPath}`);
process.exit(0);
