...
writeJson(path.join(ROOT, "format.json"), {
  version: 1,
  count: files.length,
  files,
});
const comparison = compareDiagnosticSet({
  ...
  description:
    "Files not yet mechanically formatted when the formatting gate landed; entries may only disappear.",
  envName: "FORMAT_RATCHET_BASE_REF",
});
if (write) console.log(`Formatting ratchet: wrote ${files.length} entries.`);
else if (!printDiagnosticResult("Formatting ratchet", comparison))
  process.exit(1);
