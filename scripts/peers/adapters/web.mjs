...
export const webAdapter = {
  id: "web",
  kind: "web",
  describe: () => `Chromium web host (same-origin WS gateway 127.0.0.1:${WEB_GATEWAY_PORT})`,
  ...
  async up({ log, build }) {
    ...
    const child = spawn(
      process.execPath,
      [HELPER, `--url=http://127.0.0.1:${WEB_GATEWAY_PORT}/?cross-device-control=1`, `--cdp=${WEB_CDP_PORT}`, `--ready=${READY_PATH}`],
      { cwd: repoRoot, detached: true, stdio: ["ignore", out, out] }
    );
    child.unref();
    const deadline = Date.now() + 90_000;
    while (Date.now() < deadline) {
      if (!processAlive(child.pid)) throw new Error(`web browser exited during startup (see ${logPath("web")})`);
      if (existsSync(READY_PATH)) {
        const ready = JSON.parse(readFileSync(READY_PATH, "utf8"));
        log(`web: Chromium ready (${ready.gateway})`);
        return { kind: "web", pid: child.pid, cdpPort: WEB_CDP_PORT, url: ready.url };
      }
      await sleep(250);
    }
    try { process.kill(child.pid, "SIGTERM"); } catch {}
    throw new Error(`web browser did not reach the gateway within 90s (see ${logPath("web")})`);
  },
