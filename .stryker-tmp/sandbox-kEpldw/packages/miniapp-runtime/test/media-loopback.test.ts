...
describe("two-host realtime media loopback", () => {
  it("routes timed PCM from an authorized sender to a receiver-owned speaker sink", async () => {
      ...
      streamEgressFactory: {
        async create({ admission }) {
          return {
            ...
            quality: () => ({ goodputBps: 1_000_000, rttMs: 20, jitterMs: 2, lossRatio: 0, mtu: 1_200, source: "observed" as const, samples: 8, confidence: "high" as const }),
            close: closeEgress
          };
        }
      },
    ...
    const accept = vi.fn(async (_appId, accepted, sink) => ({ handle: "in-1", offerId: accepted.id, sink }));
    ...
    const session = await sender.open(
      ...
      ["device:microphone:pcm"],
      { class: "microphone", tier: "pcm", purpose: "loopback", options: { voiceDuplex: true } }
    );
    ...
    expect(frame).toMatchObject({ version: 2, sampleKind: 2, captureAtUs: 1_000_000 });
    expect(accept).toHaveBeenCalledWith("receiver-app", offer, { kind: "speaker" });

    expect(await sender.requestShareOfferRevoke("sender-app", share.id)).toBe(true);
