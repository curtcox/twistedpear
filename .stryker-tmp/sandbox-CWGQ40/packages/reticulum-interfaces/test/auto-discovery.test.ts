...
import { BonjourDiscoveryProvider, serviceRecordToPeer, selectDiscoveryProviders, type BonjourBridge, type BonjourBridgeEvents } from "../src/index.js";
import type { MulticastNetworkInfo } from "../src/pipes.js";

class MockBonjourBridge implements BonjourBridge {
  interfaces: MulticastNetworkInfo[] = [{ name: "en0", linkLocalAddress: "fe80::1" }];
  private events: BonjourBridgeEvents = {};
  startedWith: string | null = null;
  advertised: ReadonlyArray<{ readonly id: string; readonly ifname: string; readonly host: string; readonly port: number }> = [];
  ...
  async advertise(record: { readonly id: string; readonly ifname: string; readonly host: string; readonly port: number }): Promise<void> {
    this.advertised = [...this.advertised, record];
  }

  emit(record: { readonly id: string; readonly ifname: string; readonly host: string; readonly port: number }): void {
...
describe("discovery provider policy", () => {
  it("prefers multicast when entitlement and bridge are available", () => {
    expect(selectDiscoveryProviders({
...
describe("BonjourDiscoveryProvider", () => {
  it("maps Bonjour records to discovery peers and advertises Reticulum UDP records", async () => {
    ...
    bridge.emit({ id: "remote", ifname: "en0", host: "fe80::5678", port: 42_671 });
