...
type TargetInput = { readonly sameInterface: boolean; readonly hopsMatch: boolean };
type TargetGateEvent = GateEvent<"test/target-gate", TargetInput>;

const targetGate = defineOptionGate<TargetGateEvent, "outbound" | "received", "ignore">({
...
type IndexAction = { readonly kind: "use-index"; readonly index: number } | { readonly kind: "miss" };

const indexGate = defineGate<IndexGateEvent, IndexAction>({
  event: "test/index-gate",
  actions: ["use-index", "miss"],
  decide: (event) => (event.index < 0 ? [{ kind: "miss" }] : [{ kind: "use-index", index: event.index }])
});
...
describe("stateless decision gates", () => {
  ...
  it("makes abstention explicit for option gates", () => {
    ...
    expect(gateConclusion<{ readonly kind: "outbound" | "received" | "ignore" }, "outbound" | "received">(
      "outbound",
      "received"
    )(abstained.actions)).toBeNull();
  });
  ...
  it("carries action payloads through readers", () => {
    ...
    const miss = step(initialGateState(), { kind: "test/index-gate", index: -1 });
    const indexOf = gatePayload<IndexAction, "use-index", "index">("use-index", "index");
  ...
  it("rejects conclusions outside the declared alphabet", () => {
    ...
    expect(() => decideGate(rogue, { kind: "test/rogue-gate" })).toThrow(UndeclaredGateActionError);
  });
  ...
  it("rejects malformed alphabets at construction", () => {
    ...
    expect(() =>
      defineGate<GateEvent<"test/dupe", Record<string, never>>, { readonly kind: "allow" }>({
        event: "test/dupe",
        actions: ["allow", "allow"],
        decide: () => [{ kind: "allow" }]
      })
  ...
  it("enumerates the gate coverage frame", () => {
    ...
    expect(enumerateGateCells(targetGate).map((cell) => cell.actionKind)).toEqual([
      "outbound",
      "received",
      "ignore"
    ]);
