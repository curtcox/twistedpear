// @ts-nocheck
import type { DolevYaoPower, TransportAdversaryAction } from "@twistedpear/effects";
import { compileAttackProposal, type AttackProposal, type CompiledAdversary } from "./adversary.js";
...
export async function authorAttackStrategies(model: StrategyModel, context: AttackAuthoringContext): Promise<AttackAuthoringResult> {
  ...
  for (const [index, candidate] of candidates.slice(0, context.maxProposals ?? 16).entries()) {
    try {
      accepted.push(compileAttackProposal(parseProposal(candidate), context.allowedPowers));
    } catch (error) {
...
export function authoringPrompt(context: AttackAuthoringContext): string {
  return JSON.stringify({
    ...
    constraints: {
      ...
      output: "JSON array only; each item has name and actions; action has power, source, destination, optional channel, optional payloadHex, optional delayMs"
    }
  });
}

function parseProposal(value: unknown): AttackProposal {
  if (!isRecord(value) || typeof value.name !== "string" || !Array.isArray(value.actions)) throw new Error("proposal has invalid shape");
  return { name: value.name, actions: value.actions.map((action): TransportAdversaryAction => {
    if (!isRecord(action) || typeof action.power !== "string" || typeof action.source !== "string" || typeof action.destination !== "string")
      throw new Error("proposal action has invalid shape");
    if (action.power === "inject") {
      if (typeof action.channel !== "string") throw new Error("inject action needs a channel");
      const payload = hexBytes(action.payloadHex);
      return { power: "inject", source: action.source, destination: action.destination,
        channel: action.channel, payload,
        ...(typeof action.delayMs === "number" && Number.isFinite(action.delayMs) ? { delayMs: action.delayMs } : {}) };
    }
    if (action.power === "delay") {
      if (typeof action.delayMs !== "number" || !Number.isFinite(action.delayMs)) throw new Error("delay action needs delayMs");
      return { power: "delay", source: action.source, destination: action.destination, delayMs: action.delayMs };
    }
    if (action.power === "drop" || action.power === "reorder" || action.power === "duplicate")
      return { power: action.power, source: action.source, destination: action.destination };
    throw new Error(`unknown adversary power: ${action.power}`);
  }) };
}

function hexBytes(value: unknown): Uint8Array {
  if (typeof value !== "string" || value.length % 2 !== 0 || /[^0-9a-f]/i.test(value)) throw new Error("payloadHex must be even-length hexadecimal");
