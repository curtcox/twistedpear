...
export function renderTrustListImpl(__scope, entries) {
    ...
    __scope.trustList.replaceChildren(...entries.map((entry) => {
        ...
        remove.addEventListener("click", () => {
            __scope.host?.send({ type: "trust-remove", publisherPublicKey: entry.publisherPublicKey });
        });
...
export function renderOwnIdentityImpl(__scope, identity256t) {
    ...
    if (!identity256t) {
        __scope.trustIdentityView.textContent = "No host identity yet — start the node first.";
        return;
    }
...
export function renderLimitsImpl(__scope, limits) {
    ...
    if (__scope.limitKv) {
        __scope.limitKv.value = limits.kvQuotaBytes === null ? "" : String(limits.kvQuotaBytes);
    }
    if (__scope.limitMemory) {
        __scope.limitMemory.value = limits.memoryBytes === null ? "" : String(limits.memoryBytes);
    }
...
export function renderStatusImpl(__scope, status) {
    ...
    const rows = [
        ...
        ["Propagation store", __scope.formatBytes(status.propagationStoreBytes ?? 0)],
    ...
    __scope.statusGrid.replaceChildren(...rows.flatMap(([label, value]) => {
...
export function renderCatalogImpl(__scope) {
    ...
    __scope.catalogList.replaceChildren(...__scope.catalogEntries.map((entry) => {
        const item = document.createElement("li");
        item.className = "item-row";
...
export function renderInstalledImpl(__scope) {
    ...
    __scope.installedList.replaceChildren(...__scope.installedPackages.map((pkg) => {
        const item = document.createElement("li");
        item.className = "item-row";
...
export function renderGrantsImpl(__scope, appId, capabilities) {
    ...
    for (const capability of capabilities) {
        ...
        input.addEventListener("change", () => {
            const grantedCapabilities = [...__scope.grantsPanel.querySelectorAll(".grant-row input")]
