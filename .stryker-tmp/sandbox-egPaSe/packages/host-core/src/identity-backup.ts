...
export const IDENTITY_BACKUP_ERROR = "Wrong passphrase or damaged identity backup";
export const IDENTITY_PASSPHRASE_MIN_CODE_POINTS = 12;
export const IDENTITY_SCRYPT_PARAMS = Object.freeze({ N: 32_768, r: 8, p: 3, dkLen: 32 });
...
function assertSized(bytes: Uint8Array, length: number, label: string): Uint8Array {
...
export function validateNewIdentityPassphrase(passphrase: string, confirmation: string): void {
  if ([...passphrase.normalize("NFKC")].length < IDENTITY_PASSPHRASE_MIN_CODE_POINTS) {
    throw new Error(`Identity passphrase must contain at least ${IDENTITY_PASSPHRASE_MIN_CODE_POINTS} characters`);
  }
...
export function encryptIdentityBackup(
  ...
  const privateKey = assertSized(identity.getPrivateKey(), PRIVATE_KEY_BYTES, "Identity private key");
  const salt = assertSized(entropy.salt ?? provider.randomBytes(16), 16, "scrypt salt");
  const nonce = assertSized(entropy.nonce ?? provider.randomBytes(12), 12, "AES-GCM nonce");
...
export function decryptIdentityBackup(
  ...
  try {
    if (container.length !== CONTAINER_BYTES || !equalBytes(container.subarray(0, 8), MAGIC)) throw new Error();
    const view = new DataView(container.buffer, container.byteOffset, container.byteLength);
    if (container[8] !== 0 || container[9] !== 15 || view.getUint16(10, false) !== 8 || view.getUint16(12, false) !== 3) {
      throw new Error();
    }
    ...
    privateKey = gcm(key, nonce, header).decrypt(container.subarray(CIPHERTEXT_OFFSET));
    const identity = RnsIdentity.fromBytes(provider, privateKey.slice());
    if (identity === null || !equalBytes(identity.hash, container.subarray(HASH_OFFSET, HEADER_BYTES))) throw new Error();
    return identity;
  } catch {
...
export function identityToRecoveryWords(identity: Identity): IdentityRecoveryWords {
  const privateKey = assertSized(identity.getPrivateKey(), PRIVATE_KEY_BYTES, "Identity private key");
  try {
    return {
...
export function identityFromRecoveryWords(
  ...
  if (!validateMnemonic(recovery.first.normalize("NFKD"), wordlist) ||
...
export function isEncryptedIdentityBackup(bytes: Uint8Array): boolean {
  return bytes.length >= MAGIC.length && equalBytes(bytes.subarray(0, MAGIC.length), MAGIC);
}
