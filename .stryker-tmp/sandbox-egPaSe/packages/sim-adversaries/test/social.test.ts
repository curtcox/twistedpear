// @ts-nocheck
import { propagateHarassment, reputationUnderCollusion, spamEconomics } from "../src/social.js";
import { describe, expect, it } from "vitest";

describe("social and economic adversaries", () => {
  it("makes LoRa spam materially more expensive than LAN spam", () => {
    const options = { payloadBytes: 200, messages: 100, payoffPerDelivery: 0.001 };
  ...
  it("measures how severing arrests harassment propagation", () => {
    ...
    expect(propagateHarassment({ graph, origin: "a", blocked: new Set(), severAtHop: 1 })).toEqual({
      reached: ["a"],
      arrestedAtHop: 1
    });
  });
  ...
  it("discounts a coordinated reputation ring", () => {
    ...
    expect(reputationUnderCollusion(votes, new Set(["c1", "c2"])).target).toBeCloseTo(-0.8);
  });
});
