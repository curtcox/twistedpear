...
describe("protocol lxmf peer error", () => {
  ...
  it("decodes via use-fields actions", () => {
    const ok = stepDecodeLxmfPeerErrorWithActions(initialDecodeLxmfPeerErrorState(), {
    ...
    expect(lxmfPeerErrorFromActions(ok.actions)).toBe(LXMF_PEER_ERROR_NO_IDENTITY);

    const rejected = stepDecodeLxmfPeerErrorWithActions(initialDecodeLxmfPeerErrorState(), {
      kind: "lxmf/peer-error-decode-gate",
