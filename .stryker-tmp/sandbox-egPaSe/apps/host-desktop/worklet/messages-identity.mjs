...
export function createIdentityMessageHandlers(deps) {
  ...
  const persistModerationState = (...args) => deps.persistModerationState(...args);
  ...
  const handleModerationUpdate = async (message) => {
    try {
      if (message.type === "moderation-export-reports") {
        send({ type: "moderation-report-export", json: `${JSON.stringify({ format: "twistedpear-local-reports-v1", exportedAt: Date.now(), reports: state.moderationState.reports }, null, 2)}\n` });
        return;
      }
      const sourceHash = normalizedSourceHash(message.sourceHash);
      if (message.type === "moderation-block") {
        const existing = state.moderationState.blocked.find((entry) => entry.sourceHash === sourceHash);
        state.moderationState.blocked = [...state.moderationState.blocked.filter((entry) => entry.sourceHash !== sourceHash), { sourceHash, label: message.label?.trim() || null, createdAt: existing?.createdAt ?? Date.now() }];
        state.moderationState.muted = state.moderationState.muted.filter((entry) => entry.sourceHash !== sourceHash);
      } else if (message.type === "moderation-unblock") {
        state.moderationState.blocked = state.moderationState.blocked.filter((entry) => entry.sourceHash !== sourceHash);
      } else if (message.type === "moderation-mute") {
        if (!state.moderationState.blocked.some((entry) => entry.sourceHash === sourceHash)) {
          const existing = state.moderationState.muted.find((entry) => entry.sourceHash === sourceHash);
          state.moderationState.muted = [...state.moderationState.muted.filter((entry) => entry.sourceHash !== sourceHash), { sourceHash, label: message.label?.trim() || null, createdAt: existing?.createdAt ?? Date.now() }];
        }
      } else if (message.type === "moderation-unmute") {
        state.moderationState.muted = state.moderationState.muted.filter((entry) => entry.sourceHash !== sourceHash);
      } else if (message.type === "moderation-report") {
        state.moderationState.reports = [...state.moderationState.reports, {
