...
class MulticastModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("start") {
      ...
      active.setListener(object : MulticastBridge.Listener {
        override fun onPacket(ifname: String, data: ByteArray, sourceAddress: String, port: Int) {
          sendEvent(
            "onPacket",
            mapOf(
              "ifname" to ifname,
        ...
        override fun onNetworkChange(interfaces: List<MulticastNetworkInfo>) {
          sendEvent(
            "onNetworkChange",
            mapOf(
              "interfaces" to interfaces.map { iface ->
                mapOf(
                  "name" to iface.name,
      ...
      sendEvent(
        "onNetworkChange",
        mapOf(
          "interfaces" to active.getInterfaces().map { iface ->
            mapOf(
              "name" to iface.name,
              "linkLocalAddress" to iface.linkLocalAddress
            )
