...
import { forgetPeer, logPath, peerEntry, readState, recordPeer } from "../../scripts/peers/state.mjs";
...
const allowSkip = args.includes("--allow-skip") || process.env.CROSS_DEVICE_ALLOW_SKIP === "1";
...
const FULL = VARIANTS.flatMap((from) =>
  VARIANTS.filter((to) => to !== from).map((to) => ({ id: `M-${from}-${to}`, chain: [from, to] }))
);
...
function requestedScenarios() {
  ...
  const ids = new Set(selected.slice("--scenarios=".length).split(",").filter(Boolean));
  const result = candidates.filter((scenario) => ids.has(scenario.id));
  assert(result.length === ids.size, `unknown scenario(s): ${[...ids].filter((id) => !result.some((item) => item.id === id)).join(", ")}`);
  return result;
}
...
async function bringUp(ids) {
  ...
  try {
    for (const id of ordered) {
      ...
      forgetPeer(id);
      try {
        recordPeer(id, await adapter.up({ log: (line) => step(line), build: buildPeers }));
        started.push(id);
      } catch (error) {
        if (GUI_PEER_IDS.includes(id) && allowSkip) {
          skipped.push(id);
          step(`${id} skipped: ${error instanceof Error ? error.message : String(error)}`);
        } else throw error;
      }
    }
...
async function connectBrowserDriver(id) {
  ...
  let lastError = null;
  while (Date.now() < deadline) {
    try { return await BrowserUiDriver.connect({ id, cdpPort: entry.cdpPort }); }
...
await runMain(async () => {
  ...
  section(`Cross-device develop-and-run: ${scenarios.map((scenario) => scenario.id).join(", ")}`);
  let owned = [];
  let control = null;
  const ui = new Map();
  const skipped = new Set();
  const proof = { generatedAt: new Date().toISOString(), matrix, scenarios: [] };
  ...
  try {
    ...
    for (const id of activeVariants.filter((variant) => !skipped.has(variant) && variant !== "web")) {
      try { await control.waitForAgent(id, 90_000); }
      catch (error) {
    ...
    if (activeVariants.includes("desktop") && !skipped.has("desktop")) ui.set("desktop", await connectBrowserDriver("desktop"));
    if (activeVariants.includes("web") && !skipped.has("web")) ui.set("web", await connectBrowserDriver("web"));
    if (activeVariants.includes("ios") && !skipped.has("ios")) {
      ui.set("ios", new NativeUiDriver({ id: "ios", device: peerEntry("ios").udid, repoRoot, artifactDir: artifactRoot }));
    }
    if (activeVariants.includes("android") && !skipped.has("android")) {
      const serial = spawnSync("adb", ["get-serialno"], { encoding: "utf8" }).stdout.trim();
      ui.set("android", new NativeUiDriver({ id: "android", device: serial, repoRoot, artifactDir: artifactRoot }));
    }

    const commandFor = (id) =>
      id === "web"
        ? (cmd, payload = {}) => ui.get("web").command(cmd, payload)
        : (cmd, payload = {}, timeoutMs = 120_000) => control.command(id, cmd, payload, timeoutMs);

    const approve = async (id, action, kind) => {
      const pending = Promise.resolve().then(action);
      const approval = kind === "install"
    ...
    for (const scenario of scenarios) {
      ...
      const record = {
        ...
        ...(scenario.transportVia ? { transportVia: scenario.transportVia } : {}),
      ...
      try {
        ...
        const manifest = JSON.parse(readFileSync(join(repoRoot, "apps", "devstudio", "app.manifest.json"), "utf8"));
        const bundle = readFileSync(join(repoRoot, "apps", "devstudio", "bundle.js"), "utf8");
        await dev("devstudio.load", { manifest, bundle });
        await dev("project.create");
        const created = await waitFor(dev, (value) => stateText(value.state).includes("Created project"), "project creation timed out");
        const projectNode = created.state.widgetTree.root.children.find((node) => node.id?.startsWith("proj-"));
        const project = projectNode.id.slice("proj-".length);
        const appId = `cross-${scenario.id.toLowerCase().replace(/[^a-z0-9-]/g, "-")}`;
        await dev("project.write", { path: `${project}/app.json`, content: JSON.stringify({ name: appId, version: "0.1.0", entry: "bundle.js", capabilities: ["storage:kv", "lxmf:send"] }, null, 2) });
        await dev("project.write", { path: `${project}/bundle.js`, content: FIXTURE_BUNDLE });

        await approve(developer, () => dev("preview"), "Preview an app");
        const preview = await waitFor(dev, (value) => stateText(value.preview).includes("Cross-device hello"), "preview did not render");
        ...
        const packaged = await waitFor(dev, (value) => /[A-Za-z0-9_-]{94}/.test(stateText(value.state)), "package 256t did not appear");
        const qr = packaged.state.widgetTree.root.children.find((node) => node.id === "package-qr");
        const t256 = qr.props.value;
        assert(/^[A-Za-z0-9_-]{94}$/.test(t256), `${scenario.id}: invalid 256t`);
        await approve(developer, () => dev("publish"), "Publish an app");
        await waitFor(dev, (value) => stateText(value.state).includes("Published v"), "publish did not complete");
        ...
        if (!negativeTargets.has(runner)) {
          ...
          assert(corrupted.length > 0, `${scenario.id}: published archive is empty`);
          ...
          assert(negative.refused === true && negative.stage === "sha512", `${scenario.id}: corrupted archive was not refused at SHA-512`);
          assert(negative.codeExecuted === false, `${scenario.id}: corrupted archive reached code execution`);
          assert(JSON.stringify(beforeNegative.state) === JSON.stringify(afterNegative.state), `${scenario.id}: corrupted archive changed runtime state`);
          record.negative = { target: runner, ...negative };
          negativeTargets.add(runner);
        }

        const publisher = await dev("trust.show");
        const sourcePublicKey = decodePublisherIdentity256t(publisher.identity256t);
        await approve(runner, () => target("trust.import", { identity256t: publisher.identity256t, label: `${developer} ${scenario.id}` }), "Trust a new publisher");
        const installed = await approve(runner, () => target("install", { t256 }), "install");
        assert(installed.trusted === true, `${scenario.id}: target did not mark publisher trusted`);
        assert(installed.source !== "local-cas" && installed.fetchPath !== "inline", `${scenario.id}: target did not fetch the archive from the mesh`);
        assert(installed.publisherPublicKey === sourcePublicKey, `${scenario.id}: installed publisher is not the source peer identity`);
        assert(installed.servingPublicKey === sourcePublicKey, `${scenario.id}: signed locator does not attribute the fetch to the source peer`);
        ...
        deepStrictEqual(runTree, previewTree, `${scenario.id}: preview/run widget trees differ`);
        await target("ui.event", { nodeId: "send", event: "cross.send" });
        await waitFor(target, (value) => stateText(value.state).includes("DENIED:"), "capability denial was not visible");
        ...
        writeFileSync(join(scenarioDir, "hop.json"), `${JSON.stringify(record.hops[0], null, 2)}\n`);
        writeFileSync(join(scenarioDir, "widget-tree.json"), `${JSON.stringify(runTree, null, 2)}\n`);
        step(`${scenario.id}: ${developer} → ${runner} passed`);
      } catch (error) {
        record.status = "failed";
        record.error = error instanceof Error ? error.message : String(error);
        for (const id of scenario.chain) {
          await ui.get(id)?.screenshot?.(join(scenarioDir, `${id}-failure.png`)).catch(() => {});
          try {
            const state = await commandFor(id)("state");
            writeFileSync(join(scenarioDir, `${id}-state.json`), `${JSON.stringify(state, null, 2)}\n`);
          } catch {}
          try {
    ...
    if (!allowSkip && scenarioSelection === undefined) {
      assert(coverage.empty.length === 0, `coverage cells empty: ${coverage.empty.join(", ")}`);
    }
    step(`artifacts: ${artifactRoot}`);
  } finally {
    coverage = coverageFromProof(proof);
    writeFileSync(join(artifactRoot, "proof.json"), `${JSON.stringify(proof, null, 2)}\n`);
    writeFileSync(join(artifactRoot, "coverage.json"), `${JSON.stringify(coverage, null, 2)}\n`);
