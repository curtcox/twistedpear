...
function fixture() {
  ...
  for (const name of ["status.mjs", "start-soaks.mjs", "watch-soaks.mjs", "h20.mjs", "record.mjs"]) writeFileSync(join(root, "scripts/release", name), "");
  mkdirSync(join(root, ".github/workflows"), { recursive: true });
  writeFileSync(join(root, ".github/workflows/ci.yml"), "test:release-harness\ntest:hostile-apps\ntest:sim-fixed-replay\n");
  writeFileSync(join(root, "STATUS-COMPLETE.md"), "# Complete\n");
  writeFileSync(join(root, "STATUS-HARDWARE.md"), "| H1 | phone | 2 | test |\n");
  writeFileSync(join(root, "STATUS-SOFTWARE.md"), "| Link keepalive soak | short | long |\n");
  return root;
}
...
describe("release driver", () => {
  ...
  test("a failed soak preempts the serial stage action", () => {
    ...
    writeFileSync(join(run, "status.json"), JSON.stringify({ results: [{ status: "failed", log: "transport.log" }] }));
...
describe("evidence recorder", () => {
  test("creates a self-contained attestation log for a started manual wait", () => {
    const root = fixture();
    const path = record({ root, id: "account:H12", status: "started", note: "enrollment submitted" });
    expect(readFileSync(path, "utf8")).toContain('"status": "started"');
    expect(readFileSync(join(root, "release/evidence-logs/account-h12-started.log"), "utf8")).toContain("enrollment submitted");
  });

  test("records, archives, and strikes a passed hardware row", () => {
    const root = fixture();
    writeFileSync(join(root, "h1.log"), "device session passed\n");
    record({ root, id: "hardware:H1", status: "passed", log: "h1.log", at: "2026-07-19T00:00:00.000Z" });
    expect(readFileSync(join(root, "STATUS-HARDWARE.md"), "utf8")).toContain("~~H1~~");
    expect(readFileSync(join(root, "STATUS-COMPLETE.md"), "utf8")).toContain("hardware:H1");
  });

  test("refuses a failed mac-validation log marked passed", () => {
    const root = fixture();
    writeFileSync(join(root, "bad.log"), "[mac-validation] exit: 1\n");
    expect(() => record({ root, id: "baseline:S1", status: "passed", log: "bad.log" })).toThrow(/refusing/);
  });
});
...
describe("H20 verifier", () => {
  const status = (uptimeMs, pathTableCount, propagationStoreBytes) => ({
    running: true, propagationEnabled: true, uptimeMs, pathTableCount,
    propagationStoreBytes, propagationMessageCount: 2, propagationEvictions: 0
  });

  test("accepts continuous samples spanning the committed duration", () => {
    const summary = verifySamples([
      { at: "2026-07-01T00:00:00.000Z", rssKiB: 1000, status: status(100, 1, 20) },
      { at: "2026-07-01T01:00:00.000Z", rssKiB: 1020, status: status(3_600_100, 2, 30) }
    ], { durationMs: 3_600_000, intervalMs: 3_600_000 });
    expect(summary.rssKiB.growth).toBe(20);
  });

  test("rejects a restart or monitoring gap", () => {
    expect(() => verifySamples([
      { at: "2026-07-01T00:00:00.000Z", rssKiB: 1000, status: status(5000, 1, 20) },
      { at: "2026-07-01T01:00:00.000Z", rssKiB: 1020, status: status(100, 2, 30) }
    ], { durationMs: 3_600_000, intervalMs: 3_600_000 })).toThrow(/restarted/);
  });
});
...
describe("soak watcher", () => {
  test("classifies failures and writes a minimal reproducer", () => {
    ...
    mkdirSync(logs);
    writeFileSync(join(logs, "stage-8-01-link.log"), "[mac-validation] command: npm run test:link-soak\nAssertionError: expected true\n[mac-validation] exit: 1\n");
    const results = scan(logs);
    expect(results[0]).toMatchObject({ status: "failed", category: "assertion" });
    expect(readFileSync(results[0].reproducer, "utf8")).toContain("npm run test:link-soak");
  });
