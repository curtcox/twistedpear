...
// @ts-nocheck
/**
 * Hostile mini-app smoke subset for the iOS worklet host path (Phase 5 M1).
 */
...
export async function runIosHostileSmoke() {
  ...
  try {
    ...
    throw new Error("dev side-load succeeded while developer mode was disabled");
  } catch (error) {
  ...
  for (let attempt = 0; attempt < 40; attempt += 1) {
    runtime = outbound.findLast((message) => message.type === "miniapp-runtime");
    if (runtime?.runtime?.widgetTree !== null && runtime?.runtime?.widgetTree !== undefined) {
      break;
    }
  ...
  if (runtime?.runtime?.widgetTree?.root?.children?.[0]?.props?.value !== "Hello") {
  ...
  console.log("[ios-sim/hostile-smoke] dev gate, broker flood, UI rejection, and watchdog kill passed");
}

if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  runIosHostileSmoke().catch((error) => {
    console.error(error);
    process.exit(1);
  });
}
