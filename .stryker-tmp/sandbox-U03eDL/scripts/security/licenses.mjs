...
for (const [location, pkg] of Object.entries(lock.packages ?? {})) {
  ...
  if (!licenseAllowed(license)) findings.push(`${name}@${pkg.version}:${license}`);
}
inventory.sort((a, b) => a.name.localeCompare(b.name) || a.version.localeCompare(b.version));
writeJson(path.join(ROOT, "licenses.json"), { version: 1, packages: inventory.length, findings, inventory });
const comparison = compareDiagnosticSet({
  ...
  description: "Dependency license expressions outside the approved SPDX list when the gate was established; entries may only disappear.",
  envName: "LICENSE_RATCHET_BASE_REF"
});
if (process.argv.includes("--write")) console.log(`License gate: wrote ${findings.length} exceptions.`);
