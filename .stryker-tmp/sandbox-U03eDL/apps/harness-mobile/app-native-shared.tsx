...
export const peerAudioHex = (bytes: Uint8Array) => [...bytes].map((byte) => byte.toString(16).padStart(2, "0")).join("");
export const peerAudioUnhex = (text: string) => Uint8Array.from(text.match(/../g) ?? [], (pair) => Number.parseInt(pair, 16));
...
export async function playNativeOpusOrPcm(dataHex: string, encoding: string): Promise<void> {
...
export function floatToPcm16(pcm: Float32Array): Uint8Array { const bytes = new Uint8Array(pcm.length * 2); const view = new DataView(bytes.buffer); for (let index = 0; index < pcm.length; index += 1) view.setInt16(index * 2, Math.round(Math.max(-1, Math.min(1, pcm[index] ?? 0)) * 32767), true); return bytes; }
export function pcm16ToFloat(bytes: Uint8Array): Float32Array { const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength); const pcm = new Float32Array(Math.floor(bytes.length / 2)); for (let index = 0; index < pcm.length; index += 1) pcm[index] = view.getInt16(index * 2, true) / 32768; return pcm; }
export async function playNativePeerFrames(framesHex: ReadonlyArray<string>, sampleRate = 44_100): Promise<void> { for (const frame of framesHex) await playNativePeerPcm(floatToPcm16(encodePeerAudioFsk(peerAudioUnhex(frame), { sampleRate })), sampleRate); }
export async function recordNativePeerFrames(sampleRate = 44_100): Promise<ReadonlyArray<string>> { const pcm = pcm16ToFloat(await recordNativePeerPcm(15_000, sampleRate)); const frames = decodePeerAudioFskStream(pcm, { sampleRate }); if (frames.length === 0) throw new Error("No valid peer audio frames were detected"); return frames.map(peerAudioHex); }
export async function playInboundNativeMedia(dataHex: string, encoding: string): Promise<void> {
  const frame = peerAudioUnhex(dataHex);
