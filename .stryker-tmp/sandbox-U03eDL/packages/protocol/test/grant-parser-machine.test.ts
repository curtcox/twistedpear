...
describe("grant parser transition table", () => {
  it("commits a vector for every parser table cell", () => {
    expect(vectors.cells).toHaveLength(enumerateCells(grantParserMachine).length);
    expect(vectors.cells).toHaveLength(grantParserMachine.states.length * grantParserMachine.events.length);
    expect(vectors.cells.filter((cell) => cell.legal)).toHaveLength(grantParserMachine.table.length);
  });

  it("accepts its canonical vector and rejects every curated near miss", () => {
    const canonical = utf8Encode(vectors.canonical);
    expect([...encodeGrantRecord(decodeGrantRecord(canonical))]).toEqual([...canonical]);
