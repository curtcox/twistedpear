...
const vector = (
  JSON.parse(
    readFileSync(
      new URL("../../../specs/spec-freenet/vectors/locator-state.json", import.meta.url),
      "utf8"
    )
  ) as {
    contractArtifact: {
...
describe("Freenet locator contract state", () => {
  ...
  it("uses the 256t id as contract parameters", () => {
    expect(new TextDecoder().decode(locatorContractParameters(t256))).toBe(t256);
  ...
  it("pins the generated locator contract artifact", () => {
    const wasm = Uint8Array.from(
      readFileSync(
        new URL(
          "../contract/locator/locator-contract.wasm",
          import.meta.url
        )
      )
    );
