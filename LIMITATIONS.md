...
[docs/mobile-lifecycle.md](docs/mobile-lifecycle.md) keeps an audited ledger separating the
...
- **Background execution:** iOS will suspend the app; there is no equivalent of Android's
  ...
  is shaped by it — see [docs/mobile-lifecycle.md](docs/mobile-lifecycle.md) §"Decisions
...
- **Android is less restricted than the platform currently assumes.** That foreground
  ...
  [docs/mobile-lifecycle.md](docs/mobile-lifecycle.md) record what it costs and what
...
- **Mini-app limits inherited from the mobile lifecycle are tracked, not accepted.** Four of
  the seven entries in the [mobile lifecycle ledger](docs/mobile-lifecycle.md) — concurrency,
...
- One mini-app at a time in v1, and none while the host is backgrounded. The platform is
  ...
  [STATUS-SOFTWARE.md](STATUS-SOFTWARE.md). Dev side-loading is localhost/adb-only, off by
...
- **Realtime peer media is core-complete; shipping-host evidence is partial.** The broker/SDK,
  ...
  open — see [docs/realtime-media.md](docs/realtime-media.md).
