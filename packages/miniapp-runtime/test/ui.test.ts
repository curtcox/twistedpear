...
describe("widget trees", () => {
  ...
  it("rejects unsupported components, props, styles, and depth", () => {
    expect(() =>
      validateWidgetTree({ root: { id: "x", type: "webview" as "view" } }),
    ).toThrow(/Unsupported widget type/);
    expect(() =>
      validateWidgetTree({
        root: { id: "x", type: "text", props: { html: "<b>x</b>" } },
      }),
    ).toThrow(/Unsupported text prop/);
    expect(() =>
      validateWidgetTree({
        root: { id: "x", type: "view", style: { position: "absolute" } },
      }),
      ...
      validateWidgetTree(
        {
          root: {
            ...
            type: "view",
            children: [
              { id: "b", type: "view", children: [{ id: "c", type: "view" }] },
            ],
          },
        },
        { maxDepth: 2 },
    ...
    expect(diffWidgetTrees(previous, next)).toEqual([
      {
        op: "replace",
        id: "title",
        node: { id: "title", type: "text", props: { value: "New" } },
      },
      { op: "remove", id: "gone" },
    ]);
  });
