...
describe("peer broker service", () => {
  it("derives app scope and rejects cross-app names", async () => {
    ...
    const manager = new PeerSessionManager(registry, {
      async request(adapter, request) {
        expect(request.service).toBe("chat");
        return {
          authenticated: true,
          confirmed: true,
          fingerprint: "fp",
    ...
    await expect(
      service.request("chat", "run", { purpose: "Exchange a message" }),
    ).resolves.toHaveProperty("id");
    await expect(
      service.request("chat", "run", { service: "other", purpose: "no" }),
    ).rejects.toThrow(/Cross-app/);
  });
  it("enforces peer:connect at the shipping broker chokepoint", async () => {
    ...
    const scoped = await host.dispatchRaw(
      {
        id: "scoped",
        namespace: "peers",
        method: "request",
        payload: { service: "other", purpose: "Pair" },
      },
      manifest,
      ["peer:connect"],
    );
