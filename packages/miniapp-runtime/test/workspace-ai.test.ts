...
describe("workspace service", () => {
  it("round-trips files and lists per app", async () => {
    ...
    expect(await service.read("devstudio", "hello/bundle.js")).toBe(
      "export {};\n",
    );
    const files = await service.list("devstudio", "hello/");
    expect(files.map((file) => file.path)).toEqual([
      "hello/app.manifest.json",
      "hello/bundle.js",
    ]);

    await service.delete("devstudio", "hello/bundle.js");
    await expect(
      service.read("devstudio", "hello/bundle.js"),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    expect(await service.read("other-app", "hello/bundle.js")).toBe(
      "// other\n",
    );
  });
  ...
  it("enforces file, count, and total quotas", async () => {
    ...
    await expect(
      service.write("app", "big.js", "x".repeat(17)),
    ...
    await expect(
      service.write("app", "b.js", "x".repeat(13)),
    ).rejects.toMatchObject({
      code: "WORKSPACE_FULL",
    });

    await service.write("app", "b.js", "x".repeat(12));
    await expect(service.write("app", "c.js", "x")).rejects.toMatchObject({
      code: "WORKSPACE_FULL",
    });
  });
  ...
  it("applies bounded non-overlapping text patches and rejects stale bases", async () => {
    ...
    await expect(
      service.patch("app", "bundle.js", 11, [{ start: 0, end: 1, text: "x" }]),
    ).rejects.toMatchObject({ code: "PATCH_CONFLICT" });
    await expect(
      service.patch("app", "bundle.js", 12, [
...
describe("ai service", () => {
  ...
  it("rejects malformed requests", async () => {
    ...
    await expect(service.chat("app", { messages: [] })).rejects.toMatchObject({
      code: "AI_BAD_REQUEST",
    });
    await expect(
      service.chat("app", {
        messages: [{ role: "root" as never, content: "hi" }],
      }),
  ...
  it("allows only one in-flight request per app", async () => {
    ...
    await expect(service.chat("app", request)).rejects.toMatchObject({
      code: "AI_BUSY",
    });
    release?.();
    await first;
  ...
  it("adapts a non-streaming backend and releases the slot when cancelled", async () => {
    ...
    expect(await fallback.next()).toEqual({
      done: false,
      value: { type: "delta", delta: "whole" },
    });
...
describe("ai service (continued)", () => {
  it("embeds bounded inputs and ranks cosine matches", async () => {
    ...
    await expect(
      service.search("app", {
        ...
        documents: [
          { id: "other", text: "apple" },
          { id: "match", text: "pear guide" },
        ],
        limit: 1,
      }),
    ).resolves.toMatchObject({ matches: [{ id: "match", score: 1 }] });
    await expect(service.embed("app", { inputs: [] })).rejects.toMatchObject({
      code: "AI_BAD_REQUEST",
    });
  });
  ...
  it("openrouter backend enforces the model allowlist and translates the wire shape", async () => {
    ...
    await expect(
      backend.chat("app", { ...request, model: "other/model" }),
    ).rejects.toMatchObject({
      code: "AI_BAD_REQUEST",
    });
  });
  ...
  it("parses OpenRouter-compatible SSE streams", async () => {
    ...
    const chunks = [];
    for await (const chunk of backend.stream!("app", request))
      chunks.push(chunk);
...
describe("new widgets", () => {
          ...
          children: [
            {
              id: "cam",
              type: "camera-preview",
              props: { session: "dev-1", aspectRatio: "4:3" },
            },
            { id: "meter", type: "audio-meter", props: { session: "dev-2" } },
            {
              id: "map",
              type: "map-preview",
              props: { session: "dev-3", zoom: 12 },
            },
          ],
        },
      }),
  ...
  it("rejects preview surfaces without a session handle", () => {
    expect(() =>
      validateWidgetTree({
        root: { id: "cam", type: "camera-preview", props: {} },
      }),
    ).toThrow(/session/);
  });
  ...
  it("rejects invalid code-editor and qr-code props", () => {
    expect(() =>
      validateWidgetTree({
        root: { id: "e", type: "code-editor", props: { documentId: "" } },
      }),
    ).toThrow(/documentId/);
    expect(() =>
      validateWidgetTree({
        root: {
          id: "e",
          type: "code-editor",
          props: { documentId: "a.js", language: "cobol" },
        },
      }),
    ).toThrow(/language/);
    expect(() =>
      validateWidgetTree({
        root: { id: "q", type: "qr-code", props: { value: "x".repeat(513) } },
      }),
    ).toThrow(/qr-code/);
    expect(() =>
      validateWidgetTree({
        root: {
          id: "e",
          type: "code-editor",
          props: { documentId: "a.js", content: "x" },
        },
      }),
    ).toThrow(/prop/);
  });
