...
function backend(
  policy: "allowed" | "confirm" | "denied" = "allowed",
  ...
  return {
    ...
    async probe(_appId, _peer, request) {
      ...
      expect(request.budgetBytes).toBeLessThanOrEqual(
        DEFAULT_LINK_PROBE_BUDGET_BYTES,
      );
      return quality;
    },
...
describe("link capabilities and service", () => {
  ...
  it("rate limits probes per app and opaque peer", async () => {
    ...
    await expect(service.probe("app", { id: "peer-1" })).resolves.toEqual(
      quality,
    );
    await expect(service.probe("app", { id: "peer-1" })).rejects.toMatchObject<
      Partial<LinkServiceError>
    >({
      code: "LINK_PROBE_RATE_LIMITED",
    });
    now += 60_000;
    await expect(
      service.probe("app", { id: "peer-1" }, { budgetBytes: 512 }),
  ...
  it("rejects oversized budgets before calling the backend", async () => {
    ...
    await expect(
      service.probe(
        "app",
        { id: "peer" },
        { budgetBytes: DEFAULT_LINK_PROBE_BUDGET_BYTES + 1 },
      ),
    ).rejects.toMatchObject<Partial<LinkServiceError>>({
      code: "LINK_BAD_REQUEST",
    });
  });
  ...
  it("exchanges coarse readiness and a bounded probe over paired host routes", async () => {
      ...
      list: () => [
        {
          handle: { id: remote },
          displayLabel: remote,
          dataPlane: "webrtc" as const,
          connectedAt: 900,
        },
      ],
      route: (_appId: string, handle: { id: string }) =>
        handle.id === remote
          ? {
              transport: {
    ...
    const readiness = () => ({
      ...
      accepts: [
        {
          classId: "microphone" as const,
          maxRung: "16k-opus",
          encodings: ["16k-opus"],
        },
      ],
      offers: [],
      downlinkBucket: "audio" as const,
      constrained: ["foreground-only" as const],
      consentPosture: "ask" as const,
      expiresAt: 10_000,
    });
    const a = new PeerRouteLinkObservatory(
      directory("b", bListeners, aListeners),
      { now: () => now, localReadiness: readiness },
    );
    const b = new PeerRouteLinkObservatory(
      directory("a", aListeners, bListeners),
      { now: () => now, localReadiness: readiness },
    );
    await b.peers("line-check");
    await a.peers("line-check");
    expect((await b.peers("line-check"))[0]).toMatchObject({
      readiness: { downlinkBucket: "audio" },
      quality: { source: "observed", goodputBps: 900_000, rttMs: 12, mtu: 900 },
    });
    await expect(
      a.probe(
        "line-check",
  ...
  it("aborts a probe when the authenticated route queue grows", async () => {
    ...
    const observatory = new PeerRouteLinkObservatory(
      {
        ...
        route: () => ({
          transport: {
            ...
            subscribe(next: (payload: Uint8Array) => void) {
              listener = next;
              return () => {
                listener = undefined;
              };
    ...
    ).rejects.toMatchObject({ code: "LINK_PROBE_DENIED" });
  });
});
