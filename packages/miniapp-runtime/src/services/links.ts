...
export class PeerRouteLinkObservatory implements LinkObservatoryBackend {
  ...
  probe(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pendingProbes.set(key, {
        ...
        mtu: qualityForRoute(directoryEntry.dataPlane, transport).mtu,
        initialQueueBytes,
        resolve,
        reject,
        timer,
  ...
  private async receiveRouteMessage(
    ...
    if (envelope.type === 2) {
      ...
      const reservation =
        this.options.controlReservations?.reserveControl(
          Math.max(1, Math.ceil(payload.byteLength / 5)),
        ) ?? undefined;
      if (
        this.options.controlReservations !== undefined &&
        reservation === undefined
        ...
        return;
      try {
        await reservation?.consume(payload.byteLength);
        if ((transport.quality?.().queueDepthBytes ?? 0) > initialQueueBytes)
          return;
        await transport.send(
...
export class LinkQualityService {
  ...
  async probe(
    ...
    if (
      ...
      budgetBytes < 256 ||
      budgetBytes > DEFAULT_LINK_PROBE_BUDGET_BYTES
    ) {
      throw new LinkServiceError(
        "LINK_BAD_REQUEST",
