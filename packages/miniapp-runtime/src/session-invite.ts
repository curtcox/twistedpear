...
export class SessionInviteService {
  ...
  constructor(private readonly chrome: SessionInviteChrome, private readonly now: () => number = () => 0) {}

  /** Called by the host LXMF delivery path; no mini-app code executes here. */
  async receive(input: Omit<SessionInvite, "receivedAt" | "phase"> & { readonly verified: boolean }): Promise<void> {
    if (!input.verified) throw new Error("Session invite peer must be verified");
    if (input.id.length < 1 || input.id.length > 256 || input.appId.length < 1 || input.appId.length > 256) {
      throw new Error("Session invite identity is invalid");
    }
    if (this.invites.has(input.id)) throw new Error("Session invite replay was rejected");
    if (input.verifiedPeerLabel.length < 1 || input.verifiedPeerLabel.length > 160) {
      throw new Error("Session invite peer label is invalid");
    }
    if (input.expiresAt <= this.now()) throw new Error("Session invite is unavailable");
    ...
    const invite: SessionInvite = { ...input, receivedAt: this.now(), phase: "pending" };
  ...
  list(): ReadonlyArray<SessionInvite> {
    ...
    for (const [id, invite] of this.invites) {
      if (invite.phase === "pending" && at >= invite.expiresAt) this.invites.set(id, { ...invite, phase: "expired" });
    }
  ...
  private pending(id: string): SessionInvite {
    ...
    if (invite === undefined || invite.phase !== "pending" || this.now() >= invite.expiresAt) {
