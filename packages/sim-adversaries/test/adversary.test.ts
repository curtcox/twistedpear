...
import {
  ...
  SimKernel,
  OracleViolation,
} from "../../effects/src/adapters/sim/kernel.js";
import { UnauthorizedAdversaryPowerError } from "../../effects/src/adapters/sim/transport.js";
import {
  rerunHistory,
  shrinkHistory,
} from "../../effects/src/adapters/sim/shrink.js";
import {
  parseHistory,
  type RecordedHistory,
} from "../../effects/src/adapters/sim/recorder.js";
...
interface State extends AdversaryState {
  readonly received: number;
}
const initial: State = { acted: false, entropyRequested: false, received: 0 };

const sender: StepFn<State> = (state, event) =>
  event.kind === "start"
    ? {
        state,
        intents: [
    ...
    : { state, intents: [] };
const receiver: StepFn<State> = (state, event) =>
...
const duplicateOracle = {
  ...
  check: (
    world: import("../../effects/src/adapters/sim/oracles.js").WorldView<State>,
  ) =>
    [...world.nodes].some(([, state]) => state.received > 1)
      ? {
          oracle: "duplicate-delivery",
          message: "receiver accepted duplicate delivery",
        }
      : null,
};
...
function widen(step: StepFn<AdversaryState>): StepFn<State> {
  return (state, event) => {
    ...
    return {
      ...
      intents: result.intents as Intent[],
    };
  };
}
...
describe("Dolev-Yao adversaries", () => {
  ...
  it("provides deterministic grant-boundary mutations to the fuzz tier", () => {
    ...
    expect(first.map((entry) => Array.from(entry))).toEqual(
      second.map((entry) => Array.from(entry)),
    );
    expect(
      first.every(
        (entry) =>
          entry.some((byte, index) => byte !== canonical[index]) ||
  ...
  it("classifies every hostile-app fixture as lifted or out of model", () => {
    ...
    expect(
      new Set(HISTORICAL_REPLAY_FIXTURES.map((fixture) => fixture.source)).size,
    ).toBeGreaterThanOrEqual(5);
    for (const fixture of HISTORICAL_REPLAY_FIXTURES) {
      expect(
        fixture.expressible ? fixture.proposal : fixture.reason,
      ).toBeTruthy();
      if (fixture.proposal !== undefined)
        expect(() =>
          compileAttackProposal(fixture.proposal, [
            "drop",
            "delay",
            "reorder",
            "duplicate",
  ...
  it("executes every expressible historical case against its reviewed target outcome", async () => {
    const expressible = HISTORICAL_REPLAY_FIXTURES.filter(
      (fixture) => fixture.expressible,
    );
    expect(expressible.length).toBeGreaterThan(0);
    for (const fixture of expressible) {
      expect(fixture.target).toBeTruthy();
      await expect(executeHistoricalFixture(fixture)).resolves.toBe(
  ...
  it("fails the historical floor when each named target policy is weakened", async () => {
    for (const target of ["broker", "handshake", "grant"] as const) {
      const fixture = HISTORICAL_REPLAY_FIXTURES.find(
        (entry) => entry.expressible && entry.target === target,
      )!;
      await expect(
        executeHistoricalFixture(fixture, 1, { disableContainmentFor: target }),
...
describe("Dolev-Yao adversaries (continued)", () => {
  it("searches for and shrinks a seeded canary instead of selecting it by predicate", () => {
    ...
    expect(found.minimized.trace.length).toBeLessThan(
      found.history.trace.length,
    );
    expect({
      seed: found.seed,
      oracle: found.minimized.violation?.oracle,
      minimizedTraceLength: found.minimized.trace.length,
    }).toEqual({
      seed: 1,
      oracle: "fuzz-canary",
      minimizedTraceLength: 2,
    });
  });

  it("puts a model in the authoring loop but compiles only in-model output", async () => {
    let calls = 0;
    const result = await authorAttackStrategies(
      async (prompt) => {
        calls += 1;
      ...
      {
        objective: "find replay bugs",
        allowedPowers: ["duplicate"],
        nodes: ["a", "b"],
        channels: ["x"],
      },
    );
    expect(calls).toBe(1);
    expect(result.accepted.map((entry) => entry.proposal.name)).toEqual([
      "duplicate",
    ]);
  ...
  it("replays the model-authored finding without a model", () => {
    const history = parseHistory<State>(
      JSON.stringify(duplicateFixture),
    ) as RecordedHistory<State>;
  ...
  it("authors, compiles, executes, finds, shrinks, and replays without calling the model again", async () => {
    let modelCalls = 0;
    const authored = await authorAttackStrategies(
      async () => {
        modelCalls += 1;
        return JSON.stringify([
          {
    ...
    const resolveMachine = (machine: string) =>
      machine === "authored/sender"
        ? sender
        : machine === "authored/receiver"
          ? receiver
          : zStep;
    const minimized = shrinkHistory(
      violation!.history as RecordedHistory<State>,
      {
        resolveMachine,
        oracles: [duplicateOracle],
      },
    );
    expect(minimized.trace.length).toBeLessThan(
      violation!.history.trace.length,
    );
    expect(
      rerunHistory(minimized, { resolveMachine, oracles: [duplicateOracle] })
