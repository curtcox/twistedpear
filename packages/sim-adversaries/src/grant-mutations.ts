...
export function grantRecordMutationCorpus(canonical: Uint8Array): readonly Uint8Array[] {
  ...
  return mutations.filter((value, index) => value !== text && mutations.indexOf(value) === index).map(encodeAscii);
}
