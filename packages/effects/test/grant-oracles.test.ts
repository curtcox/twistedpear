...
interface GrantOracleState {
  ...
  readonly authorizations: readonly { id: string; revokedAt?: number; accessTimes: readonly number[] }[];
}

const good: GrantOracleState = {
  stored: ["blob-a"], live: ["blob-a"], identities: [{ id: "grant-a", fingerprint: "authority-a" }],
...
const step: StepFn<GrantOracleState> = (state, event: Event) => {
  ...
  if (event.channel === "break/coverage") return { state: { ...state, stored: [...state.stored, "blob-orphan"] }, intents: [] };
  if (event.channel === "break/identity") return { state: { ...state,
    identities: [{ id: "grant-a", fingerprint: "authority-b" }] }, intents: [] };
  if (event.channel === "break/revoke") return { state: { ...state,
    authorizations: [{ id: "grant-a", revokedAt: 10, accessTimes: [11] }] }, intents: [] };
  return { state, intents: [] };
};

const coverage = grantCoverageOracle<GrantOracleState>((state) => ({
  storedBlobIds: state.stored, liveGrantBlobIds: state.live
}));
const uniqueness = idUniquenessOracle<GrantOracleState>((state) => state.identities);
const revocation = revocationMonotonicityOracle<GrantOracleState>((state) => state.authorizations);

describe("foundational grant oracles on multi-node production projections", () => {
  it("accepts covered, unique, monotonic grant state", () => {
    const kernel = new SimKernel({ seed: 1, nodes: [
      { id: "authority", machine: "grant-projection", initial: good, step },
      { id: "storage", machine: "grant-projection", initial: { ...good, identities: [] }, step }
  ...
  for (const item of [
    ...
    it(`records, replays, and shrinks a typed ${item.oracle.name} violation`, () => {
      ...
      try { kernel.inject(item.node, recv(item.channel)); } catch (error) {
        if (error instanceof OracleViolation) violation = error; else throw error;
      }
      expect(violation).toBeInstanceOf(OracleViolation);
      expect(violation!.violation.oracle).toBe(item.oracle.name);
      const minimized = shrinkHistory(violation!.history as any, { resolveMachine: () => step, oracles });
      expect(minimized.trace).toHaveLength(1);
      expect(rerunHistory(minimized, { resolveMachine: () => step, oracles }).violation.violation.oracle)
...
function recv(channel: string): Event { return { kind: "transport/recv", channel, source: "test",
  payload: new Uint8Array(), at: 1 }; }
