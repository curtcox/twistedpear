...
const stepDemo: StepFn<DemoState> = (state, event: Event) => {
  ...
  if (event.kind === "start") {
    intents.push(
      ...
      {
        kind: "store/write",
        write: { key: "boot", value: new Uint8Array([9, 9]) },
      },
    );
  ...
  if (event.kind === "entropy") {
    return {
      state,
      intents: [
        { kind: "store/write", write: { key: "seed", value: event.bytes } },
      ],
    };
  }
  ...
  if (event.kind === "timer/fired") {
    return {
      state,
      intents: [{ kind: "log", level: "debug", message: `tick@${event.at}` }],
    };
  }
  return { state, intents: [] };
};
...
function demoHistory() {
  const kernel = new SimKernel<DemoState>({
    ...
    nodes: [
      {
        id: "a",
        machine: "spec-trace-demo",
        initial: { peer: "b", pongs: 0 },
        step: stepDemo,
      },
      {
        id: "b",
        machine: "spec-trace-demo",
        initial: { peer: "a", pongs: 0 },
        step: stepDemo,
      },
    ],
...
describe("SPEC-TRACE canonical form", () => {
  it("serializes object keys sorted regardless of insertion order", () => {
    const scrambled = JSON.parse('{"node":"a","t":"advance","at":5}') as Record<
      string,
      unknown
    >;
    expect(canonicalJson(scrambled)).toBe('{"at":5,"node":"a","t":"advance"}');
    expect(canonicalJson({ t: "advance", at: 5, node: "a" })).toBe(
      '{"at":5,"node":"a","t":"advance"}',
    );
  });

  it("encodes bytes, drops undefined, rejects non-finite numbers", () => {
    expect(canonicalJson({ b: new Uint8Array([0, 255]), u: undefined })).toBe(
      '{"b":{"$bytes":"00ff"}}',
    );
  ...
  it("hashes are insensitive to producer key order", () => {
    const a: TraceEntry[] = [
      { t: "event", node: "n", event: { kind: "start", at: 0 } },
    ];
    const b = JSON.parse(
      '[{"event":{"at":0,"kind":"start"},"node":"n","t":"event"}]',
    ) as TraceEntry[];
...
describe("SPEC-TRACE schemas", () => {
  const entryValidator = createValidator(
    join(schemaDir, "trace-entry.schema.json"),
  );
  const historyValidator = createValidator(
    join(schemaDir, "recorded-history.schema.json"),
  );
  ...
  it("accept the stored regression and campaign reproducer fixtures", () => {
    ...
    if (existsSync(reproducers)) {
      const names = readdirSync(reproducers)
  ...
  it("detects a machine that diverges from the recording", () => {
    ...
    const divergent: StepFn<DemoState> = (state, event) => {
      ...
      if (event.kind === "timer/fired") {
        return {
          state: result.state,
          intents: [{ kind: "log", level: "info", message: "divergent" }],
        };
      }
      return result;
    };
