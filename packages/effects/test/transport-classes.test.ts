...
describe("executable transport classes", () => {
  it("makes LoRa scarcity visible and remains deterministic", () => {
    ...
    expect(lora.transport.nextDeliverAt()).toBeGreaterThan(
      lan.transport.nextDeliverAt() ?? 0,
    );
  ...
  it("reorders actual delivery slots and reports affected messages", () => {
    ...
    for (const channel of ["first", "second", "third"])
      transport.applySend(
        {
          kind: "transport/send",
          send: { channel, destination: "b", payload: new Uint8Array([1]) },
        },
        "a",
        0,
      );
    transport.applyAdversary(
      { power: "reorder", source: "a", destination: "b" },
      "z",
      0,
    );
    expect(
      transport
        .deliverDue(Number.MAX_SAFE_INTEGER)
