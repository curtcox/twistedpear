...
describe("SimulatedMediaCodecDriver", () => {
  it("exercises the codec effect boundary without aliasing media buffers", async () => {
    ...
    await driver.close();
    await expect(
      driver.encode(configuration, { captureAtUs: 20, bytes }),
...
describe("BundledOpusMediaCodecDriver", () => {
  it("compresses mono float32 PCM to Opus and decodes back", async () => {
    ...
    const out = new Float32Array(
      ...
      decoded.bytes.byteLength / 4,
    );
    let energy = 0;
    for (let index = 0; index < out.length; index += 1)
      energy += (out[index] ?? 0) ** 2;
...
describe("WebCodecsMediaCodecDriver", () => {
  it("uses the host WebCodecs boundary for mono Opus", async () => {
    ...
    class EncoderFake {
      constructor(
        private readonly callbacks: { output(chunk: ChunkFake): void },
      ) {}
      configure() {}
    ...
    class DecoderFake {
      constructor(
        private readonly callbacks: { output(audio: AudioDataFake): void },
      ) {}
      configure() {}
      decode(chunk: ChunkFake) {
        this.callbacks.output(
          new AudioDataFake({
            data: chunk.bytes.slice().buffer,
            numberOfFrames: chunk.bytes.length / 4,
          }),
        );
      }
      flush() {
    ...
    try {
      ...
      await expect(driver.decode(configuration, encoded)).resolves.toEqual({
        captureAtUs: 12,
        bytes,
      });
    } finally {
