...
import {
  ...
  rerunHistory,
  shrinkHistory,
} from "../src/adapters/sim/shrink.js";
...
describe("oracle recording and shrinking", () => {
  it("records a self-describing history and reruns the same oracle", () => {
    ...
    expect(() => kernel.inject("host", { kind: "tick", at: 777 })).toThrow(
      OracleViolation,
    );
  ...
  it("serializes bytes and writes through the portable on-disk recorder", () => {
    ...
    const recorder = new FileHistoryRecorder<State>(
      "/histories",
      (path, contents) => {
    ...
    expect(
      event?.t === "event" && event.event.kind === "entropy"
        ? [...event.event.bytes]
        : [],
    ).toEqual([1, 2, 255]);
  });
    ...
    for (let at = 0; at < 1_000; at += 1) {
      try {
        ...
        const minimized = shrinkHistory(
          error.history as import("../src/adapters/sim/recorder.js").RecordedHistory<State>,
          {
            resolveMachine: () => step,
            oracles: [oracle],
          },
        ...
        expect(minimized.trace[0]).toMatchObject({
          t: "event",
          event: { kind: "tick", at: 777 },
        });
        return;
      }
    }
  ...
  it("ddmin returns a known one-minimal set", () => {
    expect(
      ddmin(
        [1, 2, 3, 4, 5],
        (candidate) => candidate.includes(2) && candidate.includes(5),
      ),
    ).toEqual([2, 5]);
  });
});
