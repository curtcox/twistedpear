...
describe("device session machine", () => {
  ...
  it("expires and refuses revival", () => {
    ...
    const revived = stepDeviceSession(state, {
      kind: "device/open",
      at: 20,
      ttlMs: 100,
    });
  ...
  it("enumerates the legal transition table", () => {
    expect(
      deviceSessionMachine.table.map(
  ...
  it("checks in a Layer-3 vector for every table cell", () => {
    expect(vectors.cells).toHaveLength(
      enumerateCells(deviceSessionMachine).length,
    );
    expect(vectors.cells).toHaveLength(
      deviceSessionMachine.states.length * deviceSessionMachine.events.length,
    );
    expect(vectors.cells.filter((cell) => cell.legal)).toHaveLength(
      deviceSessionMachine.table.length,
    );
  });
});
...
describe("device quantize", () => {
  it("quantizes location to ~1 km cells", () => {
    const coarse = quantizeLocationCoarse({
      latitude: 37.7749,
      longitude: -122.4194,
    });
...
describe("device registry", () => {
  it("pins host API 0.12.0 and includes device classes", () => {
    ...
    expect(deviceCapabilityId("location", "precise")).toBe(
