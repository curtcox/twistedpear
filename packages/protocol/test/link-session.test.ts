...
describe("protocol link session", () => {
  it("initiator requests a link then becomes active on proof", () => {
    ...
    let result = stepLinkSessionWithActions(state, {
      kind: "session/request-link",
      at: 10,
    });
    expect(result.actions[0]).toEqual({
      kind: "send-link-request",
      peerId: "b",
    });
  ...
  it("responder sends proof on establish", () => {
    ...
    expect(
      result.actions.some((action) => action.kind === "send-link-proof"),
    ).toBe(true);
    expect(result.state.established).toBe(true);
  });
  ...
  it("double-runs identically", () => {
    const run = () => {
      let state = initialLinkSessionState({
        role: "initiator",
        peerId: "peer",
      });
      state = stepLinkSessionWithActions(state, {
        kind: "session/request-link",
        at: 1,
      }).state;
