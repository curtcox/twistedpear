...
describe("protocol packet context", () => {
  it("plans link DATA context dispatch kinds", () => {
    ...
    expect(planLinkDataContext(PacketContextCode.LINKIDENTIFY)).toBe(
      "identify",
    );
    ...
    expect(planLinkDataContext(PacketContextCode.RESOURCE_ADV)).toBe(
      "resource-adv",
    );
    expect(planLinkDataContext(PacketContextCode.RESOURCE_REQ)).toBe(
      "resource-req",
    );
    expect(planLinkDataContext(PacketContextCode.RESOURCE_HMU)).toBe(
      "resource-hmu",
    );
    expect(planLinkDataContext(PacketContextCode.RESOURCE_ICL)).toBe(
      "resource-icl",
    );
    expect(planLinkDataContext(PacketContextCode.RESOURCE_RCL)).toBe(
      "resource-rcl",
    );
  ...
  it("emits link DATA context actions without ad-hoc plan reads", () => {
    const cases: Array<{
      ...
      {
        context: PacketContextCode.LRRTT,
        kind: "rtt",
        check: shouldHandleLinkDataRtt,
      },
      {
        context: PacketContextCode.KEEPALIVE,
        kind: "keepalive",
        check: shouldHandleLinkDataKeepalive,
      },
      {
        context: PacketContextCode.LINKCLOSE,
        kind: "close",
        check: shouldHandleLinkDataClose,
      },
      {
        context: PacketContextCode.LINKIDENTIFY,
        kind: "identify",
        check: shouldHandleLinkDataIdentify,
      },
      {
        context: PacketContextCode.REQUEST,
        kind: "request",
        check: shouldHandleLinkDataRequest,
      },
      {
        context: PacketContextCode.RESPONSE,
        kind: "response",
        check: shouldHandleLinkDataResponse,
      },
      {
        context: PacketContextCode.CHANNEL,
        kind: "channel",
        check: shouldHandleLinkDataChannel,
      },
      {
        context: PacketContextCode.RESOURCE_ADV,
        kind: "resource-adv",
        check: shouldHandleLinkDataResourceAdv,
      },
      {
        context: PacketContextCode.RESOURCE_REQ,
        kind: "resource-req",
        check: shouldHandleLinkDataResourceReq,
      },
      {
        context: PacketContextCode.RESOURCE_HMU,
        kind: "resource-hmu",
        check: shouldHandleLinkDataResourceHmu,
      },
      {
        context: PacketContextCode.RESOURCE_ICL,
        kind: "resource-icl",
        check: shouldHandleLinkDataResourceIcl,
      },
      {
        context: PacketContextCode.RESOURCE_RCL,
        kind: "resource-rcl",
        check: shouldHandleLinkDataResourceRcl,
      },
      {
        context: PacketContextCode.RESOURCE,
        kind: "resource",
        check: shouldHandleLinkDataResource,
      },
      {
        context: PacketContextCode.NONE,
        kind: "plaintext",
        check: shouldHandleLinkDataPlaintext,
      },
      {
        context: PacketContextCode.LRPROOF,
        kind: "ignore",
        check: shouldIgnoreLinkDataContext,
      },
    ];

    for (const entry of cases) {
      const planned = stepLinkDataContextPlanWithActions(
        initialLinkDataContextPlanState(),
        {
      ...
      const stepped = stepLinkDataContextWithActions(
        initialLinkDataContextState(),
        {
  ...
  it("emits keepalive-context only from keepalive/other actions", () => {
    const keepalive = stepLinkKeepaliveContextWithActions(
      initialLinkKeepaliveContextState(),
      {
    ...
    const other = stepLinkKeepaliveContextWithActions(
      initialLinkKeepaliveContextState(),
      {
        kind: "link/keepalive-context-gate",
        context: PacketContextCode.NONE,
    ...
    const empty = stepLinkKeepaliveContextWithActions(
      initialLinkKeepaliveContextState(),
      {
        kind: "timer/fired",
        timer: { id: "x" },
