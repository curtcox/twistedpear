...
describe("protocol link keygen entropy", () => {
  it("splits initiator entropy into two keys", () => {
    const entropy = new Uint8Array(LINK_INITIATOR_ENTROPY_SIZE).map(
      (_, i) => i + 1,
    );
  ...
  it("splits responder entropy into one key", () => {
    const entropy = new Uint8Array(LINK_RESPONDER_ENTROPY_SIZE).map(
      (_, i) => 200 - i,
    );
  ...
  it("rejects short entropy", () => {
    expect(() => splitInitiatorLinkEntropy(new Uint8Array(63))).toThrow(
      /at least 64/,
    );
    expect(() => splitResponderLinkEntropy(new Uint8Array(31))).toThrow(
      /at least 32/,
    );
  });
  ...
  it("emits use-fields|reject actions for initiator entropy split", () => {
    ...
    const ok = stepSplitInitiatorLinkEntropyWithActions(
      initialSplitInitiatorLinkEntropyState(),
      {
  ...
  it("emits use-fields|reject actions for responder entropy split", () => {
    ...
    const ok = stepSplitResponderLinkEntropyWithActions(
      initialSplitResponderLinkEntropyState(),
      {
