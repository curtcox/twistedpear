...
describe("protocol resource status", () => {
  ...
  it("plans advertise phase, assemble outcome, and proof accept", () => {
    ...
    expect(shouldQueueResourceAdvertisePhasePlan(queuePhase.actions)).toBe(
      true,
    );
    expect(resourceAdvertisePhasePlanFromActions(queuePhase.actions)).toBe(
      "queue",
    );
    ...
    expect(
      shouldAdvertiseResourceAdvertisePhasePlan(advertisePhase.actions),
    ).toBe(true);
    expect(resourceAdvertisePhasePlanFromActions(advertisePhase.actions)).toBe(
      "advertise",
    );
  ...
  it("emits resource assemble/proof-accept actions from WithActions steps", () => {
    ...
    expect(
      shouldCompleteResourceAssembleOutcomePlan(completePlan.actions),
    ).toBe(true);
    expect(resourceAssembleOutcomePlanFromActions(completePlan.actions)).toBe(
      "complete",
    );
    ...
    expect(shouldCorruptResourceAssembleOutcomePlan(corruptPlan.actions)).toBe(
      true,
    );
    expect(resourceAssembleOutcomePlanFromActions(corruptPlan.actions)).toBe(
      "corrupt",
    );

    const complete = stepResourceAssembleWithActions(
      initialResourceAssembleState(),
      {
    ...
    expect(shouldCommitResourceAssemblePayloadNow(skipCommit.actions)).toBe(
      false,
    );
    expect(shouldSkipCommitResourceAssemblePayload(skipCommit.actions)).toBe(
      true,
    );

    const corrupt = stepResourceAssembleWithActions(
      initialResourceAssembleState(),
      {
        kind: "resource/assemble-gate",
        decryptedPresent: false,
    ...
    expect(shouldCompleteResourceProofAcceptPlan(acceptPlan.actions)).toBe(
      true,
    );
    expect(resourceProofAcceptPlanFromActions(acceptPlan.actions)).toBe(
      "complete",
    );
    ...
    expect(resourceProofAcceptPlanFromActions(ignorePlan.actions)).toBe(
      "ignore",
    );

    const accept = stepResourceProofAcceptWithActions(
      initialResourceProofAcceptState(),
      {
    ...
    const ignore = stepResourceProofAcceptWithActions(
      initialResourceProofAcceptState(),
      {
        kind: "resource/proof-accept-gate",
        status: ResourceStatus.AWAITING_PROOF,
  ...
  it("emits transfer/status gate actions from WithActions steps", () => {
    ...
    const watchdogOk = stepResourceWatchdogAllowWithActions(
      initialResourceWatchdogAllowState(),
      {
    ...
    const watchdogDeny = stepResourceWatchdogAllowWithActions(
      initialResourceWatchdogAllowState(),
      {
        kind: "resource/watchdog-allow-gate",
        status: ResourceStatus.FAILED,
    ...
    const proveOk = stepProveResourceAllowWithActions(
      initialProveResourceAllowState(),
      {
    ...
    const proveDeny = stepProveResourceAllowWithActions(
      initialProveResourceAllowState(),
      {
        kind: "resource/prove-allow-gate",
        dataPresent: false,
    ...
    const advertiseOk = stepAdvertiseResourceWithActions(
      initialAdvertiseResourceState(),
      {
    ...
    const advertiseSkip = stepAdvertiseResourceWithActions(
      initialAdvertiseResourceState(),
      {
        kind: "resource/advertise-option-gate",
    ...
    expect(
      shouldAcceptIncomingResourceAdvertisementNow(incomingOk.actions),
    ).toBe(true);
    ...
    expect(shouldSkipIncomingResourceAdvertisement(incomingSkip.actions)).toBe(
      true,
    );
  });
  ...
  it("steps through advertise → transferring → awaiting-proof → complete", () => {
    ...
    state = applyResourceStatusEvent(state, {
      kind: "resource/awaiting-proof",
    });
  ...
  it("steps assemble → corrupt / fail", () => {
    let state = applyResourceStatusEvent(
      initialResourceStatusState(ResourceStatus.TRANSFERRING),
      {
