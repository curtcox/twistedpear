...
describe("protocol link proof materials", () => {
  it("builds signed material and packs proof data", () => {
    ...
    const signed = linkProofSignedMaterial(
      linkId,
      publicKey,
      ownerSig,
      signalling,
    ...
    expect(signed.length).toBe(
      linkId.length + publicKey.length + ownerSig.length + signalling.length,
    );
    ...
    const signedFromActions = linkProofSignedMaterialRawFromActions(
    ...
    expect([...packed.subarray(0, LINK_PROOF_SIGNATURE_SIZE)]).toEqual([
      ...signature,
    ]);
  });
  ...
  it("emits signalling and MTU encode bytes from WithActions steps", () => {
    ...
    expect(shouldUseEncodeLinkSignallingBytes(signallingStepped.actions)).toBe(
      true,
    );
    const signalling = encodeLinkSignallingBytesRawFromActions(
      signallingStepped.actions,
    );
    expect(signalling).not.toBeNull();
    expect([...signalling!]).toEqual([...encodeLinkSignallingBytes(500, 0x01)]);

    const mtuStepped = stepEncodeLinkMtuBytesWithActions(
      initialEncodeLinkMtuBytesState(),
      {
  ...
  it("emits mode / MTU decode and proof-payload classify from WithActions steps", () => {
    ...
    expect(shouldUseModeFromLinkRequestData(requestModeStepped.actions)).toBe(
      true,
    );
    ...
    const proofModeStepped = stepModeFromLinkProofDataWithActions(
      initialModeFromLinkProofDataState(),
      {
    ...
    const requestMtuOk = stepMtuFromLinkRequestDataWithActions(
      initialMtuFromLinkRequestDataState(),
      {
    ...
    expect(shouldRejectMtuFromLinkRequestData(requestMtuOk.actions)).toBe(
      false,
    );
    expect(mtuFromLinkRequestDataFromActions(requestMtuOk.actions)).toBe(
      mtuFromLinkRequestData(requestData),
    );
    ...
    expect(shouldRejectMtuFromLinkRequestData(requestMtuRejected.actions)).toBe(
      true,
    );
    expect(
      mtuFromLinkRequestDataFromActions(requestMtuRejected.actions),
    ).toBeNull();

    const proofMtuOk = stepMtuFromLinkProofDataWithActions(
      initialMtuFromLinkProofDataState(),
      {
    ...
    expect(mtuFromLinkProofDataFromActions(proofMtuOk.actions)).toBe(
      mtuFromLinkProofData(proofData),
    );

    const proofMtuRejected = stepMtuFromLinkProofDataWithActions(
      initialMtuFromLinkProofDataState(),
      {
        kind: "link-proof/mtu-from-proof-gate",
    ...
    expect(shouldRejectMtuFromLinkProofData(proofMtuRejected.actions)).toBe(
      true,
    );
    expect(
      mtuFromLinkProofDataFromActions(proofMtuRejected.actions),
    ).toBeNull();

    const bodyOnly = stepClassifyLinkProofPayloadWithActions(
      initialClassifyLinkProofPayloadState(),
      {
    ...
    const bodyWithMtu = stepClassifyLinkProofPayloadWithActions(
      initialClassifyLinkProofPayloadState(),
      {
        kind: "link-proof/classify-payload-gate",
    ...
    expect(shouldClassifyLinkProofPayloadBodyWithMtu(bodyWithMtu.actions)).toBe(
      true,
    );

    const rejected = stepClassifyLinkProofPayloadWithActions(
      initialClassifyLinkProofPayloadState(),
      {
        kind: "link-proof/classify-payload-gate",
        dataLength: 10,
  ...
  it("packs and splits link-request payloads", () => {
    ...
    const packed = packLinkRequestData(
      publicKey,
      signaturePublicKey,
      signalling,
    );
    ...
    expect([
      ...linkRequestHashablePart(hashable, LINK_REQUEST_ECPUB_SIZE),
    ]).toEqual([...hashable]);
    expect(
      linkRequestHashablePart(hashable, LINK_REQUEST_ECPUB_SIZE + 3).length,
    ).toBe(17);
    ...
    expect(shouldUseLinkRequestHashablePart(hashableStepped.actions)).toBe(
      true,
    );
    const truncated = linkRequestHashablePartRawFromActions(
  ...
  it("emits pack framing bytes from WithActions steps", () => {
    ...
    const proofStepped = stepPackLinkProofDataWithActions(
      initialPackLinkProofDataState(),
      {
    ...
    expect([...proofPacked!]).toEqual([
      ...packLinkProofData(signature, publicKey, signalling),
    ]);

    const signaturePublicKey = new Uint8Array(LINK_PROOF_PUBLIC_KEY_SIZE).fill(
      3,
    );
    const requestStepped = stepPackLinkRequestDataWithActions(
      initialPackLinkRequestDataState(),
      {
    ...
    const requestPacked = packLinkRequestDataRawFromActions(
  ...
  it("emits split fields or reject from WithActions steps", () => {
    ...
    const proofOk = stepSplitLinkProofBodyWithActions(
      initialSplitLinkProofBodyState(),
      {
    ...
    const proofRejected = stepSplitLinkProofBodyWithActions(
      initialSplitLinkProofBodyState(),
      {
        kind: "link-proof/split-body-gate",
    ...
    const signalling = new Uint8Array([3, 4, 5]);
    const requestPacked = packLinkRequestData(
      publicKey,
      signaturePublicKey,
      signalling,
    );
    const requestOk = stepSplitLinkRequestDataWithActions(
      initialSplitLinkRequestDataState(),
      {
    ...
    const requestRejected = stepSplitLinkRequestDataWithActions(
      initialSplitLinkRequestDataState(),
      {
        kind: "link-request/split-gate",
        data: new Uint8Array(10),
    ...
    expect(shouldRejectSplitLinkRequestData(requestRejected.actions)).toBe(
      true,
    );
