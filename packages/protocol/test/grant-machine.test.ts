...
describe("grant lifecycle transition table", () => {
  it("contains only the legal lifecycle edges", () => {
    expect(
      grantMachine.table.map((row) => `${row.from}:${row.on.name}->${row.to}`),
    ).toEqual([
  ...
  it("checks in a vector for every table cell", () => {
    ...
    expect(vectors.cells).toHaveLength(
      grantMachine.states.length * grantMachine.events.length,
    );
    expect(vectors.cells.filter((cell) => cell.legal)).toHaveLength(
      grantMachine.table.length,
    );
  });
});
