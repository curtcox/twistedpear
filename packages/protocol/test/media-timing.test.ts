...
function frame(
  sequence: number,
  captureAtUs: number,
): Extract<DeviceStreamFrame, { version: 2 }> {
  return {
    version: 2,
...
describe("media timing", () => {
  it("estimates clock offset by halving RTT", () => {
    const estimate = updateClockOffset(null, {
      sentAtUs: 1_000,
      receivedAtUs: 1_200,
      remoteAtUs: 1_150,
    });
  ...
  it("orders capture timestamps and drains only presentation-ready frames", () => {
    ...
    state = pushJitterFrame(
      state,
      { frame: frame(2, 1_100), receivedAtUs: 1_050 },
      1_050,
    );
    state = pushJitterFrame(
      state,
      { frame: frame(1, 1_000), receivedAtUs: 1_040 },
      1_050,
    );
    const first = drainJitterBuffer(state, 1_100);
    expect(first.ready.map((entry) => entry.frame.sequence)).toEqual([1]);
    expect(
      drainJitterBuffer(first.state, 1_200).ready.map(
