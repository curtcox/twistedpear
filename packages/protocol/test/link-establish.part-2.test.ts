...
describe("protocol link establish", () => {
  it("gates proof validation and identify", () => {
    expect(
      canValidateLinkProof({ status: LinkStatus.PENDING, initiator: true }),
    ).toBe(true);
    expect(
      canValidateLinkProof({ status: LinkStatus.PENDING, initiator: false }),
    ).toBe(false);
    expect(
      canValidateLinkProof({
    ...
    expect(
      canIdentifyOnLink({ status: LinkStatus.ACTIVE, initiator: true }),
    ).toBe(true);
    expect(
      canIdentifyOnLink({ status: LinkStatus.ACTIVE, initiator: false }),
    ).toBe(false);
  });
...
describe("protocol link establish (continued)", () => {
  it("gates application requests on ACTIVE with RTT", () => {
    ...
    expect(canLinkRequest({ status: LinkStatus.PENDING, rtt: 0.1 })).toBe(
      false,
    );
    ...
    expect(
      planLinkTokenAccess({ derivedKeyPresent: false, tokenPresent: false }),
    ).toBe("reject-no-key");
    expect(
      planLinkTokenAccess({ derivedKeyPresent: true, tokenPresent: false }),
    ).toBe("create");
    expect(
      planLinkTokenAccess({ derivedKeyPresent: true, tokenPresent: true }),
    ).toBe("reuse");
    const rejectPlan = stepLinkTokenAccessPlanWithActions(
      initialLinkTokenAccessPlanState(),
      {
    ...
    expect(linkTokenAccessPlanFromActions(rejectPlan.actions)).toBe(
      "reject-no-key",
    );
    const createPlan = stepLinkTokenAccessPlanWithActions(
      initialLinkTokenAccessPlanState(),
      {
        kind: "token/access-plan-gate",
    ...
    const reusePlan = stepLinkTokenAccessPlanWithActions(
      initialLinkTokenAccessPlanState(),
      {
        kind: "token/access-plan-gate",
        derivedKeyPresent: true,
    ...
    const rejectToken = stepLinkTokenAccessWithActions(
      initialLinkTokenAccessState(),
      {
    ...
    const createToken = stepLinkTokenAccessWithActions(
      initialLinkTokenAccessState(),
      {
        kind: "token/access-gate",
        derivedKeyPresent: true,
    ...
    const reuseToken = stepLinkTokenAccessWithActions(
      initialLinkTokenAccessState(),
      {
        kind: "token/access-gate",
        derivedKeyPresent: true,
  ...
  it("concludes reuse active link via actions", () => {
    const reuse = stepReuseActiveLinkWithActions(
      initialReuseActiveLinkState(),
      {
...
describe("protocol link establish (continued)", () => {
  it("concludes link closed / packet-interface / encrypt / request / last-data via actions", () => {
    ...
    const encrypt = stepEncryptLinkPayloadWithActions(
      initialEncryptLinkPayloadState(),
      {
    ...
    const plaintext = stepEncryptLinkPayloadWithActions(
      initialEncryptLinkPayloadState(),
      {
        kind: "link/encrypt-payload-gate",
        encryptOption: false,
    ...
    const requestDeny = stepLinkRequestAllowWithActions(
      initialLinkRequestAllowState(),
      {
        kind: "link/request-allow-gate",
        status: LinkStatus.ACTIVE,
    ...
    const update = stepUpdateLinkLastDataWithActions(
      initialUpdateLinkLastDataState(),
      {
    ...
    const skipUpdate = stepUpdateLinkLastDataWithActions(
      initialUpdateLinkLastDataState(),
      {
        kind: "link/update-last-data-gate",
    ...
    const data = stepLinkInboundDataPacketWithActions(
      initialLinkInboundDataPacketState(),
      {
    ...
    const other = stepLinkInboundDataPacketWithActions(
      initialLinkInboundDataPacketState(),
      {
        kind: "link/inbound-data-packet-gate",
    ...
    const create = stepCreateLinkChannelWithActions(
      initialCreateLinkChannelState(),
      {
    ...
    const reuseCh = stepCreateLinkChannelWithActions(
      initialCreateLinkChannelState(),
      {
        kind: "link/create-channel-gate",
        channelPresent: true,
...
describe("protocol link establish (continued)", () => {
  it("concludes handshake / prove / validate / identify / rtt / plaintext / resend via actions", () => {
    ...
    const proveDeny = stepProveLinkAllowWithActions(
      initialProveLinkAllowState(),
      {
        kind: "link/prove-allow-gate",
        ownerPresent: true,
    ...
    const validate = stepValidateLinkProofAllowWithActions(
      initialValidateLinkProofAllowState(),
      {
    ...
    const crypto = stepAttemptLinkProofCryptoWithActions(
      initialAttemptLinkProofCryptoState(),
      {
    ...
    const cryptoSkip = stepAttemptLinkProofCryptoWithActions(
      initialAttemptLinkProofCryptoState(),
      {
        kind: "link/attempt-proof-crypto-gate",
        modeMatches: false,
    ...
    const teardownRtt = stepTeardownLinkFromRttWithActions(
      initialTeardownLinkFromRttState(),
      {
    ...
    const teardownRttSkip = stepTeardownLinkFromRttWithActions(
      initialTeardownLinkFromRttState(),
      {
        kind: "link/teardown-from-rtt-gate",
    ...
    const identify = stepIdentifyOnLinkAllowWithActions(
      initialIdentifyOnLinkAllowState(),
      {
    ...
    const identifyDeny = stepIdentifyOnLinkAllowWithActions(
      initialIdentifyOnLinkAllowState(),
      {
        kind: "link/identify-allow-gate",
        status: LinkStatus.ACTIVE,
    ...
    const plaintext = stepDispatchLinkPlaintextWithActions(
      initialDispatchLinkPlaintextState(),
      {
    ...
    const resend = stepResendLinkPacketAllowWithActions(
      initialResendLinkPacketAllowState(),
      {
    ...
    const resendDeny = stepResendLinkPacketAllowWithActions(
      initialResendLinkPacketAllowState(),
      {
        kind: "link/resend-packet-allow-gate",
        packetDecoded: true,
...
describe("protocol link establish (continued)", () => {
  it("reuses present ACTIVE links", () => {
    ...
    expect(
      shouldReuseActiveLink({ linkPresent: false, status: LinkStatus.ACTIVE }),
    ).toBe(false);
    expect(
...
describe("protocol link establish (continued)", () => {
  it("plans link validate-request gates without ad-hoc plan === reads", () => {
    ...
    const okPlan = stepLinkValidateRequestPlanWithActions(
      initialLinkValidateRequestPlanState(),
      {
    ...
    const badPlan = stepLinkValidateRequestPlanWithActions(
      initialLinkValidateRequestPlanState(),
      {
        kind: "validate-request/plan-gate",
        requestPresent: false,
    ...
    const ownerPlan = stepLinkValidateRequestPlanWithActions(
      initialLinkValidateRequestPlanState(),
      {
        kind: "validate-request/plan-gate",
        requestPresent: true,
    ...
    expect(
      shouldOwnerMissingIdentityLinkValidateRequestPlan(ownerPlan.actions),
    ).toBe(true);
    const modePlan = stepLinkValidateRequestPlanWithActions(
      initialLinkValidateRequestPlanState(),
      {
        kind: "validate-request/plan-gate",
        requestPresent: true,
    ...
    expect(shouldModeDisabledLinkValidateRequestPlan(modePlan.actions)).toBe(
      true,
    );

    const proceed = stepLinkValidateRequestWithActions(
      initialLinkValidateRequestState(),
      {
    ...
    expect(shouldSkipContinueLinkValidateRequest(skipMissing.actions)).toBe(
      true,
    );
    expect(
      shouldContinueLinkValidateRequest({
    ...
    const badRequest = stepLinkValidateRequestWithActions(
      initialLinkValidateRequestState(),
      {
        kind: "validate-request/gate",
        requestPresent: false,
    ...
    const ownerMissing = stepLinkValidateRequestWithActions(
      initialLinkValidateRequestState(),
      {
        kind: "validate-request/gate",
        requestPresent: true,
    ...
    expect(
      shouldRejectLinkValidateOwnerMissingIdentity(ownerMissing.actions),
    ).toBe(true);

    const modeDisabled = stepLinkValidateRequestWithActions(
      initialLinkValidateRequestState(),
      {
        kind: "validate-request/gate",
        requestPresent: true,
    ...
    expect(shouldRejectLinkValidateModeDisabled(modeDisabled.actions)).toBe(
      true,
    );
  });
});
...
describe("protocol link establish (continued)", () => {
  it("plans initiator MTU from discovery and next-hop", () => {
    ...
    const discoveredPlan = stepLinkInitiatorMtuPlanWithActions(
      initialLinkInitiatorMtuPlanState(),
      {
    ...
    const discovered = stepLinkInitiatorMtuWithActions(
      initialLinkInitiatorMtuState(),
      {
    ...
    const fallback = stepLinkInitiatorMtuWithActions(
      initialLinkInitiatorMtuState(),
      {
        kind: "link/initiator-mtu-gate",
        discoveryEnabled: false,
