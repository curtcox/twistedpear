...
describe("design-first escrow and recovery tables", () => {
  it("never releases escrow without quorum", () => {
    ...
    const below = stepEscrow(state, {
      kind: "escrow/authorize",
      authorizers: ["a"],
    }).state;
    expect(below.phase).toBe("release-requested");
    const released = stepEscrow(state, {
      kind: "escrow/authorize",
      authorizers: ["a", "b"],
    }).state;
  ...
  it("never recovers below threshold, including duplicate shares", () => {
    ...
    state = stepRecoveryQuorum(state, {
      kind: "recovery/share",
      guardian: "a",
    }).state;
    state = stepRecoveryQuorum(state, {
    ...
    expect(
      stepRecoveryQuorum(state, { kind: "recovery/authorize" }).state.phase,
    ).toBe("collecting");
    state = stepRecoveryQuorum(state, {
      kind: "recovery/share",
      guardian: "b",
    }).state;
    const recovered = stepRecoveryQuorum(state, {
  ...
  it("commits every state/event vector cell from both tables", () => {
    expect(escrowVectors.cells).toHaveLength(
      enumerateCells(escrowMachine).length,
    );
    expect(recoveryVectors.cells).toHaveLength(
      enumerateCells(recoveryQuorumMachine).length,
    );
  });
});
