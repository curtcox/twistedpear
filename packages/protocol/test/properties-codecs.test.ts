...
describe("protocol codec properties", () => {
  ...
  it("round-trips packet headers", () => {
    fc.assert(
      fc.property(
        fc.record({
          headerType: fc.constantFrom(PACKET_HEADER_1, PACKET_HEADER_2),
          contextFlag: fc.integer({ min: 0, max: 1 }),
          transportType: fc.integer({ min: 0, max: 1 }),
          destinationType: fc.integer({ min: 0, max: 3 }),
          packetType: fc.integer({ min: 0, max: 3 }),
          hops: fc.integer({ min: 0, max: 255 }),
          context: fc.integer({ min: 0, max: 255 }),
          destinationHash: fixedBytes(TRANSPORT_ID_BYTES),
          transportId: fixedBytes(TRANSPORT_ID_BYTES),
          data: bytes(),
        }),
        ({
          headerType,
...
describe("protocol resource and transport codec properties", () => {
