...
const parameters = {
  numRuns,
  seed: process.env.PROPERTY_SEED ? Number(process.env.PROPERTY_SEED) : undefined
};
const bytes = (maxLength = 2048) => fc.uint8Array({ maxLength });
const fixedBytes = (length: number) => fc.uint8Array({ minLength: length, maxLength: length });
...
describe("protocol codec properties", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (integer, binary, integers, entries) => {
          expect(msgpackUnpack(msgpackPackUInt(integer))).toEqual({ type: "int", int: integer });
          expect(msgpackUnpack(msgpackPackBin(binary))).toEqual({ type: "bin", bin: binary });

          const array = msgpackUnpack(msgpackPackArray(integers.map(msgpackPackUInt)));
          expect(array.type).toBe("array");
          if (array.type === "array") {
            expect(array.array).toEqual(integers.map((int) => ({ type: "int", int })));
          }
  ...
  it("round-trips packet headers", () => {
    ...
    fc.assert(
      fc.property(
        ...
        fixedBytes(TRANSPORT_ID_BYTES),
        bytes(),
        (headerType, contextFlag, transportType, destinationType, packetType, hops, context, destinationHash, transportId, data) => {
          const fields = {
            headerType,
            contextFlag,
            transportType,
  ...
  it("round-trips LXMF outer wire, destination prefix, and fields", () => {
    fc.assert(
      fc.property(
        ...
        (destinationHash, sourceHash, signature, payload, timestamp, title, content, stamp) => {
          expect(splitLxmfWire(packLxmfWire({ destinationHash, sourceHash, signature, payload }))).toEqual({
            destinationHash,
            sourceHash,
            signature,
            payload
          });
          expect(splitLxmfDestinationPrefixed(packLxmfDestinationPrefixed(destinationHash, payload))).toEqual({
            destinationHash,
            remainder: payload
          });

          const fields = { 1: payload.subarray(0, 32), 7: title.subarray(0, 32) };
          const unpacked = unpackLxmPayload(packLxmPayload(timestamp, title, content, fields, stamp));
    ...
    fc.assert(
      fc.property(
        ...
        (iv, ciphertext, hmac, msgType, sequence, payload) => {
          const token = splitTokenFrame(packTokenFrame({ iv, ciphertext, hmac }));
          expect(token).not.toBeNull();
          expect(token?.iv).toEqual(iv);
          expect(token?.ciphertext).toEqual(ciphertext);
          expect(token?.hmac).toEqual(hmac);
          expect(token?.signedMaterial).toEqual(assembleByteArrays([iv, ciphertext]));

          expect(unpackChannelEnvelope(packChannelEnvelope({ msgType, sequence, payload }))).toEqual({
            msgType,
            sequence,
    ...
    fc.assert(
      fc.property(
        ...
        (flagFields, integer, hash, map, request, resourceHash) => {
          ...
          expect(decodeResourceAdvertisementFlags(encodedFlags)).toEqual(flagFields);
          const fields = {
            t: integer,
            d: integer,
            n: integer,
          ...
          expect(unpackResourceAdvertisement(packResourceAdvertisement(fields))).toEqual(fields);

          const updateBytes = packResourceHashmapUpdate(integer, map);
          expect(unpackResourceHashmapUpdate(updateBytes)).toEqual({ segment: integer, hashmap: map });
          expect(splitResourceHashmapUpdatePacket(packResourceHashmapUpdatePacket(resourceHash, updateBytes))).toEqual({
            resourceHash,
            updateBytes
          });
        }
  ...
  it("round-trips announce payloads with and without ratchets", () => {
    fc.assert(
      fc.property(
        ...
        fc.option(fc.uint8Array({ minLength: 1, maxLength: 256 }), { nil: null }),
        fc.boolean(),
        (publicKey, nameHash, randomHash, ratchet, signature, appData, hasRatchet) => {
          const ratchetPublicKey = hasRatchet ? ratchet : null;
          expect(
            parseAnnouncePayload(
              packAnnouncePayload({ publicKey, nameHash, randomHash, ratchetPublicKey, signature, appData }),
              hasRatchet
            )
          ).toEqual({ publicKey, nameHash, randomHash, ratchetPublicKey, signature, appData });
        }
      ),
      parameters
    );
      ...
      fc.property(
        ...
        (flags, hops, nextHop, body) => {
          ...
          const stripped = stripTransportHeadersBytes(
            wrapTransportPacketBytes({ packedFlags: flags, hops, raw, nextHop })
          );
  ...
  it("decodes masked WebSocket client frames and never throws on arbitrary bytes", () => {
    fc.assert(
      fc.property(bytes(65535), fixedBytes(4), bytes(), (payload, mask, arbitrary) => {
        const frame = maskedClientFrame(payload, mask);
        expect(decodeWsClientFrame(frame)).toEqual({ opcode: 2, payload, consumed: frame.length });
        ...
        expect(() => splitResourceHashmapUpdatePacket(arbitrary)).not.toThrow();
  ...
  it("assembles byte parts and truncates only to the requested prefix", () => {
    fc.assert(
      fc.property(fc.array(bytes(256), { maxLength: 32 }), fc.integer({ min: 0, max: 512 }), (parts, requested) => {
        const assembled = assembleByteArrays(parts);
        expect(assembled).toEqual(Uint8Array.from(parts.flatMap((part) => [...part])));
        const length = Math.min(requested, assembled.length);
        expect(truncateHashBytes(assembled, length)).toEqual(assembled.subarray(0, length));
      }),
      parameters
    );
  });
...
function maskedClientFrame(payload: Uint8Array, mask: Uint8Array): Uint8Array {
  ...
  const masked = Uint8Array.from(payload, (byte, index) => byte ^ mask[index % mask.length]!);
