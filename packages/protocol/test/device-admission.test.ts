...
describe("device stream admission", () => {
  ...
  it("degrades camera on a LoRa-like reticulum path", () => {
    ...
    expect(decision.rung).toBe(
      degradationLadderFor("camera")[decision.rungIndex],
    );
  });

  it("treats raw media targetBps as a bitrate rather than multiplying by frame rate", () => {
    expect(demandBps({ classId: "camera", tierId: "frames", rateHz: 30 })).toBe(
      2_000_000,
    );
    expect(
      demandBps({ classId: "microphone", tierId: "pcm", rateHz: 50 }),
    ).toBe(524_288);
  });
  ...
  it("checks admitted rung demand rather than clamped supply", () => {
    ...
    expect(
      admittedWithinHeadroom(
        { ...decision, admittedDemandBps: 50_001 },
        50_000,
      ),
    ).toBe(false);
  });
  ...
  it("starts lower on metered or low-battery links", () => {
    ...
    const normal = decideStreamAdmission(
      { classId: "microphone", tierId: "pcm" },
      [fat],
    );
    const metered = decideStreamAdmission(
      { classId: "microphone", tierId: "pcm" },
      [{ ...fat, metered: true }],
    );
  ...
  it("keeps admitted supply within host headroom for registry classes", () => {
    ...
    for (const classId of ["camera", "microphone", "location", "motion"]) {
      ...
      expect(
        demandBps({
          classId,
          tierId: classId === "location" ? "coarse" : "derived",
        }),
      ).toBeGreaterThan(0);
    }
  });
