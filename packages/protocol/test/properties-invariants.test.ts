...
describe("protocol invariant properties", () => {
  it("never allows more client requests than the configured window budget", () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.integer({ min: 0, max: CLIENT_RATE_WINDOW_MS - 1 }), { maxLength: 200 }),
        (limit, offsets) => {
          let state = initialClientRateLimitState(limit);
          let allowed = 0;
          for (const at of offsets.sort((left, right) => left - right)) {
            state = stepClientRateLimit(state, { kind: "rate/check", clientKey: "client", at }).state;
      ...
      fc.property(
        ...
        (events) => {
          ...
          for (const event of events) {
            state = stepPathTable(state, { kind: "path/announce", ...event }).state;
            expect([...state.entries.values()].every((entry) => entry.hops <= PATHFINDER_MAX_HOPS)).toBe(true);
          }
  ...
  it("keeps grant evaluation monotone when declared capabilities grow", () => {
    fc.assert(
      fc.property(
        fc.uniqueArray(fc.string({ minLength: 1, maxLength: 32 }), { maxLength: 20 }),
        fc.uniqueArray(fc.string({ minLength: 1, maxLength: 32 }), { maxLength: 20 }),
        (requested, additions) => {
  ...
  it("bounds announce history regardless of input trace length", () => {
    fc.assert(
      fc.property(
        fc.array(fc.double({ min: 0, max: 1_000_000, noNaN: true }), { maxLength: 500 }),
        (times) => {
          ...
          for (const at of times.sort((left, right) => left - right)) {
            ...
            expect(state.table.get("destination")?.timestamps.length ?? 0).toBeLessThanOrEqual(
              MAX_ANNOUNCE_RATE_TIMESTAMPS
            );
          }
        }
