...
describe("peer QR framing", () => {
  ...
  it("reassembles lossless frames in any order and ignores identical duplicates", () => {
    ...
    const duplicate = stepPeerQrAssembly(state, frames[7]!, 1);
    state = duplicate.state;
    expect(stepPeerQrAssembly(state, frames[7]!, 2).state.received).toBe(1);
    let complete: Uint8Array | null = null;
    for (const frame of frames.slice(0, 7).reverse()) {
      const result = stepPeerQrAssembly(state, frame, 3);
      state = result.state;
  ...
  it("rejects corruption, mixed sessions, conflicting duplicates, and expiry", () => {
    const frames = framePeerQrPayload(session, payload, 100);
    const corrupted = frames[0]!.slice();
    corrupted[20] = (corrupted[20] ?? 0) ^ 1;
    expect(() => decodePeerQrFrame(corrupted)).toThrow(/checksum/);
    const started = stepPeerQrAssembly(
      initialPeerQrAssemblyState(10),
      frames[0]!,
      0,
    ).state;
    const other = framePeerQrPayload(new Uint8Array(16).fill(8), payload, 100);
    expect(() => stepPeerQrAssembly(started, other[1]!, 1)).toThrow(
      /different sessions/,
    );
    const conflicting = framePeerQrPayload(
      session,
      new Uint8Array(payload.length).fill(9),
      100,
    );
    expect(() => stepPeerQrAssembly(started, conflicting[0]!, 1)).toThrow(
      /different payload/,
    );
    expect(() => stepPeerQrAssembly(started, frames[1]!, 10)).toThrow(
      /expired/,
    );
  });

  it("enforces frame and assembled-size budgets", () => {
    expect(() => framePeerQrPayload(session, new Uint8Array(16_385))).toThrow(
      /budget/,
    );
