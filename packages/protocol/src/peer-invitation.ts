...
function assertBytes(value: unknown, name: string, min: number, max: number): asserts value is Uint8Array {
  if (!(value instanceof Uint8Array) || value.length < min || value.length > max) {
    throw new PeerInvitationError("MALFORMED", `${name} must be ${min}..${max} bytes`);
  }
}

function assertText(value: unknown, name: string, min: number, max: number): asserts value is string {
  if (typeof value !== "string" || value.length < min || value.length > max || [...value].some((character) => { const code = character.codePointAt(0) ?? 0; return code < 32 || code === 127; })) {
...
export function validatePeerInvitation(invitation: PeerInvitation, now?: number): void {
  if (invitation.version !== PEER_INVITATION_VERSION) throw new PeerInvitationError("MALFORMED", "unsupported invitation version");
  assertBytes(invitation.sessionId, "session id", 16, 32);
  assertText(invitation.service, "service", 1, MAX_PEER_SERVICE_LENGTH);
  if (invitation.role !== "offer" && invitation.role !== "answer") throw new PeerInvitationError("MALFORMED", "invalid role");
  assertBytes(invitation.peerEphemeralKey, "ephemeral key", 32, 65);
  if (invitation.identityProof !== undefined) assertBytes(invitation.identityProof, "identity proof", 16, 512);
  ...
  if (now !== undefined && (now < invitation.issuedAt - 30_000 || now >= invitation.expiresAt)) throw new PeerInvitationError("EXPIRED", "invitation is expired or not yet valid");
  if (!Array.isArray(invitation.capabilities) || invitation.capabilities.length > MAX_PEER_CAPABILITIES) throw new PeerInvitationError("MALFORMED", "too many capabilities");
  for (const capability of invitation.capabilities) assertText(capability, "capability", 1, 64);
  assertBytes(invitation.signature, "signature", 32, 512);
}

interface CborMap { readonly [key: string]: CborValue; }
type CborValue = number | string | Uint8Array | ReadonlyArray<CborValue> | CborMap;
function concat(parts: ReadonlyArray<Uint8Array>): Uint8Array { const size = parts.reduce((n, p) => n + p.length, 0); const out = new Uint8Array(size); let at = 0; for (const p of parts) { out.set(p, at); at += p.length; } return out; }
function head(major: number, value: number): Uint8Array {
  if (!Number.isSafeInteger(value) || value < 0) throw new PeerInvitationError("MALFORMED", "invalid CBOR integer");
  if (value < 24) return new Uint8Array([(major << 5) | value]);
  if (value <= 0xff) return new Uint8Array([(major << 5) | 24, value]);
  if (value <= 0xffff) return new Uint8Array([(major << 5) | 25, value >>> 8, value & 255]);
  if (value <= 0xffff_ffff) { const out = new Uint8Array(5); out[0] = (major << 5) | 26; new DataView(out.buffer).setUint32(1, value, false); return out; }
  const out = new Uint8Array(9); out[0] = (major << 5) | 27; new DataView(out.buffer).setBigUint64(1, BigInt(value), false); return out;
}
function cbor(value: CborValue): Uint8Array {
  if (typeof value === "number") return head(0, value);
  if (typeof value === "string") { const bytes = utf8Encode(value); return concat([head(3, bytes.length), bytes]); }
  ...
  const entries = Object.entries(value).map(([key, item]) => [cbor(key), item] as const).sort(([a], [b]) => a.length - b.length || compare(a, b));
  return concat([head(5, entries.length), ...entries.flatMap(([key, item]) => [key, cbor(item)])]);
}
function compare(a: Uint8Array, b: Uint8Array): number { for (let i = 0; i < Math.min(a.length, b.length); i++) { const d = (a[i] ?? 0) - (b[i] ?? 0); if (d !== 0) return d; } return a.length - b.length; }

function fields(invitation: PeerInvitation, signature: boolean): Record<string, CborValue> {
  const value: Record<string, CborValue> = {
    v: invitation.version, sid: invitation.sessionId, svc: invitation.service, role: invitation.role,
    key: invitation.peerEphemeralKey, cand: invitation.candidates.map((x) => [x.kind, x.value]),
    display: invitation.display, iat: invitation.issuedAt, exp: invitation.expiresAt,
    caps: invitation.capabilities
  };
  if (invitation.identityProof !== undefined) value.id = invitation.identityProof;
  if (signature) value.sig = invitation.signature;
  return value;
}

export function peerInvitationSigningBytes(invitation: PeerInvitation): Uint8Array { validatePeerInvitation(invitation); return cbor(fields(invitation, false)); }
export function encodePeerInvitation(invitation: PeerInvitation): Uint8Array { validatePeerInvitation(invitation); const out = cbor(fields(invitation, true)); if (out.length > MAX_PEER_INVITATION_BYTES) throw new PeerInvitationError("OVERSIZED", "invitation exceeds size budget"); return out; }

class Reader {
  at = 0;
  constructor(readonly bytes: Uint8Array) {}
  take(n: number): Uint8Array { if (n < 0 || this.at + n > this.bytes.length) throw new PeerInvitationError("MALFORMED", "truncated CBOR"); const out = this.bytes.subarray(this.at, this.at + n); this.at += n; return out; }
  length(add: number): number { if (add < 24) return add; if (add === 24) return this.take(1)[0] ?? 0; if (add === 25) { const x = this.take(2); return ((x[0] ?? 0) << 8) | (x[1] ?? 0); } if (add === 26) return new DataView(this.take(4).buffer, this.bytes.byteOffset + this.at - 4, 4).getUint32(0, false); if (add === 27) { const x = new DataView(this.take(8).buffer, this.bytes.byteOffset + this.at - 8, 8).getBigUint64(0, false); const n = Number(x); if (!Number.isSafeInteger(n)) throw new PeerInvitationError("MALFORMED", "integer too large"); return n; } throw new PeerInvitationError("MALFORMED", "indefinite CBOR forbidden"); }
  read(depth = 0): CborValue { if (depth > 8) throw new PeerInvitationError("MALFORMED", "CBOR nesting limit"); const first = this.take(1)[0] ?? 0; const major = first >>> 5; const len = this.length(first & 31); if (major === 0) return len; if (major === 2) return this.take(len); if (major === 3) return utf8Decode(this.take(len)); if (major === 4) return Array.from({ length: len }, () => this.read(depth + 1)); if (major === 5) { const out: Record<string, CborValue> = {}; for (let i = 0; i < len; i++) { const key = this.read(depth + 1); if (typeof key !== "string" || key in out) throw new PeerInvitationError("MALFORMED", "invalid CBOR map"); out[key] = this.read(depth + 1); } return out; } throw new PeerInvitationError("MALFORMED", "unsupported CBOR type"); }
}
function map(value: CborValue): CborMap { if (typeof value !== "object" || value === null || value instanceof Uint8Array || Array.isArray(value)) throw new PeerInvitationError("MALFORMED", "expected map"); return value as CborMap; }
function array(value: CborValue | undefined): ReadonlyArray<CborValue> { if (!Array.isArray(value)) throw new PeerInvitationError("MALFORMED", "expected array"); return value; }

export function decodePeerInvitation(bytes: Uint8Array, now?: number): PeerInvitation {
  if (bytes.length > MAX_PEER_INVITATION_BYTES) throw new PeerInvitationError("OVERSIZED", "invitation exceeds size budget");
  const reader = new Reader(bytes); const x = map(reader.read()); if (reader.at !== bytes.length) throw new PeerInvitationError("MALFORMED", "trailing CBOR data");
  const candidates = array(x.cand).map((entry) => { const pair = array(entry); return { kind: pair[0] as PeerCandidateKind, value: pair[1] as Uint8Array }; });
  const invitation: PeerInvitation = { version: x.v as 1, sessionId: x.sid as Uint8Array, service: x.svc as string, role: x.role as PeerInvitationRole, peerEphemeralKey: x.key as Uint8Array, candidates, display: x.display as string, issuedAt: x.iat as number, expiresAt: x.exp as number, capabilities: array(x.caps) as string[], signature: x.sig as Uint8Array, ...(x.id === undefined ? {} : { identityProof: x.id as Uint8Array }) };
  validatePeerInvitation(invitation, now);
  if (compare(encodePeerInvitation(invitation), bytes) !== 0) throw new PeerInvitationError("MALFORMED", "invitation is not canonical CBOR");
  return invitation;
}
