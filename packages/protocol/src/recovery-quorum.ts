...
export type RecoveryPhase =
  "idle" | "collecting" | "recovered" | "rejected" | "expired";
...
export function initialRecoveryQuorumState(
  threshold: number,
): RecoveryQuorumState {
  if (!Number.isSafeInteger(threshold) || threshold < 1)
    throw new Error("recovery threshold must be positive");
  return { phase: "idle", threshold, shares: [], recoveredWith: [] };
}

const start: EventClass<RecoveryQuorumEvent> = {
  name: "start",
  matches: (event) => event.kind === "recovery/start",
};
const share: EventClass<RecoveryQuorumEvent> = {
  name: "share",
  matches: (event) => event.kind === "recovery/share",
};
const authorize: EventClass<RecoveryQuorumEvent> = {
  name: "threshold-authorize",
  matches: (event) => event.kind === "recovery/authorize",
};
const reject: EventClass<RecoveryQuorumEvent> = {
  name: "reject",
  matches: (event) => event.kind === "recovery/reject",
};
const ttl: EventClass<RecoveryQuorumEvent> = {
  name: "ttl",
  matches: (event) => event.kind === "recovery/ttl",
};

export const recoveryQuorumMachine: Machine<
  RecoveryQuorumState,
  RecoveryQuorumEvent
> = {
  ...
  table: [
    { from: "idle", on: start, to: "collecting" },
    {
      ...
      to: "collecting",
      guard: (_state, event) =>
        event.kind === "recovery/share" && event.guardian.length > 0,
      reduce: (state, event) =>
        event.kind === "recovery/share"
          ? {
              ...state,
              shares: [...new Set([...state.shares, event.guardian])].sort(),
            }
          : state,
    },
    {
...
export function recoveryQuorumSafetyViolation(
  state: RecoveryQuorumState,
): string | null {
  return state.phase === "recovered" &&
    state.recoveredWith.length < state.threshold
