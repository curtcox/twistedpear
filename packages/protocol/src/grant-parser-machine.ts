...
export type GrantParserPhase =
  ...
  | "expect-appId-colon"
  | "expect-appId"
  | "expect-publisher-comma"
  | "expect-publisher-key"
  | "expect-publisher-colon"
  | "expect-publisher"
  | "expect-granted-comma"
  | "expect-granted-key"
  | "expect-granted-colon"
  | "expect-array-open"
  | "expect-capability-or-end"
  | "expect-capability-comma-or-end"
  | "expect-capability"
  | "expect-updated-comma"
  | "expect-updated-key"
  | "expect-updated-colon"
  | "expect-updated"
  | "expect-close"
  | "expect-eof"
  | "accept";
...
const token = <K extends GrantParserToken["kind"]>(
  kind: K,
): EventClass<GrantParserToken> => ({
  name: kind,
...
const stringIs =
  (expected: string) =>
  (_state: GrantParserState, event: GrantParserToken): boolean =>
    event.kind === "string" && event.value === expected;
const captureString =
  (field: "appId" | "publisherPublicKey") =>
  (state: GrantParserState, event: GrantParserToken): GrantParserState =>
    event.kind === "string" ? { ...state, [field]: event.value } : state;
const addCapability = (
  state: GrantParserState,
  event: GrantParserToken,
): GrantParserState =>
  event.kind === "string"
    ? { ...state, granted: [...state.granted, event.value] }
    : state;
...
export const grantParserMachine: Machine<GrantParserState, GrantParserToken> = {
  ...
  table: [
    ...
    {
      from: "expect-appId-key",
      on: t.string,
      to: "expect-appId-colon",
      guard: stringIs("appId"),
    },
    { from: "expect-appId-colon", on: t.colon, to: "expect-appId" },
    {
      from: "expect-appId",
      on: t.string,
      to: "expect-publisher-comma",
      reduce: captureString("appId"),
    },
    { from: "expect-publisher-comma", on: t.comma, to: "expect-publisher-key" },
    {
      from: "expect-publisher-key",
      on: t.string,
      to: "expect-publisher-colon",
      guard: stringIs("publisherPublicKey"),
    },
    { from: "expect-publisher-colon", on: t.colon, to: "expect-publisher" },
    {
      from: "expect-publisher",
      on: t.string,
      to: "expect-granted-comma",
      reduce: captureString("publisherPublicKey"),
    },
    { from: "expect-granted-comma", on: t.comma, to: "expect-granted-key" },
    {
      from: "expect-granted-key",
      on: t.string,
      to: "expect-granted-colon",
      guard: stringIs("granted"),
    },
    { from: "expect-granted-colon", on: t.colon, to: "expect-array-open" },
    {
      from: "expect-array-open",
      on: t.arrayOpen,
      to: "expect-capability-or-end",
    },
    {
      from: "expect-capability-or-end",
      on: t.arrayClose,
      to: "expect-updated-comma",
    },
    {
      from: "expect-capability-or-end",
      on: t.string,
      to: "expect-capability-comma-or-end",
      reduce: addCapability,
    },
    {
      from: "expect-capability-comma-or-end",
      on: t.arrayClose,
      to: "expect-updated-comma",
    },
    {
      from: "expect-capability-comma-or-end",
      on: t.comma,
      to: "expect-capability",
    },
    {
      from: "expect-capability",
      on: t.string,
      to: "expect-capability-comma-or-end",
      reduce: addCapability,
    },
    ...
    {
      from: "expect-updated-key",
      on: t.string,
      to: "expect-updated-colon",
      guard: stringIs("updatedAt"),
    },
