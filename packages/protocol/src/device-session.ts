...
export type DeviceSessionEvent =
  | { readonly kind: "device/open"; readonly at: number; readonly ttlMs: number }
  | { readonly kind: "device/degrade"; readonly at: number; readonly rung: number }
  | { readonly kind: "device/restore"; readonly at: number; readonly rung: number }
...
export const deviceSessionMachine: Machine<DeviceSessionState, DeviceSessionEvent> = {
  ...
  withState: (state, phase) => ({ ...state, phase: phase as DeviceSessionPhase }),
  table: [
    {
      from: "requested",
      on: open,
    ...
    },
    {
      ...
      to: "degraded",
      reduce: (state, event) =>
        event.kind === "device/degrade" ? { ...state, degradationRung: event.rung } : state
    },
    {
      from: "degraded",
      on: restore,
      to: "active",
      reduce: (state, event) =>
        event.kind === "device/restore" ? { ...state, degradationRung: event.rung } : state
    },
    {
      from: "degraded",
      on: degrade,
    ...
    },
    {
      ...
      to: "closed",
      reduce: (state, event) =>
        event.kind === "device/close" ? { ...state, closedAt: event.at } : state
    },
    {
      from: "degraded",
      on: close,
    ...
    },
    {
      ...
      to: "expired",
      guard: (state, event) =>
        event.kind === "device/ttl" && state.expiresAt !== null && event.at >= state.expiresAt
    },
    {
      from: "degraded",
      on: ttlExpired,
    ...
    },
    {
      ...
      to: "revoked",
      reduce: (state, event) =>
        event.kind === "device/revoke" ? { ...state, revokedAt: event.at } : state
    },
    {
      from: "degraded",
      on: revoke,
