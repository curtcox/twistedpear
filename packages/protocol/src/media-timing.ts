...
export function updateClockOffset(
  ...
  return {
    offsetUs: previous === null ? offsetUs : previous.offsetUs + alpha * (offsetUs - previous.offsetUs),
    rttUs: previous === null ? rttUs : previous.rttUs + alpha * (rttUs - previous.rttUs),
...
export function initialJitterBuffer(targetDelayUs = 120_000): JitterBufferState {
  return { targetDelayUs: Math.max(0, targetDelayUs), frames: [], droppedLate: 0 };
}
...
export function pushJitterFrame(
  ...
  const presentationAt = timed.frame.captureAtUs - clockOffsetUs + state.targetDelayUs;
  if (presentationAt < nowUs - state.targetDelayUs) {
    return { ...state, droppedLate: state.droppedLate + 1 };
  }
  const frames = [...state.frames, timed].sort((left, right) =>
    left.frame.captureAtUs - right.frame.captureAtUs || left.frame.sequence - right.frame.sequence
  );
  return { ...state, frames };
}
...
export function drainJitterBuffer(
  ...
  nowUs: number,
  clockOffsetUs = 0
): { readonly state: JitterBufferState; readonly ready: ReadonlyArray<TimedMediaFrame> } {
