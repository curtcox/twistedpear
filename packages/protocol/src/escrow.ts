...
export function initialEscrowState(quorum: number): EscrowState {
  if (!Number.isSafeInteger(quorum) || quorum < 1)
    throw new Error("escrow quorum must be positive");
  return {
    phase: "pending",
    amount: 0,
    quorum,
...
const deposit: EventClass<EscrowEvent> = {
  name: "deposit",
  matches: (event) => event.kind === "escrow/deposit",
};
const requestRelease: EventClass<EscrowEvent> = {
  name: "request-release",
  matches: (event) => event.kind === "escrow/request-release",
};
const authorize: EventClass<EscrowEvent> = {
  name: "quorum-authorize",
  matches: (event) => event.kind === "escrow/authorize",
};
const refund: EventClass<EscrowEvent> = {
  name: "refund",
  matches: (event) => event.kind === "escrow/refund",
};
const ttl: EventClass<EscrowEvent> = {
  name: "ttl",
  matches: (event) => event.kind === "escrow/ttl",
};
  ...
  table: [
    {
      ...
      to: "funded",
      guard: (_state, event) =>
        event.kind === "escrow/deposit" && event.amount > 0,
      reduce: (state, event) =>
        event.kind === "escrow/deposit"
    ...
    { from: "funded", on: requestRelease, to: "release-requested" },
    {
      ...
      to: "released",
      guard: (state, event) =>
        event.kind === "escrow/authorize" &&
        new Set(event.authorizers).size >= state.quorum,
      reduce: (state, event) =>
        event.kind === "escrow/authorize"
          ? {
              ...state,
              authorizers: [...new Set(event.authorizers)].sort(),
...
export function escrowSafetyViolation(state: EscrowState): string | null {
  if (state.phase === "released" && state.authorizers.length < state.quorum)
    return "escrow released without quorum";
  if (state.releasedAmount > state.amount)
    return "escrow released more than funded";
  return null;
}
