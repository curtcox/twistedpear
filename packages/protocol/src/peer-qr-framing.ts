...
export function encodePeerQrFrame(frame: PeerQrFrame): Uint8Array {
  ...
  view.setUint32(
    ...
    peerQrCrc32(output.subarray(0, headerLength + frame.payload.length)),
    false,
  );
  return output;
}
...
export function decodePeerQrFrame(bytes: Uint8Array): PeerQrFrame {
  if (
    bytes.length < 33 ||
    !sameBytes(bytes.subarray(0, 4), MAGIC) ||
    bytes[4] !== PEER_QR_FRAME_VERSION
  )
    throw new PeerQrFrameError("MALFORMED", "invalid QR frame header");
  const sessionLength = bytes[5] ?? 0;
  if (sessionLength < 16 || sessionLength > 32)
    throw new PeerQrFrameError("MALFORMED", "invalid QR session id length");
  ...
  if (bytes.length !== headerLength + payloadLength + 4)
    throw new PeerQrFrameError("MALFORMED", "QR frame length mismatch");
  const expected = view.getUint32(bytes.length - 4, false);
  if (peerQrCrc32(bytes.subarray(0, -4)) !== expected)
    throw new PeerQrFrameError("CRC_MISMATCH", "QR frame checksum mismatch");
  const frame = {
    sessionId: bytes.slice(12, headerLength),
    sequence,
    total,
    payload: bytes.slice(headerLength, -4),
  };
  validateFrame(frame);
  return frame;
}

export function framePeerQrPayload(
  sessionId: Uint8Array,
  payload: Uint8Array,
  framePayloadBytes = MAX_PEER_QR_FRAME_PAYLOAD_BYTES,
): ReadonlyArray<Uint8Array> {
  if (payload.length < 1 || payload.length > MAX_PEER_QR_ASSEMBLED_BYTES)
    throw new PeerQrFrameError(
      "OVERSIZED",
      "QR payload exceeds assembly budget",
    );
  if (
    !Number.isInteger(framePayloadBytes) ||
    framePayloadBytes < 1 ||
    framePayloadBytes > MAX_PEER_QR_FRAME_PAYLOAD_BYTES
  )
    throw new PeerQrFrameError("MALFORMED", "invalid QR chunk size");
  const total = Math.ceil(payload.length / framePayloadBytes);
  if (total > MAX_PEER_QR_FRAMES)
    throw new PeerQrFrameError(
      "OVERSIZED",
      "QR payload requires too many frames",
    );
  return Array.from({ length: total }, (_, sequence) =>
    encodePeerQrFrame({
      ...
      payload: payload.slice(
        sequence * framePayloadBytes,
        Math.min(payload.length, (sequence + 1) * framePayloadBytes),
      ),
    }),
  );
}
...
export function initialPeerQrAssemblyState(
  expiresAt: number,
): PeerQrAssemblyState {
...
function validateAssemblyCompatibility(
  ...
  if (state.sessionId !== null && !sameBytes(state.sessionId, frame.sessionId))
    throw new PeerQrFrameError(
      "MIXED_SESSION",
      "QR frames belong to different sessions",
    );
...
function assemblePayload(chunks: ReadonlyArray<Uint8Array | null>): Uint8Array {
  ...
  if (size > MAX_PEER_QR_ASSEMBLED_BYTES)
    throw new PeerQrFrameError(
      "OVERSIZED",
      "assembled QR payload exceeds budget",
    );
  const payload = new Uint8Array(size);
  let offset = 0;
  for (const chunk of chunks) {
    if (chunk === null)
      throw new PeerQrFrameError("MALFORMED", "QR assembly is incomplete");
