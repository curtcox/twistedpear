export type BandwidthBucket =
  "none" | "derived" | "narrowband" | "audio" | "sd-video" | "hd-video";
...
export interface PeerMediaReadiness {
  ...
  readonly constrained: ReadonlyArray<
    "metered" | "low-battery" | "thermal" | "foreground-only"
  >;
  readonly consentPosture: "open" | "ask" | "closed";
  readonly expiresAt: number;
}

export type MediaReadinessPhase =
  "unknown" | "requested" | "ready" | "unreachable";
...
export type MediaCapability =
  "hd-video" | "sd-video" | "audio" | "narrowband" | "derived" | "unreachable";
...
export function minimumBandwidthBucket(
  left: BandwidthBucket,
  right: BandwidthBucket,
): BandwidthBucket {
  return (
    BUCKET_ORDER[
      Math.min(BUCKET_ORDER.indexOf(left), BUCKET_ORDER.indexOf(right))
    ] ?? "none"
  );
}
...
function normalizeClasses(
  classes: ReadonlyArray<MediaReadinessClass>,
): ReadonlyArray<MediaReadinessClass> {
  return [...classes]
