...
const cases: ReadonlyArray<SurfaceCase> = [
  ...
  {
    label: "storage.sync.open",
    call: () => storage.sync.open("topic"),
  ...
  {
    label: "storage.sync.append",
    call: () => storage.sync.append("topic", { n: 1 }, { key: "k" }),
    namespace: "storage.sync",
    method: "append",
    capability: "storage:sync",
    payload: { topic: "topic", payload: { n: 1 }, key: "k" },
  },
  {
    label: "storage.sync.view",
    call: () => storage.sync.view("topic"),
    namespace: "storage.sync",
    method: "view",
    capability: "storage:sync",
    payload: { topic: "topic" },
