...
| `miniapp-runtime` | Sandbox broker and capability enforcement | `peer-discovery`, `protocol` | `src/index.ts` | `npm test -- packages/miniapp-runtime/test` | changing the sandbox/broker; keep capability/budget checks intact |
...
| `bridge-hyper` | Hyperswarm/Hyperdrive distribution bridge | `app-registry`, `reticulum-interfaces`, `reticulum-ts` | `src/index.ts` | `npm test -- packages/bridge-hyper/test` | changing Hyper-based distribution/seeding |
...
| `cli` | Command-line host and tooling | `app-registry`, `bridge-freenet`, `bridge-hyper`, `host-core`, `miniapp-runtime`, `reticulum-ts`, `cas-256t` | `src/index.ts` | `npm test -- packages/cli/test` | changing CLI commands |
| `host-core` | Runtime-neutral host orchestration | `app-registry`, `bridge-freenet`, `bridge-hyper`, `cas-256t`, `lxmf-ts`, `peer-discovery`, `reticulum-interfaces`, `reticulum-ts` | `src/index.ts` | `npm test -- packages/host-core/test` | changing how a host wires the stack together |
