...
export class ContainmentTracker {
  ...
  constructor(
    readonly transport: TransportClassName | "unclassified" = "unclassified",
  ) {}

  revoked(at: number, affectedNodes: readonly NodeId[]): number {
    this.revocations.push({
      at,
      pending: new Set(affectedNodes),
      completedAt: affectedNodes.length === 0 ? at : null,
    });
  ...
  nodeStoppedUsingGrant(revocation: number, node: NodeId, at: number): void {
    ...
    if (item === undefined || at < item.at)
      throw new Error("invalid revocation observation");
  ...
  damage(kill: number, units = 1): void {
    ...
    if (item === undefined || item.severedAt !== null)
      throw new Error("damage outside active kill window");
    item.damage += Math.max(0, units);
  }

  severed(kill: number, at: number): void {
    const item = this.kills[kill];
    if (item === undefined || at < item.requestedAt)
      throw new Error("invalid sever observation");
    item.severedAt = at;
  }
  ...
  snapshot(): ContainmentMetrics {
    ...
    const attributable = this.egress.map(
      (item) =>
    ...
    return {
      ...
      revocationPropagationMs:
        propagation.length === 0 ? null : Math.max(...propagation),
      egressAttributability:
        attributable.length === 0
          ? null
          : attributable.reduce((sum, score) => sum + score, 0) /
            (attributable.length * 3),
      networkKillLatencyMs:
        killLatency.length === 0 ? null : Math.max(...killLatency),
...
export function containmentRegressions(
  ...
  for (const expected of baseline) {
    const observed = actual.find(
      (item) => item.transport === expected.transport,
    );
    if (observed === undefined) {
      regressions.push(`${expected.transport}: missing containment metrics`);
      continue;
    }
    if (
      observed.revocationPropagationMs === null ||
      observed.revocationPropagationMs > expected.revocationPropagationMsMax
    )
      regressions.push(
        `${expected.transport}: revocation propagation ${observed.revocationPropagationMs} exceeds ${expected.revocationPropagationMsMax}`,
      );
    if (
      observed.egressAttributability === null ||
      observed.egressAttributability < expected.egressAttributabilityMin
    )
      regressions.push(
        `${expected.transport}: attributability ${observed.egressAttributability} is below ${expected.egressAttributabilityMin}`,
      );
    if (
      observed.networkKillLatencyMs === null ||
      observed.networkKillLatencyMs > expected.networkKillLatencyMsMax
    )
      regressions.push(
        `${expected.transport}: kill latency ${observed.networkKillLatencyMs} exceeds ${expected.networkKillLatencyMsMax}`,
      );
  }
  return regressions;
}
...
export function summarizeContainment(
  metrics: readonly ContainmentMetrics[],
): readonly ContainmentSummary[] {
  ...
  return [...transports].sort().map((transport) => {
    ...
    return {
      ...
      revocationPropagationMs:
        revocation.length === 0 ? null : Math.max(...revocation),
      egressAttributability:
        attribution.length === 0
          ? null
          : attribution.reduce((sum, value) => sum + value, 0) /
            attribution.length,
