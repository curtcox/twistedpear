...
export function injectCanaries(
  ...
  while (
    selected.length < Math.min(available.length, Math.max(0, Math.floor(count)))
  ) {
...
export function estimateCompleteness(options: {
  ...
  const recaptured = [...canaries].filter(
    (id) => options.firstCapture.has(id) || options.secondCapture.has(id),
  ).length;
  const [low, high] = wilson(recaptured, canaries.size);
  const overlap = [...options.firstCapture].filter((id) =>
    options.secondCapture.has(id),
  ).length;
  const population =
    options.firstCapture.size === 0 || options.secondCapture.size === 0
      ? null
      : ((options.firstCapture.size + 1) * (options.secondCapture.size + 1)) /
          (overlap + 1) -
        1;
  return {
    floor: low,
