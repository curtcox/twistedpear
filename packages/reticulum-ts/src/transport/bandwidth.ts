...
export class BandwidthLimiter implements ByteRateLimiter {
  ...
  private readonly reservations = new Map<
    string,
    { class: BandwidthReservationClass; bytesPerSecond: number }
  >();
  ...
  async consume(bytes: number): Promise<void> {
    if (!Number.isSafeInteger(bytes) || bytes < 0) {
      throw new Error(
        "Bandwidth byte count must be a non-negative safe integer",
      );
    }
    if (bytes === 0) return;
   ...
   */
  reserve(
    reservationClass: BandwidthReservationClass,
    bytesPerSecond: number,
  ): BandwidthReservation | null {
    if (!Number.isSafeInteger(bytesPerSecond) || bytesPerSecond <= 0) {
    ...
    const totalCommitted = [...this.reservations.values()].reduce(
      (sum, entry) => sum + entry.bytesPerSecond,
      0,
    );
    if (
      classCommitted + bytesPerSecond > classCap ||
      totalCommitted + bytesPerSecond > this.bytesPerSecond
    ) {
      return null;
    }
      ...
      consume: (bytes) => {
        if (released)
          return Promise.reject(
            new Error("Bandwidth reservation has been released"),
          );
  ...
  reservationSnapshot(): ReadonlyArray<{
    ...
    return [...this.reservations].map(([id, reservation]) => ({
      id,
      ...reservation,
    }));
  }
