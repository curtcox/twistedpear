/**
 * Bare worklet entrypoint for reticulum-ts.
 * Excludes the Node WebSocket gateway server (`WebSocketServerInterface`), which
 * depends on `node:http` and is not packable for react-native-bare-kit.
 */
