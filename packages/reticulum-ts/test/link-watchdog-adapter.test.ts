...
describe("link watchdog adapter", () => {
  it("matches legacy keepalive formula via use-keepalive actions", () => {
    for (const rtt of [0.01, 0.1, 0.5, 1.0, 1.75, 2.0, 10.0]) {
      ...
      expect(linkKeepaliveFromActions(stepped.actions)).toBe(computeKeepalive(rtt));
      expect(linkKeepaliveFromActions(stepped.actions)! * LINK_STALE_FACTOR).toBe(
        legacy * LINK_STALE_FACTOR
      );
    }
  });
});
