...
export async function startSeederRole(
  options: SeederRoleOptions,
): Promise<SeederRoleSession> {
  ...
  const syncDrives = async (next: SeederState) => {
    ...
    for (const drive of next.drives) {
      if (
        state.drives.some((existing) => existing.driveKey === drive.driveKey)
      ) {
        continue;
      }
  ...
  const usedBytes = () =>
    state.drives.reduce((total, drive) => {
      return (
        total +
        Object.values(drive.versions).reduce(
          (driveTotal, version) => driveTotal + version.size,
          0,
        )
      );
    }, 0);
