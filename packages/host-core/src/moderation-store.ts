...
export type LocalReportReason =
  "spam" | "harassment" | "impersonation" | "unsafe-content" | "other";
...
const EMPTY_SNAPSHOT: ModerationSnapshot = {
  version: 1,
  blocked: [],
  muted: [],
  reports: [],
...
export class FileModerationStore {
  ...
  block(sourceHash: string, label: string | null = null): void {
    ...
    this.snapshot = {
      ...
      muted: this.snapshot.muted.filter(
        (entry) => entry.sourceHash !== normalized,
      ),
    };
    this.persist();
  }
  ...
  unblock(sourceHash: string): void {
    this.snapshot = {
      ...this.snapshot,
      blocked: this.remove(this.snapshot.blocked, sourceHash),
    };
    this.persist();
  }
  ...
  mute(sourceHash: string, label: string | null = null): void {
    ...
    this.snapshot = {
      ...this.snapshot,
      muted: this.upsert(this.snapshot.muted, normalized, label),
    };
    this.persist();
  }
  ...
  unmute(sourceHash: string): void {
    this.snapshot = {
      ...this.snapshot,
      muted: this.remove(this.snapshot.muted, sourceHash),
    };
    this.persist();
  }
  ...
  report(input: {
    ...
    this.snapshot = {
      ...this.snapshot,
      reports: [...this.snapshot.reports, record],
    };
    this.persist();
    return record;
  }
  ...
  private upsert(
    entries: ReadonlyArray<ModerationEntry>,
    sourceHash: string,
    label: string | null,
  ): ReadonlyArray<ModerationEntry> {
