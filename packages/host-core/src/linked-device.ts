...
export const LINKED_DEVICE_MAGIC = new Uint8Array([
...
function decodeUint64(bytes: Uint8Array): number {
  ...
  if (!Number.isSafeInteger(value))
    throw new Error("linked-device timestamp is out of range");
  return value;
}

function certificatePayload(
  certificate: Omit<LinkedDeviceCertificate, "signature">,
): Uint8Array {
  ...
  const deviceId = hexBytes(
    certificate.deviceId,
    LINKED_DEVICE_ID_BYTES,
    "device id",
  );
  const devicePublicKey = hexBytes(
  ...
  if (label.length === 0 || label.length > LINKED_DEVICE_MAX_LABEL_BYTES) {
    throw new Error(
      `Device label must be 1-${LINKED_DEVICE_MAX_LABEL_BYTES} UTF-8 bytes`,
    );
  }
  return concat(
    LINKED_DEVICE_MAGIC,
...
export function deriveLinkedDeviceIdentity(
  ...
  try {
    ...
    derived.fill(0);
    if (identity === null)
      throw new Error("Could not derive linked device identity");
    return identity;
  } finally {
    privateKey.fill(0);
  }
...
export function verifyLinkedDeviceCertificate(
  provider: CryptoProvider,
  certificate: LinkedDeviceCertificate,
): boolean {
  try {
...
export function encodeLinkedDeviceCertificate(
  certificate: LinkedDeviceCertificate,
): Uint8Array {
  const bytes = concat(
  ...
  if (bytes.length > LINKED_DEVICE_MAX_CERTIFICATE_BYTES)
    throw new Error("Linked-device certificate exceeds announce budget");
  return bytes;
}

export function decodeLinkedDeviceCertificate(
  bytes: Uint8Array,
): LinkedDeviceCertificate {
  if (bytes.length < LINKED_DEVICE_MAGIC.length + 64 + 16 + 64 + 8 + 1 + 64)
    throw new Error("Linked-device certificate is too short");
  if (
    !equalBytes(
      bytes.subarray(0, LINKED_DEVICE_MAGIC.length),
      LINKED_DEVICE_MAGIC,
    )
  )
    throw new Error("Linked-device certificate magic mismatch");
  let offset = LINKED_DEVICE_MAGIC.length;
  const accountPublicKey = bytesToHex(bytes.subarray(offset, offset + 64));
  offset += 64;
  const deviceId = bytesToHex(bytes.subarray(offset, offset + 16));
  offset += 16;
  const devicePublicKey = bytesToHex(bytes.subarray(offset, offset + 64));
  offset += 64;
  ...
  if (
    labelLength === 0 ||
    labelLength > LINKED_DEVICE_MAX_LABEL_BYTES ||
...
export function linkedDeviceAnnounceAspects(
  provider: CryptoProvider,
