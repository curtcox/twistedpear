...
import { createObserveRing, ringToRecordedHistory } from "../src/observe-ring.js";

describe("observe ring", () => {
  it("bounds capacity and snapshots SPEC-EVENTS intents", () => {
    const ring = createObserveRing(2);
    ring.push({ kind: "observe/drop", stage: "ingress-dispatch", reason: "ignored" });
    ring.push({
      kind: "observe/drop",
    ...
    expect(snap[0]?.intent).toEqual(
      expect.objectContaining({ stage: "announce-rate-limit", reason: "rate_limited" })
    );
    expect(snap[1]?.intent).toEqual(
      expect.objectContaining({ stage: "announce-local-echo", reason: "local_echo" })
    );
  ...
  it("notifies subscribers of new entries", () => {
    ...
    ring.push({ kind: "observe/drop", stage: "path-entry", reason: "path_not_added" });
