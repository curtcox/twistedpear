...
describe("FileMultipartCheckpointStore", () => {
  it("persists resumable chunks atomically with private permissions", () => {
    const path = join(
      mkdtempSync(join(tmpdir(), "tp-multipart-")),
      "checkpoints.json",
    );
    const store = new FileMultipartCheckpointStore(path);
    const checkpoint = {
      transferId: "01".repeat(16),
      sourceHash: "02".repeat(16),
      destinationHash: "03".repeat(16),
      totalBytes: 3,
      chunkCount: 1,
    ...
    expect(
      new FileMultipartCheckpointStore(path).load(checkpoint.transferId),
    ).toEqual(checkpoint);
