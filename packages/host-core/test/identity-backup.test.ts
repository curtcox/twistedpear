...
describe("identity backup", () => {
  it("produces the fixed v1 container and restores the exact identity", () => {
    ...
    expect(Buffer.from(backup).toString("hex")).toMatchInlineSnapshot(
    ...
    expect(
      decryptIdentityBackup(provider, backup, PASSPHRASE).getPrivateKey(),
  ...
  it("rejects a wrong passphrase and authenticated-header or ciphertext tampering generically", () => {
    ...
    expect(() =>
      decryptIdentityBackup(provider, backup, "wrong passphrase"),
    ).toThrow(IDENTITY_BACKUP_ERROR);
    for (const offset of [42, 80]) {
      const damaged = backup.slice();
      damaged[offset] ^= 1;
      expect(() =>
        decryptIdentityBackup(provider, damaged, PASSPHRASE),
      ).toThrow(IDENTITY_BACKUP_ERROR);
    }
  });
  ...
  it("round-trips both labelled 24-word BIP-39 groups", () => {
    ...
    expect(
      identityFromRecoveryWords(provider, recovery).getPrivateKey(),
    ).toEqual(identity.getPrivateKey());
    expect(() =>
      identityFromRecoveryWords(provider, {
        ...recovery,
        second: `${recovery.second} abandon`,
      }),
    ).toThrow("Invalid identity recovery words");
  });
  ...
  it("writes mode 0600 and atomically migrates a legacy raw identity", async () => {
    ...
    const migrated = await loadOrCreateIdentity(provider, path, {
    ...
    expect(
      decryptIdentityBackup(provider, stored, PASSPHRASE).getPrivateKey(),
    ).toEqual(identity.getPrivateKey());
  });
  ...
  it("does not replace a valid vault when encryption fails", () => {
    ...
    expect(() =>
      persistEncryptedIdentity(provider, path, fixedIdentity(), ""),
    ).toThrow();
  ...
  it("enforces creation policy separately from permissive unlock", () => {
    expect(() =>
      validateNewIdentityPassphrase("too short", "too short"),
    ).toThrow("at least 12");
    expect(() =>
      validateNewIdentityPassphrase("long enough pass", "different passphrase"),
    ).toThrow("does not match");
    expect(() =>
      validateNewIdentityPassphrase("long enough pass", "long enough pass"),
    ).not.toThrow();
  });

  it("round-trips randomly generated identities with fresh container entropy", () => {
    for (const passphrase of [
      "random passphrase alpha",
      "random passphrase beta",
    ]) {
      ...
      expect(
        decryptIdentityBackup(provider, first, passphrase).getPrivateKey(),
      ).toEqual(identity.getPrivateKey());
      expect(
        decryptIdentityBackup(provider, second, passphrase).getPrivateKey(),
      ).toEqual(identity.getPrivateKey());
    }
  });
});
