...
describe("tp identity", () => {
  it("creates an encrypted vault, exports it, and imports without changing the identity", async () => {
    ...
    try {
      expect(
        await runInit({ cwd, args: [], identityPassphrase: vaultPassphrase }),
      ).toBe(0);
      ...
      expect(
        isEncryptedIdentityBackup(new Uint8Array(readFileSync(vaultPath))),
      ).toBe(true);
  ...
  it("requires force and an interactive candidate-hash confirmation before replacement", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(cwd, "candidate.tpidentity"),
        encryptIdentityBackup(provider, candidate, backupPassphrase),
      );
