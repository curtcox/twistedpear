...
import { decodePeerInvitation, decodePeerInvitationText, encodePeerInvitationText, type PeerInvitationRole } from "@twistedpear/protocol";
...
export interface NtfyRendezvousSecret { readonly topic: Uint8Array; readonly key: Uint8Array; readonly codeSeed?: Uint8Array; }
...
export interface NtfySecretChannel { availability(): Promise<DiscoveryAvailability>; presentCode(session: DiscoverySession, code: string, options: OfferOptions): Promise<void>; requestCode(session: DiscoverySession, options: AcceptOptions): Promise<string>; cancel(sessionId: string): Promise<void>; }
...
function deriveShortSecret(seed: Uint8Array): NtfyRendezvousSecret {
  if (seed.length !== SHORT_SECRET_BYTES) throw new Error("ntfy short code requires a 16-byte seed");
  const topic = sha256(concat([new TextEncoder().encode("twistedpear/ntfy/topic/v2"), seed])).subarray(0, 16);
  const key = sha256(concat([new TextEncoder().encode("twistedpear/ntfy/key/v2"), seed]));
  return { topic, key, codeSeed: seed.slice() };
}
async function withNtfyPromiseBudget<T>(source: Promise<T>, options: OfferOptions, cancel: () => Promise<void>): Promise<T> {
  let timer: ReturnType<typeof setTimeout> | undefined; let abortListener: (() => void) | undefined;
  const budget = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => reject(new PeerDiscoveryError("TIMEOUT", "Peer discovery timed out")), options.timeoutMs);
    if (options.signal !== undefined) { abortListener = () => reject(new PeerDiscoveryError("CANCELLED", "Peer discovery cancelled")); if (options.signal.aborted) abortListener(); else options.signal.addEventListener("abort", abortListener, { once: true }); }
  });
  try { return await Promise.race([source, budget]); }
  catch (error) { await cancel(); throw error; }
  finally { if (timer !== undefined) clearTimeout(timer); if (options.signal !== undefined && abortListener !== undefined) options.signal.removeEventListener("abort", abortListener); }
}
...
export function encodeNtfyRendezvousSecret(secret: NtfyRendezvousSecret): string {
  ...
  if (secret.codeSeed !== undefined) {
    ...
    if (!equal(derived.topic, secret.topic) || !equal(derived.key, secret.key)) throw new Error("ntfy short-code seed does not match its topic and key");
...
export function decodeNtfyRendezvousSecret(code: string): NtfyRendezvousSecret {
  ...
  if (normalized.startsWith(SHORT_SECRET_PREFIX)) {
    let seed: Uint8Array;
    try { seed = decodePeerInvitationText(normalized.slice(SHORT_SECRET_PREFIX.length)); }
    catch { throw new PeerDiscoveryError("INVALID_INVITATION", "Invalid ntfy short code or checksum"); }
    if (seed.length !== SHORT_SECRET_BYTES) throw new PeerDiscoveryError("INVALID_INVITATION", "Invalid ntfy short code length");
...
export class NtfyRendezvousClient {
  ...
  async createSecret(): Promise<NtfyRendezvousSecret> {
    ...
    if (seed.length !== SHORT_SECRET_BYTES) throw new Error("Entropy provider returned an invalid ntfy short-code seed");
    return deriveShortSecret(seed);
  }
...
export class NtfyPeerDiscoveryAdapter implements PeerDiscoveryAdapter {
  ...
  async *offer(envelope: Uint8Array, options: OfferOptions): AsyncIterable<DiscoveryEvent> {
    ...
    try { await withNtfyPromiseBudget(this.options.channel.presentCode(session, encodeNtfyRendezvousSecret(secret), options), options, () => this.cancel(session.id)); yield { kind: "ready", session }; await this.options.client.publish(secret, envelope); for await (const message of this.wait(session, secret, options)) { if (message.role !== "answer") continue; yield { kind: "invitation", session, envelope: message.envelope }; return; } }
  ...
  async *accept(options: AcceptOptions): AsyncIterable<DiscoveryEvent> {
    ...
    const code = await withNtfyPromiseBudget(this.options.channel.requestCode(session, options), options, () => this.cancel(session.id));
    const secret = decodeNtfyRendezvousSecret(code); this.secrets.set(session.id, secret);
    try { for await (const message of this.wait(session, secret, options)) { if (message.role !== "offer") continue; const invitation = decodePeerInvitation(message.envelope, this.now()); if (invitation.service !== options.service) throw new PeerDiscoveryError("INVALID_INVITATION", "ntfy invitation has the wrong service"); yield { kind: "invitation", session, envelope: message.envelope }; return; } }
    finally { this.cancelled.delete(session.id); }
  }
