...
export interface PairingOfferContext {
  readonly envelope: Uint8Array;
  readonly privateState: unknown;
}
export interface PairingAnswerContext {
  readonly envelope: Uint8Array;
  readonly privateState: unknown;
  readonly peer: UnconfirmedPeer;
}
export interface AuthenticatedPairingContext {
  readonly privateState: unknown;
  readonly peer: UnconfirmedPeer;
}
...
export interface InvitationPairingDriverOptions {
  readonly backend: PeerPairingSecurityBackend;
  readonly replayCache?: PeerReplayCache;
  readonly now?: () => number;
}

function sessionKey(invitation: PeerInvitation): string {
...
function assertEnvelope(
  ...
  if (invitation.role !== role || invitation.service !== request.service)
    throw new PeerDiscoveryError(
      "INVALID_INVITATION",
      `Expected a ${role} for service ${request.service}`,
    );
  return invitation;
}
...
export class InvitationPairingDriver implements PeerPairingDriver {
  ...
  constructor(private readonly options: InvitationPairingDriverOptions) {
    this.replay = options.replayCache ?? new PeerReplayCache();
  }
  async request(
    adapter: PeerDiscoveryAdapter,
    request: PeerConnectRequest,
    ...
    const offerInvitation = assertEnvelope(
      offer.envelope,
      request,
      "offer",
      this.now(),
    ...
    const offerState = stepPeerPairing(initialPeerPairingState(), {
      ...
      expiresAt: offerInvitation.expiresAt,
    });
    for await (const event of adapter.offer(offer.envelope, {
