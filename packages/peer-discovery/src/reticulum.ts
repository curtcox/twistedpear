...
export interface ReticulumDiscoveryChannel {
  ...
  offer(session: DiscoverySession, envelope: Uint8Array, options: OfferOptions): AsyncIterable<Uint8Array>;
...
export class ReticulumPeerDiscoveryAdapter implements PeerDiscoveryAdapter {
  ...
  async *offer(envelope: Uint8Array, options: OfferOptions): AsyncIterable<DiscoveryEvent> {
    ...
    if (invitation.role !== "offer") throw new PeerDiscoveryError("INVALID_INVITATION", "Automatic Reticulum discovery requires an offer envelope");
    const session = { id: this.options.createSessionId(), kind: this.kind } as const;
    yield { kind: "ready", session };
    for await (const answerEnvelope of withDiscoveryBudget(this.options.channel.offer(session, envelope, options), options.timeoutMs, options.signal, () => this.cancel(session.id))) {
      const answer = decodePeerInvitation(answerEnvelope, this.now());
      if (answer.role !== "answer" || answer.service !== invitation.service || !same(answer.sessionId, invitation.sessionId)) {
        throw new PeerDiscoveryError("INVALID_INVITATION", "Automatic Reticulum answer does not match the offer");
      }
  ...
  async *accept(options: AcceptOptions): AsyncIterable<DiscoveryEvent> {
    for await (const inbound of withDiscoveryBudget(this.options.channel.listen(options), options.timeoutMs, options.signal, async () => {},)) {
      const offer = decodePeerInvitation(inbound.envelope, this.now());
      if (offer.role !== "offer" || offer.service !== options.service) throw new PeerDiscoveryError("INVALID_INVITATION", "Automatic Reticulum invitation has the wrong role or service");
      if (inbound.session.kind !== this.kind) throw new PeerDiscoveryError("INVALID_INVITATION", "Automatic Reticulum channel returned the wrong session kind");
      yield { kind: "invitation", session: inbound.session, envelope: inbound.envelope };
      return;
    }
  }

  async answer(session: DiscoverySession, envelope: Uint8Array): Promise<void> {
    if (session.kind !== this.kind || decodePeerInvitation(envelope, this.now()).role !== "answer") throw new PeerDiscoveryError("INVALID_INVITATION", "Automatic Reticulum answer is invalid");
