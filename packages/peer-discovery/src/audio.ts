...
export interface AudioInboundFrame {
...
export interface AudioDiscoveryAdapterOptions {
  readonly channel: AudioDiscoveryChannel;
...
export class AudioPeerDiscoveryAdapter implements PeerDiscoveryAdapter {
  ...
  async *offer(
    ...
    if (invitation.role !== "offer")
      throw new PeerDiscoveryError(
        "INVALID_INVITATION",
        "Audio offer requires an offer envelope",
      );
    const session = {
    ...
    let assembly = initialPeerAudioAssemblyState(
      this.now() + options.timeoutMs,
    );
    for await (const frame of withDiscoveryBudget(
      this.options.channel.transmit(
        session,
        framePeerAudioPayload(
      ...
      const result = stepPeerAudioAssembly(assembly, frame, this.now());
      assembly = result.state;
      yield {
        kind: "progress",
        session,
      ...
      if (result.payload !== null) {
        const answer = decodePeerInvitation(result.payload, this.now());
        if (answer.role !== "answer")
          throw new PeerDiscoveryError(
            "INVALID_INVITATION",
            "Audio return payload is not an answer",
          );
        yield { kind: "invitation", session, envelope: result.payload };
      }
    }
  }
  async *accept(options: AcceptOptions): AsyncIterable<DiscoveryEvent> {
    ...
    for await (const inbound of withDiscoveryBudget(
      this.options.channel.receive(options),
      options.timeoutMs,
      options.signal,
      async () => {
      ...
      if (active !== null && active.id !== inbound.session.id)
        throw new PeerDiscoveryError(
          "INVALID_INVITATION",
          "Mixed audio channel sessions",
        );
      active = inbound.session;
      const result = stepPeerAudioAssembly(assembly, inbound.frame, this.now());
      assembly = result.state;
      yield {
        kind: "progress",
      ...
      if (result.payload !== null) {
        const offer = decodePeerInvitation(result.payload, this.now());
        if (offer.role !== "offer" || offer.service !== options.service)
          throw new PeerDiscoveryError(
            "INVALID_INVITATION",
            "Audio invitation has the wrong role or service",
          );
        yield {
          kind: "invitation",
          session: inbound.session,
  ...
  async answer(session: DiscoverySession, envelope: Uint8Array): Promise<void> {
    ...
    if (invitation.role !== "answer")
      throw new PeerDiscoveryError(
        "INVALID_INVITATION",
        "Audio answer requires an answer envelope",
      );
