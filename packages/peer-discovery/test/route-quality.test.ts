...
function fakeRoute(): {
  ...
  return {
    ...
    route: {
      ...
      quality: () => ({ goodputBps: 2_000_000, rttMs: 40, mtu: 1_200, queueDepthBytes: 7 })
    }
  };
}
...
describe("meterHostPeerRoute", () => {
  it("passes the transport's own declared claim through until something is measured", () => {
    ...
    const metered = meterHostPeerRoute(route, { now: () => 0, declaredBps: 64_000, declaredMtu: 500 });
  ...
  it("reports observed goodput once enough real bytes have moved", async () => {
    ...
    let now = 0;
    const metered = meterHostPeerRoute(route, { now: () => now, declaredBps: 64_000, declaredMtu: 500 });
    metered.subscribe?.(() => {});
    now = 100;
    ...
    expect(quality).toMatchObject({ goodputBps: 32_768, source: "observed", samples: 1, rttMs: 40 });
  ...
  it("does not count bytes a failing transport never accepted", async () => {
    ...
    let now = 0;
    const metered = meterHostPeerRoute(failing, { now: () => now, declaredBps: 64_000, declaredMtu: 500 });
    for (let index = 0; index < 8; index += 1) {
      now += 100;
      await expect(metered.send(new Uint8Array(4_096))).rejects.toThrow("route is down");
    }
    expect(metered.quality?.().source).toBeUndefined();
  });

  it("leaves a route without a subscribe hook without one", () => {
    const metered = meterHostPeerRoute({ send() {} }, { now: () => 0, declaredBps: 64_000, declaredMtu: 500 });
