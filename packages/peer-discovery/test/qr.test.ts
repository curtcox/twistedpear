...
function invitation(
  ...
  const value: PeerInvitation = {
    version: 1,
    sessionId: new Uint8Array(16).fill(sessionByte),
    service: "chat",
    role,
    peerEphemeralKey: new Uint8Array(32).fill(2),
    candidates: large
      ? [{ kind: "webrtc", value: new Uint8Array(2_000).fill(3) }]
      : [],
    display: "Peer",
    issuedAt: 1_000,
    expiresAt: 61_000,
...
describe("QR discovery adapter", () => {
  it("uses one static QR for small invitations and bounded animated frames for large ones", () => {
    ...
    expect(
      Math.max(...animated.map((code) => code.length)),
  ...
  it("accepts reordered and duplicated animated frames", async () => {
    ...
    expect(events.filter((event) => event.kind === "invitation")).toHaveLength(
      1,
    );
  });

  it("rejects mixed animated sessions and exposes side-effect-free availability", async () => {
    const first = encodePeerQrCodes(invitation("offer", true, 1));
    const second = encodePeerQrCodes(invitation("offer", true, 2));
    ...
    const channel: QrDiscoveryChannel = {
      ...
      async *present() {},
      async *scan() {
        yield { session, code: first[0]! };
        yield { session, code: second[1]! };
      },
      async answer() {},
      async cancel() {},
    };
