...
function envelope(role: "offer" | "answer") {
  const value: PeerInvitation = {
    ...
    identityProof: bytes(64, 3),
    candidates: [{ kind: "reticulum", value: bytes(16, 4) }],
    display: "peer",
    issuedAt: now - 1,
...
describe("audio discovery adapter", () => {
  it("uses shared frames and recovers a lost response frame", async () => {
    ...
    const adapter = new AudioPeerDiscoveryAdapter({
      now: () => now,
      createSessionId: () => "audio-session",
      channel: {
        async availability() {
          return { state: "permission-required" };
        },
        async *transmit(_session, frames) {
          expect(frames.length).toBeGreaterThan(1);
          const response = framePeerAudioPayload(bytes(16, 1), answer, 64);
          for (const [index, frame] of response.entries())
            if (index !== 1) yield frame;
        },
        async *receive() {},
        async answer() {},
        async cancel() {},
    ...
    const events = [];
    for await (const event of adapter.offer(envelope("offer"), {
      timeoutMs: 5_000,
    }))
      events.push(event);
      ...
      channel: {
        async availability() {
          return {
            ...
            reason: "Microphone starts after user action",
          };
        },
        async *transmit() {
          effects += 1;
