...
const definitions = {
  ...
  stream: { machine: streamMachine, model: "../specs/spec-stream/model/stream.tla", traces: "../specs/spec-stream/model/stream-conformance-traces.json", vector: "stream.json" }
};
...
export function assertStreamProperties() {
  ...
  if (selected?.plane !== "webrtc") throw new Error("stream plane order is not conforming");
  ...
  if (!admittedWithinHeadroom(admitted, 50_000)) throw new Error("stream admission exceeds headroom");
  ...
  if (adapted.rungIndex < admitted.rungIndex) throw new Error("stream degradation is not monotone");
  ...
  if (JSON.stringify(refused) !== JSON.stringify(unreachable)) throw new Error("readiness refusal leaks posture");
  ...
  if (decoded.version !== 2 || decoded.captureAtUs !== 3 || decoded.clockId !== 4) {
