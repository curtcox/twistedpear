...
if (definitions === null)
  throw new Error(
    "usage: node formal/run-symbolic-provers.mjs <tamarin|proverif>",
  );

if (
  mode === "tamarin" &&
  process.env.SIM_EXPECTED_TAMARIN_VERSION !== undefined
) {
  const version = run(
    definitions.executable,
    ["--version"],
    "Tamarin version check",
    timeoutMs,
  ).output;
  const expected = process.env.SIM_EXPECTED_TAMARIN_VERSION;
  if (
    !version.includes(`Tamarin version ${expected}`) &&
    !version.includes(`tamarin-prover ${expected}`)
  ) {
...
for (const file of definitions.files) {
  ...
  if (mode === "tamarin") {
    const expected = [...source.matchAll(/^lemma\s+([A-Za-z0-9_]+)/gm)].map(
      (match) => match[1],
    );
    if (expected.length === 0)
      throw new Error(`Tamarin model declares no lemmas: ${path}`);
    for (const lemma of expected) {
      ...
      const proved = new RegExp(
        `^\\s*${escapeRegExp(lemma)}\\s+\\([^)]*\\):\\s+verified\\b`,
        "m",
      );
      if (!proved.test(output)) {
        throw new Error(
          `Tamarin did not verify expected lemma ${lemma} in ${path}`,
        );
      }
  ...
  if (/is false|cannot be proved/i.test(output))
    throw new Error(`ProVerif failed ${path}`);
  const expected = [...source.matchAll(/^query\s/gm)].length;
  const proved = [...output.matchAll(/^RESULT .* is true\.\s*$/gm)].length;
  if (proved !== expected)
    throw new Error(`ProVerif proved ${proved}/${expected} queries in ${path}`);
  console.log(
    `ProVerif verified ${path} (${proved} queries) in ${elapsedMs}ms`,
  );
}
