...
| Freenet integration | F1–F5 software advanced on the [simulator-first plan](docs/freenet-simulator-first-work-plan.md): S4 browser unsupported by policy, F2 notify reconciliation, user-supplied-binary supervisor, mobile remote-node grant chrome. Remaining: live S2/S7 write auth, signing/embedding, physical BareKit confirmation. | [Integration plan](docs/freenet-integration-plan.md), [simulator-first plan](docs/freenet-simulator-first-work-plan.md), [Option A ADR](docs/adr-freenet-app-execution.md), [F0 evidence](conformance/freenet-spike/evidence-status.json) |
