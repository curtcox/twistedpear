...
| Freenet integration | F1 software foundation landed. F0 S1/S3/S6/S8 complete; S2 local latency evidence opens all transport roles on the measured path. SPEC-FREENET pins locator, packet-log, and propagation-set WASM contracts. F2 `FreenetInterface` is wired (90 kbps policy; `npm run test:freenet-interface` HDLC exchange); announce+LXMF exit still open. F3 store proof passed (`npm run test:freenet-propagation`); host mirror wiring open. Live S2 and authorized Atlas write remain open. F4–F6 gated. | [Integration plan](docs/freenet-integration-plan.md), [F0 evidence](conformance/freenet-spike/evidence-status.json), [SPEC-FREENET](specs/spec-freenet/spec.md) |
