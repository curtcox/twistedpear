...
| Device I/O and sensors for mini-apps | Plan phases 1–7 landed in software: registry, Device Manager, SDK (`inventory`/`diagnostics`/`open`/`close`/`read`/`write`/`stream`), derived+raw sensors, actuators, sidecar, admission, remote acquisition, NFC payment AID blocklist, biometric assertions, scalar sensors, [device-class runbook](docs/device-class-runbook.md). Shipping hosts and handbook inject simulated `DeviceManager`; `InterfaceManager` is the node `relayService`. Remaining: host Devices UI, real OS drivers, formal vectors/TLA+, hardware-gated evidence, shipping-host `relayService` wiring | [Device I/O plan](docs/device-io-plan.md), [SPEC-DEVICE](specs/spec-device/spec.md), [platform capabilities status](docs/platform-capabilities-status.md) |
