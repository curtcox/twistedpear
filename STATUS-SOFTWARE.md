...
| BUG-DEVSTUDIO-SUBSET-GRANT  | open   | Subset capability grants cannot launch: optional is dropped in the devstudio packaging path     | —                                                                        | `npm run test:devstudio-loop`                                                                                                |
| BUG-IOS-CHANNEL-ISOLATION   | open   | ios-sim apps-channel-isolation regressed after the per-host confirmation limiter                | —                                                                        | `bash conformance/ios-sim/ci-required.sh`                                                                                    |
| BUG-HANDBOOK-RENDER-TIMEOUT | open   | test:handbook times out waiting for the handbook title to render                                | —                                                                        | `npm run test:handbook`                                                                                                      |
| BUG-SITE-CHECKS-CHECKOUT    | open   | reported-checks uses local composite actions without actions/checkout                           | —                                                                        | `npm run actionlint`                                                                                                         |
...
| QL-CHECK-RUN-HISTORY        | open   | Bounded per-run gate history: duration, retries, refusals, peak RSS, quiet intervals            | Plan: [check efficiency](docs/check-efficiency-plan.md) §1               | `npx vitest run conformance/checks/run-history.test.mjs`                                                                     |
| QL-CHECK-OUTCOMES           | open   | Per-gate outcome history backfilled from ci-metrics, split real / flake / infrastructure        | Plan: [check efficiency](docs/check-efficiency-plan.md) §1               | `npx vitest run conformance/checks/gate-outcomes.test.mjs`                                                                   |
| QL-CHECK-COST-REPORT        | open   | checks:cost — per-gate cost, yield, supervision, and the tier the numbers justify               | Plan: [check efficiency](docs/check-efficiency-plan.md) §1               | `npx vitest run conformance/checks/cost-report.test.mjs`                                                                     |
| QL-CHECK-TIERS              | open   | daily, weekly and monthly gate tiers declared in the registry and expanded into schedules       | Plan: [check efficiency](docs/check-efficiency-plan.md) §2               | `npx vitest run conformance/checks/gate-tiers.test.mjs`                                                                      |
| QL-CHECK-PLACEMENT-AUDIT    | open   | Placement audit: propose tier moves from measured cost and yield, with the irreversibility list | Plan: [check efficiency](docs/check-efficiency-plan.md) §3               | `npx vitest run conformance/checks/placement-audit.test.mjs`                                                                 |
| QL-CHECK-SERIAL-LANES       | open   | Pack the PR gate matrix into duration-ordered serial lanes and record the weighted-minute delta | Plan: [check efficiency](docs/check-efficiency-plan.md) §5               | `npx vitest run conformance/checks/lane-packing.test.mjs`                                                                    |
| QL-CHECK-COVERAGE-REUSE     | open   | Remove the unit/coverage double traversal — 76.2% of the last recorded base check               | Measured: [agent session performance](docs/agent-session-performance.md) | `npx vitest run conformance/checks/coverage-evidence.test.mjs`                                                               |
| QL-CHECK-UNATTENDED         | open   | Unattended gate runs: streamed output, heartbeats, fail-fast, preserved exit codes, resume      | Plan: [check efficiency](docs/check-efficiency-plan.md) §6               | `npx vitest run conformance/checks/unattended-runner.test.mjs`                                                               |
| QL-CHECK-REVIEW-LOOP        | open   | checks:review and the merge/daily/weekly/monthly review cadence                                 | Plan: [check efficiency](docs/check-efficiency-plan.md) §review-cadence  | `runbook:docs/check-efficiency-plan.md#monthly`                                                                              |
