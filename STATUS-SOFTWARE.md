...
| Freenet integration | F1–F3 software largely landed: package path, wired `FreenetInterface` (90 kbps; HDLC live proof; sim announce+LXMF; relay policy), and Freenet propagation mirror on `createNodeHost` when enabled. F0 S1/S3/S6/S8 complete; S2 local latency opens roles. Live S2/Atlas write and F4–F6 (S5/S4/S7) remain gated. | [Integration plan](docs/freenet-integration-plan.md), [F0 evidence](conformance/freenet-spike/evidence-status.json), [SPEC-FREENET](specs/spec-freenet/spec.md) |
