...
| Freenet integration | F1–F3/F5 software landed: package path, wired `FreenetInterface`, Freenet propagation mirror, `freenet:contract` (HOST_API 0.11.0) with node/desktop `freenetBackend` wiring, desktop Settings chrome, and Option A ADR. F4 blocked on S5; live S2/S7 write gated. | [Integration plan](docs/freenet-integration-plan.md), [Option A ADR](docs/adr-freenet-app-execution.md), [F0 evidence](conformance/freenet-spike/evidence-status.json) |
