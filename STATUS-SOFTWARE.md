...
| Freenet integration | F1–F3/F5 software landed: package path, wired `FreenetInterface`, Freenet propagation mirror, `freenet:contract` with node/desktop wiring and desktop Settings (contracts + HDLC toggle), `tp node --freenet` external-node flags, Option A ADR. S5 has Linux/Windows release archive sizes; F4 still blocked on fresh signing/embedding. Live S2/S7 write gated. | [Integration plan](docs/freenet-integration-plan.md), [Option A ADR](docs/adr-freenet-app-execution.md), [F0 evidence](conformance/freenet-spike/evidence-status.json) |
