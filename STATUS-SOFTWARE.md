...
| Device I/O and sensors for mini-apps | Phases 1–3 in progress: registry, Device Manager, `device` SDK (incl. `write`), sensors through derived camera/mic/motion, preview widgets, and actuators (`torch`/`speaker`/`tts`/`haptics`/`nfc:ndef`) with safety caps. Remaining: raw sidecar, streaming, remote acquisition, host Devices UI | [Device I/O plan](docs/device-io-plan.md), [SPEC-DEVICE](specs/spec-device/spec.md) |
