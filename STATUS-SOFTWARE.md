...
| Realtime peer media (`line-check`) | Core software landed: app-scoped link observation and bounded probes; expiring two-sided readiness; host-authored outbound share offers; fail-closed five-plane egress binding and receive-side host sinks; corrected admission plus backpressure adaptation; `TPD2` timestamps, clock/jitter primitives, encoding profiles and codec Effects boundary; foreground-only signed invite routing; Line Check cookbook app; SPEC-STREAM executable/TLA+/trace/vector cross-check (`npm run formal:stream`). Two headless peers on one Mac complete a decoded readiness exchange, a measured active probe, and derived+PCM `TPD2` carrier round trips (`npm run test:local-multipeer`); the ladder collapses, settles, and recovers across four adversarial link profiles and four transport classes (`npm run test:sim-media-ladder`); share grant/revoke/expiry/restart/indicator and invite accept/decline are driven through the shipping desktop renderer (`npm run test:share-policy`). Remaining software is shipping-host integration: live transport telemetry, inbound `session-invite` delivery, concrete WebRTC/Pears/CAS openers, real platform codec drivers, a device-run Maestro share probe, and end-to-end multi-host media conformance. Hardware/network evidence remains in STATUS-HARDWARE | [Realtime peer media plan](docs/realtime-media-plan.md), [SPEC-STREAM](specs/spec-stream/spec.md), [platform capabilities](docs/platform-capabilities-status.md) |
