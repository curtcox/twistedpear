...
| Freenet integration | F1–F5 software advanced on the [simulator-first plan](docs/freenet-simulator-first-work-plan.md): S4 browser unsupported by policy, F2 notify reconciliation + distinct-node F2/F3 runner, user-supplied-binary supervisor, mobile remote-node grant chrome with Maestro probes. Remaining: live S2/S7 write auth, reviewed cross-node notify series, signing/embedding, physical BareKit confirmation. | [Integration plan](docs/freenet-integration-plan.md), [simulator-first plan](docs/freenet-simulator-first-work-plan.md), [Option A ADR](docs/adr-freenet-app-execution.md), [F0 evidence](conformance/freenet-spike/evidence-status.json) |
