...
| HEADROOM-RIVAL-RSS             | open   | Headroom probe counts zero-memory processes as rival heaps             | —                                                                     | `npm test -- conformance/checks/headroom.test.mjs`                                                                           |
| TYPED-LINT-BUILT-WORKER        | open   | Typed lint ratchet scans the gitignored built web worker bundle        | —                                                                     | `npm run lint:typed`                                                                                                         |
| ARTIFACT-SIZES-BASELINE-FORMAT | open   | artifact-sizes:baseline writes JSON the format gate rejects            | —                                                                     | `npm run format:check`                                                                                                       |
