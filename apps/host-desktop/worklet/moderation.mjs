...
export function createModerationOps(deps) {
  ...
  async function persistModerationState() {
    await runtime.store.set(
      MODERATION_STORE_KEY,
      new TextEncoder().encode(JSON.stringify(state.moderationState)),
    );
  }
  ...
  function pushModerationState() {
    send({
      ...
      blocked: state.moderationState.blocked,
      muted: state.moderationState.muted,
      reports: state.moderationState.reports,
    });
  }
