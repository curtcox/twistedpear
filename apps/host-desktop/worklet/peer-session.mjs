...
export function createPeerSessionOps(deps) {
  ...
  const ensurePeerLinkDestination = (...args) =>
    deps.ensurePeerLinkDestination(...args);
  const automaticReticulumChannel = (...args) =>
    deps.automaticReticulumChannel(...args);
  ...
  async function ensurePeerSessionManager() {
    ...
    if (identity === null)
      throw new Error("Unlock the host identity before connecting to a peer");
    ...
    registry.register(
      new ManualPeerDiscoveryAdapter({
        channel: {
          offer: (session, code, options) =>
            peerChrome.manual.offer(session, code, options),
    ...
    registry.register(
      new QrPeerDiscoveryAdapter({ channel: peerChrome.qr, createSessionId }),
    );
    registry.register(
    ...
    registry.register(
      new UnavailablePeerDiscoveryAdapter("bluetooth", {
        ...
        reason:
          "Native invitation GATT is not enabled; the BLE Reticulum interface remains available as a data path",
      }),
    );
    ...
    registry.register(
      new UnavailablePeerDiscoveryAdapter("local-peer-to-peer", {
        state: "unsupported",
        reason: "This runtime does not implement LP2PRequest/LP2PReceiver",
      }),
    );
    const backend = new CryptoPeerPairingBackend({
      ...
      async establish(context, peer, adapter) {
        if (peer.dataPlane === "webrtc") {
          ...
          if (!webRtcRoutePending.has(sessionId))
            webRtcRoutePending.set(sessionId, []);
          ...
          return {
            ...
            route: meterHostPeerRoute(
              {
                async send(payload) {
                  const sendToken = generateConfirmationToken((length) =>
                    provider.randomBytes(length),
                  );
                ...
                subscribe(listener) {
                  ...
                  for (const pending of webRtcRoutePending
                    .get(sessionId)
                    ?.splice(0) ?? [])
                    listener(pending);
        ...
        const remoteIdentity = Identity.fromPublicKey(
          provider,
        ...
        const aspect = bytesToHex(
          provider
            .sha256(new TextEncoder().encode(context.remoteInvitation.service))
            .subarray(0, 16),
        );
          ...
          route: meterHostPeerRoute(
            {
              ...
              quality() {
                return {
                  goodputBps: link.attachedInterface?.bitrate ?? 2_000_000,
                  rttMs: (link.rtt ?? 0) * 1_000,
                  mtu: link.mtu,
                  queueDepthBytes: outboundBandwidthLimiter.queueDepthBytes(),
                };
              },
            ...
            {
              now: () => Date.now(),
              declaredBps: link.attachedInterface?.bitrate ?? 2_000_000,
              declaredMtu: link.mtu,
            },
          ),
          async close() {
            peerLinks.delete(peer.fingerprint);
            await link.teardown();
          },
        };
      },
    });
    state.peerSessionManager = new PeerSessionManager(
      registry,
  ...
  const peerSessionManagerProxy = {
    ...
    list(appId) {
      return state.peerSessionManager?.list(appId) ?? [];
    },
    route(appId, handle) {
      return state.peerSessionManager?.route(appId, handle);
    },
    info(appId, runtimeId, handle) {
      if (state.peerSessionManager === null)
    ...
    async close(appId, runtimeId, handle) {
      if (state.peerSessionManager !== null)
        await state.peerSessionManager.close(appId, runtimeId, handle);
    },
    async closeRuntime(appId, runtimeId) {
      if (state.peerSessionManager !== null)
        await state.peerSessionManager.closeRuntime(appId, runtimeId);
    },
  };
