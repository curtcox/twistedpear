...
export function createDesktopPeerChrome({
  requestReply,
  send,
  createToken,
  ntfyServer = null,
  ...
  const cancel = async (sessionId) => {
    ...
    if (token !== undefined)
      send({ type: "peer-chrome-cancel", token, sessionId });
  };
  return {
    manual: {
      async *offer(session, code, options) {
        const reply = await request(
          "peer-manual-present",
          { sessionId: session.id, code, expectsResponse: true },
          options.timeoutMs,
        );
      ...
      async *accept(options) {
        ...
        const reply = await request(
          "peer-manual-enter",
          { sessionId, service: options.service },
          options.timeoutMs,
        );
        if (reply?.accepted === true && typeof reply.code === "string")
          yield {
            session: { id: sessionId, kind: "manual" },
            code: reply.code,
          };
      },
      async answer(session, code) {
        await request(
          "peer-manual-present",
          { sessionId: session.id, code, expectsResponse: false },
          120_000,
        );
      },
      cancel,
    ...
    qr: {
      async availability() {
        ...
        return (
          reply?.availability ?? {
            state: "unsupported",
            reason: "QR camera/display support could not be detected",
          }
        );
      },
      async *present(session, codes, options) {
        const reply = await request(
          "peer-qr-present",
          { sessionId: session.id, codes, expectsResponse: true },
          options.timeoutMs,
        );
        if (reply?.accepted === true && typeof reply.code === "string")
          yield reply.code;
      ...
      async *scan(options) {
        ...
        if (reply?.accepted === true && typeof reply.code === "string")
          yield { session: { id: sessionId, kind: "qr" }, code: reply.code };
      },
      async answer(session, codes) {
        await request(
          "peer-qr-present",
          { sessionId: session.id, codes, expectsResponse: false },
          120_000,
        );
      },
      cancel,
    ...
    audio: {
      async availability() {
        ...
        return (
          reply?.availability ?? {
            state: "unsupported",
            reason: "Desktop audio support could not be detected",
          }
        );
      },
      async *transmit(session, frames, options) {
        ...
        for (const frame of reply?.framesHex ?? [])
          yield Uint8Array.from(frame.match(/../g) ?? [], (pair) =>
      ...
      async *receive(options) {
        ...
        const reply = await request(
          "peer-audio-receive",
          { sessionId: session.id, service: options.service },
          options.timeoutMs,
        );
        if (reply?.error !== undefined) throw new Error(reply.error);
        for (const frame of reply?.framesHex ?? [])
          yield {
            session,
            frame: Uint8Array.from(frame.match(/../g) ?? [], (pair) =>
              Number.parseInt(pair, 16),
            ),
          };
      },
      async answer(session, frames) {
        ...
        if (reply?.accepted !== true)
          throw new Error(
            reply?.error ?? "Audio answer playback was cancelled",
          );
      },
      cancel,
    },
    ntfy: {
      async availability() {
        ...
        return (
          reply?.availability ?? {
            state: "offline",
            reason: "ntfy configuration could not be detected",
          }
        );
      },
      async presentCode(session, code, options) {
        const reply = await request(
          "peer-ntfy-present",
          { sessionId: session.id, code, server: ntfyServer },
          options.timeoutMs,
        );
        if (reply?.accepted !== true)
