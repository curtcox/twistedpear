...
export async function handleMediaOpusPlayRequest(message, send) {
  try {
    const played = await playInboundMediaFrame({
      sink: { kind: "speaker" },
      encoding:
        typeof message.encoding === "string" ? message.encoding : "16k-opus",
...
async function encodeAudio(configuration, captureAtUs, bytes) {
  ...
  if (Encoder === undefined || Audio === undefined)
    throw new Error("Chromium WebCodecs Opus encode is unavailable.");
...
async function decodeAudio(configuration, captureAtUs, bytes) {
  ...
  if (Decoder === undefined || Chunk === undefined)
    throw new Error("Chromium WebCodecs Opus decode is unavailable.");
  return new Promise((resolve, reject) => {
    let settled = false;
    const settle = (fn) => {
      if (settled) return;
...
function audioConfiguration(encoding) {
  return {
    codec:
      encoding.includes("opus") || encoding.includes("narrowband")
        ? "opus"
        : "pcm",
    sampleKind: "audio",
    bitrateBps: encoding.includes("8k") ? 12_000 : 24_000,
    sampleRate: encoding.includes("48k")
      ? 48_000
...
function assertAudioConfiguration(configuration) {
  if (
    ...
    throw new Error("Desktop media codec configuration is unsupported.");
}
...
function unhex(value) {
  if (
    ...
    throw new Error("Media bytes are malformed.");
