...
export async function supportsQrDetection(
  BarcodeDetectorClass = globalThis.BarcodeDetector,
  portableDecoder = globalThis.jsQR,
) {
  if (typeof BarcodeDetectorClass !== "function")
    return typeof portableDecoder === "function";
  ...
  return (
    ...
    typeof portableDecoder === "function"
  );
}

export async function decodeQrVideoFrame(
  video,
  BarcodeDetectorClass = globalThis.BarcodeDetector,
  portableDecoder = globalThis.jsQR,
) {
  if (typeof BarcodeDetectorClass === "function") {
    const supported =
      ...
      (await BarcodeDetectorClass.getSupportedFormats()).includes("qr_code");
    if (supported) {
      const codes = await new BarcodeDetectorClass({
        formats: ["qr_code"],
      }).detect(video);
      return (
        codes.find((code) => typeof code.rawValue === "string")?.rawValue ??
        null
      );
    }
  }
