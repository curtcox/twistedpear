...
interface BonjourNative {
  ...
  addListener(
    event: "onServiceFound",
    listener: (event: BonjourServiceRecord) => void,
  ): EventSubscription;
  addListener(
    event: "onServiceLost",
    listener: (event: {
...
export function getBonjourCapability(): {
...
export function createNativeBonjourBridge(): BonjourBridge {
  if (NativeBonjour === null) {
    throw new Error(
      "Bonjour bridge is only available in native Android and iOS builds",
    );
  }

  let events: BonjourBridgeEvents = {};
  let interfaces: ReadonlyArray<{
    readonly name: string;
    readonly linkLocalAddress: string;
  }> = [];
  ...
  return {
    ...
    async start(serviceType = BONJOUR_RETICULUM_SERVICE) {
      ...
      foundSubscription = NativeBonjour.addListener(
        "onServiceFound",
        (record) => {
      ...
      networkSubscription = NativeBonjour.addListener(
        "onNetworkChange",
        (event) => {
...
export async function stopNativeBonjourBridge(
