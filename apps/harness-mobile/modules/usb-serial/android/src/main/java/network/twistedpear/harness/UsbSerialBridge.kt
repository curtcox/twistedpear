...
class UsbSerialBridge(
    ...
    fun requestPermission(
        deviceId: Int,
        permissionIntent: android.app.PendingIntent,
    ): Boolean {
        val device = findDevice(deviceId) ?: return false
    ...
    fun open(
        ...
        readThread =
            thread(name = "twistedpear-usb-serial-read", isDaemon = true) {
                readLoop()
            }
    ...
    private fun findDevice(deviceId: Int): UsbDevice? = usbManager.deviceList.values.firstOrNull { it.deviceId == deviceId }
    ...
    companion object {
        ...
        val KNOWN_RNODE_USB_IDS: Set<Pair<Int, Int>> =
            setOf(
                0x10C4 to 0xEA60,
                0x0403 to 0x6001,
                0x1A86 to 0x7523,
        ...
        fun isKnownRNodeUsbId(
            vendorId: Int,
            productId: Int,
        ): Boolean = KNOWN_RNODE_USB_IDS.contains(vendorId to productId)
