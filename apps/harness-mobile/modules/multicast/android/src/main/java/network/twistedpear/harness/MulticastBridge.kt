 ...
 */
class MulticastBridge(private val context: Context) {
  interface Listener {
    fun onPacket(ifname: String, data: ByteArray, sourceAddress: String, port: Int)
  ...
  fun joinGroup(ifname: String, groupAddress: String, port: Int) {
    if (!running.get()) {
      return
    }
  ...
  fun bindPort(ifname: String, port: Int) {
    if (!running.get()) {
      return
    }
  ...
  fun send(ifname: String, groupAddress: String, port: Int, data: ByteArray) {
    if (!running.get()) {
      return
    }
  ...
  fun sendUnicast(ifname: String, targetAddress: String, port: Int, data: ByteArray) {
    if (!running.get()) {
      return
    }
  ...
  private fun openSocket(ifname: String, port: Int, reuse: Boolean): MulticastSocket? {
    return try {
  ...
  private fun startReader(key: String, socket: MulticastSocket, ifname: String) {
    if (readThreads.containsKey(key)) {
      return
    }

    val reader = thread(name = "multicast-reader-$key", isDaemon = true) {
  ...
  companion object {
    fun descopeLinkLocal(address: String): String {
      return address.split("%")[0].replace(Regex("fe80:[0-9a-f]*::", RegexOption.IGNORE_CASE), "fe80::")
    }
  }
}
