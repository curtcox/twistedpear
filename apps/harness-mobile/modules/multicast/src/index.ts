...
interface MulticastNative {
  ...
  addListener(
    event: "onPacket",
    listener: (event: MulticastPacketEvent) => void,
  ): EventSubscription;
  addListener(
    event: "onNetworkChange",
    listener: (event: MulticastNetworkChangeEvent) => void,
  ): EventSubscription;
}
...
export function getMulticastCapability(): {
  ...
  if (NativeMulticast === null) {
    return {
      supported: false,
      reason: "native multicast bridge unavailable",
...
export function createNativeMulticastBridge(): MulticastBridge {
  if (NativeMulticast === null) {
    throw new Error(
      "Multicast bridge is only available in native Android and iOS builds",
    );
  }
  ...
  return {
    ...
    async start() {
      ...
      networkSubscription = NativeMulticast.addListener(
        "onNetworkChange",
        (event) => {
...
export async function stopNativeMulticastBridge(
