...
public final class TwistedPearPeerAudioModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("playPcm16") { (pcm: Data, sampleRate: Int) -> Bool in
      ...
      guard let format = AVAudioFormat(
        ...
        throw Exception(
          name: "PCM_FORMAT",
          description: "Native PCM playback format is unavailable"
        )
      }
      buffer.frameLength = buffer.frameCapacity
      pcm.withUnsafeBytes { raw in
        if let source = raw.bindMemory(to: Int16.self).baseAddress,
           let target = buffer.int16ChannelData?[0] {
          target.update(from: source, count: pcm.count / 2)
        }
      }
      engine.connect(player, to: engine.mainMixerNode, format: format)
      try engine.start()
      let completed = DispatchSemaphore(value: 0)
      player.scheduleBuffer(buffer) { completed.signal() }
      player.play()
      _ = completed.wait(timeout: .now() + .seconds(31))
      player.stop()
      engine.stop()
      return true
    }
  ...
  private func record(durationMs: Int, sampleRate: Int, voiceDuplex: Bool) throws -> Data {
    guard durationMs >= 100 && durationMs <= 30_000 else {
      throw Exception(
        name: "PCM_BOUNDS",
        description: "Native peer recording is outside the host budget"
      )
    }
    ...
    try session.setCategory(
      .playAndRecord,
      mode: voiceDuplex ? .voiceChat : .measurement,
    ...
    let lock = NSLock()
    var output = Data()
    let maximum = Int(Double(durationMs) * Double(sampleRate) * 2.0 / 1000.0)
    input.installTap(
      onBus: 0,
      bufferSize: 4096,
      ...
      for index in 0..<Int(buffer.frameLength) {
        var sample = Int16(
          max(-1, min(1, channel[index])) * Float(Int16.max)
        ).littleEndian
        withUnsafeBytes(of: &sample) { bytes.append(contentsOf: $0) }
      }
      lock.lock()
      if output.count < maximum {
        output.append(bytes.prefix(maximum - output.count))
      }
      lock.unlock()
    }
  ...
  private func validate(sampleRate: Int, samples: Int) throws {
    guard sampleRate >= 8_000,
          sampleRate <= 48_000,
          samples >= 1,
          samples <= sampleRate * 30 else {
      throw Exception(
        name: "PCM_BOUNDS",
        description: "Native peer PCM is outside the host budget"
      )
    }
  }
}
