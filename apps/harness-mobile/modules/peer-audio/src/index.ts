...
interface NativePeerAudio { requestPermission?(): Promise<boolean>; playPcm16(pcm: Uint8Array, sampleRate: number): Promise<boolean>; recordPcm16(durationMs: number, sampleRate: number): Promise<Uint8Array>; }
...
export async function recordNativePeerPcm(durationMs: number, sampleRate: number): Promise<Uint8Array> { if (Native === null) throw new Error("Native peer audio is unavailable"); const bytes = await Native.recordPcm16(durationMs, sampleRate); return bytes instanceof Uint8Array ? bytes : Uint8Array.from(bytes as ArrayLike<number>); }
