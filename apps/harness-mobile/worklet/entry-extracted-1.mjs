...
export async function ensureBareWebSocketImpl(context) {
    if (context.bareWebSocketReady === null) {
        context.bareWebSocketReady = import("../../../conformance/freenet-spike/bare-websocket-shim.mjs").then(({ installBareWebSocketGlobal }) => {
...
export async function importTrustedPublisherImpl(context, identityString, label, source = "paste") {
    const publisherPublicKey = decodePublisherIdentity256t(identityString);
    const confirmation = await context.requestHostReply({
        type: "confirm-request",
        token: generateConfirmationToken((length) => context.provider.randomBytes(length)),
        kind: "trust-import",
        appId: "host",
        publisherPublicKey,
...
export function ensureCrossDeviceTestDriverImpl(context) {
    if (context.crossDeviceTestDriver === null) {
        const base = createCrossDeviceTestDriver({
            ...
            runtime,
            installFromT256,
            importTrust: (identity256t, label) => context.importTrustedPublisher(identity256t, label),
        ...
        context.crossDeviceTestDriver = async (request) => {
            ...
            if (request?.cmd === "renderer-ping") {
                const reply = await context.requestHostReply({ type: "peer-qr-availability", token: context.peerToken() }, 10000);
                return {
                    ok: reply !== null,
            ...
            if (request?.cmd === "peer-pair-code-out") {
                const taken = await context.harnessPeerPair.takeOutboundCode(typeof request.timeoutMs === "number" ? request.timeoutMs : 60000);
            ...
            if (request?.cmd === "peer-pair-code-in") {
                ...
                context.harnessPeerPair.giveInboundCode(request.code, typeof request.sessionId === "string" ? request.sessionId : undefined);
                return { ok: true };
            }
            if (request?.cmd === "peer-pair-wait") {
                return context.harnessPeerPair.wait(typeof request.timeoutMs === "number" ? request.timeoutMs : 120000);
            }
            if (request?.cmd === "webrtc-open-media") {
                ...
                if (bytesSent === 0 && typeof attached.sessionId === "string") {
                    for (let attempt = 0; attempt < 20 && bytesSent === 0; attempt += 1) {
                        ...
                        const stats = await context.requestHostReply({ type: "peer-webrtc-media-stats", token: context.peerToken(), sessionId: attached.sessionId }, 10000);
...
export async function handleNativeMediaOpusCommandImpl(context, request) {
    ...
    const duplex = await context.requestHostReply({ type: "media-opus-duplex-request", token: context.peerToken() }, 90000);
    ...
    if (context.miniappHost === null) {
        const relayService = createWorkletFlagRelayService({
            ...
            getFlags: () => ({
                ...
                autoOnline: context.autoIface?.online === true || context.status.autoPeers > 0,
        ...
        context.miniappHost = createWorkletMiniappHost({
            ...
            openPearsBulkPlane: {
                async append({ appId, peer, frame, sequence }) {
                    ...
                    if (drive === null)
                        throw new Error("Hyperdrive is not initialized for pears-bulk media.");
            ...
            getHostInfoSnapshot: () => {
                const barePlatform = typeof Bare !== "undefined" && Bare !== null && typeof Bare.platform === "string"
                    ? Bare.platform
                    : "";
                const platform = barePlatform === "ios" || barePlatform === "ios-simulator"
                    ? "ios"
                ...
                if (context.status.tcpEnabled)
                    interfaceTypes.push("tcp");
                if (context.status.autoEnabled)
                    interfaceTypes.push("auto");
                if (context.status.bleEnabled)
                    interfaceTypes.push("ble");
...
export async function ensurePackageDriveManagerImpl(context) {
    if (context.packageDriveManager === null) {
        ...
        context.packageSwarm = createSwarm({ inboundBandwidthLimiter, outboundBandwidthLimiter });
...
export function peerTokenImpl(context) { return bytesToHex(context.provider.randomBytes(16)); }

export function ntfyHostFetchImpl(context, input, init = {}) {
    const headers = {};
    new Headers(init.headers).forEach((value, name) => { headers[name] = value; });
    return context.requestHostReply({ type: "peer-ntfy-http", token: context.peerToken(), request: { url: String(input), method: init.method ?? "GET", headers, ...(typeof init.body === "string" ? { body: init.body } : {}) } }, 30000).then((reply) => {
        if (reply === null || reply.error !== undefined || reply.http === undefined)
            throw new Error(reply?.error ?? "ntfy host request timed out");
        const result = reply.http;
        return { ok: result.status >= 200 && result.status < 300, status: result.status, headers: { get(name) { return name.toLowerCase() === "content-length" ? result.contentLength : null; } }, async text() { return result.body; } };
    });
}

export function sendBluetoothInvitationImpl(context, envelope) {
    const invitation = decodePeerInvitation(envelope, Date.now());
    context.send({ type: "peer-bluetooth-send", framesHex: framePeerAudioPayload(invitation.sessionId, envelope, 192).map(bytesToHex) });
}
...
export function receiveBluetoothFrameImpl(context, frameBytes) {
    ...
    const current = context.bluetoothAssemblies.get(key) ?? initialPeerAudioAssemblyState(Date.now() + 120000);
    ...
    const inbound = { session: { id: `ble:${key}`, kind: "bluetooth" }, envelope: result.payload };
