  ...
  if (context.miniappHost === null) {
    context.miniappHost = createWebWorkletMiniappHost({
      ...
      getHostInfoSnapshot: () => {
        ...
        if (context.status.wsEnabled) interfaceTypes.push("websocket");
      ...
      realtimeReservations: {
        reserveRealtime: (bytesPerSecond) =>
          context.webOutboundBandwidthLimiter.reserve(
            "realtime",
            bytesPerSecond,
          ),
      },
      ...
      getPublisherIdentity: async () => {
        ...
        return loadOrCreateWebIdentity(
      ...
      publishArchive: async ({ t256, archive }) => {
        ...
        return context
          ...
          .publish(session, { t256, archive });
      },
      installFromT256: async (t256) =>
            ...
            chat: async (_appId, request) => {
              ...
              return {
                message: {
                  role: "assistant",
                  content:
                    typeof last === "string" && last.includes("handbook")
                      ? "handbook"
                      : "ok",
                },
