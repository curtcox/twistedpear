...
export async function handleHostMessageImpl(context, raw) {
  ...
  if (message.type === "start") {
    ...
    if (message.gatewayUrl !== undefined) {
      context.webConfig = {
        ...
        identityPassphrase:
          message.identityPassphrase ?? context.DEFAULT_PASSPHRASE,
        ...(message.sharedToken === undefined
          ? {}
          : { sharedToken: message.sharedToken }),
        ...(message.ntfyUrl === undefined || message.ntfyUrl.trim() === ""
          ? {}
          : { ntfyUrl: message.ntfyUrl.trim() }),
        ...(message.ntfyToken === undefined || message.ntfyToken.trim() === ""
          ? {}
          : { ntfyToken: message.ntfyToken.trim() }),
      };
    ...
    if (context.status.tcpEnabled || context.status.wsEnabled) {
      ...
      context.log(
        `Gateway configured (${context.webConfig.gatewayUrl || "unset"}); enable WS gateway to connect`,
      );
    }
    return;
  }
  ...
  if (message.type === "import-identity") {
    try {
      ...
      context.log(
        `Import identity failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    return;
  }
  ...
  if (message.type === "cross-device-command") {
    try {
      const cmd =
        typeof message.command?.cmd === "string" ? message.command.cmd : "";
      const result =
        cmd === "renderer-ping" ||
        cmd === "peer-pair-start" ||
  ...
  if (message.type === "trust-remove") {
    await context
      .ensureInstallService()
      .trustStore.remove(message.publisherPublicKey);
  ...
  if (message.type === "install-app") {
    ...
    try {
      const installed = await storage.installArchive(
      ...
      context.log(
        `Installed ${installed.appId} v${installed.version} (${installed.archiveBytes} bytes)`,
      );
    } catch (error) {
      context.log(
  ...
  if (message.type === "seed-miniapp-kv") {
    await context
      .ensureMiniappKvStore()
      .set(message.key, hexToBytes(message.valueHex));
