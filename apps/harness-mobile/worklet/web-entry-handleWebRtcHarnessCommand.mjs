...
export async function handleWebRtcHarnessCommandImpl(context, request) {
    switch (request.cmd) {
        case "harness-info":
            return {
                lxmfAddress: context.hostLxmfDelivery?.lxmfAddress ?? context.status.lxmfAddress ?? null,
        ...
        case "send-invite": {
            ...
            await context.hostLxmfDelivery.router.packAndSend({
                destination: context.hostLxmfDelivery.router.createOutboundDestination(recipient),
        ...
        case "accept-invite": {
            ...
            const raised = context.harnessInviteEntries.findLast((entry) => entry.kind === "raised" && entry.id === inviteId);
            if (raised === undefined)
            ...
            try {
                ...
                if (!detail.startsWith("No installed version for "))
                    throw error;
                context.log(`Session invite ${inviteId} accepted without launch (${detail})`);
            }
            context.harnessInviteEntries.push({ ...raised, kind: "accepted", at: Date.now() });
        ...
        case "peer-pair-wait": {
            const paired = await context.harnessPeerPair.wait(typeof request.timeoutMs === "number" ? request.timeoutMs : 120000);
            return paired;
        }
