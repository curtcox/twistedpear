...
export async function handleHostMessageImpl(context, raw) {
    ...
    if (message.type === "start") {
        ...
        if (context.status.tcpEnabled) {
            ...
            context.log(`Target set to ${message.targetHost}:${message.targetPort} (enable TCP to connect)`);
        }
        return;
    }
    ...
    if (message.type === "trust-add") {
        try {
            await context.importTrustedPublisher(message.identityString, message.label ?? "Unnamed publisher", message.source ?? "paste");
    ...
    if (message.type === "install-app") {
        ...
        const installVerifiedPackage = async (archive, path) => {
            ...
            pushCatalog();
            context.log(`Installed ${entry.name} v${verified.manifest.version} via ${path} (verified)`);
        };
        try {
            ...
            const publisherPublicKeyHex = entry.manifest?.publisherPublicKey ?? (entry.publisherPublicKey.length === 128 ? entry.publisherPublicKey : null);
    ...
    if (message.type === "launch-miniapp") {
        const { installedStore: installed } = ensureCatalog();
        try {
            await context.ensureMiniappHost().launch(installed, context.runtime, message.appId);
    ...
    if (message.type === "benchmark-miniapp") {
        try {
            ...
            context.log(`Bare worker benchmark: spawn ${result.spawnMs}ms, kill ${result.killMs}ms, ` +
                `busy-loop ${result.busyLoopKillMs}ms, wasm=${result.wasmExecuted}`);
        }
        catch (error) {
            context.log(`Benchmark failed: ${error instanceof Error ? error.message : String(error)}`);
        }
        return;
    }
    ...
    if (message.type === "connect-dev-channel") {
        ...
        try {
            ...
            context.log(`Dev channel connect failed: ${error instanceof Error ? error.message : String(error)}`);
        }
        return;
    }
    ...
    if (message.type === "connect-test-agent") {
        ...
        try {
            ...
            context.testAgent = await connectTestAgent({
                ...
                port: message.port,
                log,
                handleCommand: (request) => context.ensureCrossDeviceTestDriver()(request),
                ...
                acceptSessionInvite: async (inviteId) => {
                    try {
                        ...
                        context.log(`Session invite ${inviteId} accepted without launch (${message})`);
                    }
                }
            });
    ...
    if (message.type === "set-freenet-config") {
        ...
        const authToken = typeof message.authToken === "string" && message.authToken.length > 0
            ? message.authToken
            : undefined;
        const rendezvousHex = typeof message.rendezvousHex === "string" && message.rendezvousHex.length > 0
        ...
        context.status.freenetContractReads = context.freenetCapabilities.contractReads;
        context.status.freenetContractWrites = context.freenetCapabilities.contractWrites;
        context.status.freenetPacketTunnel = context.freenetCapabilities.packetTunnel;
    ...
    if (context.multicastBridge !== null && (message.type === "multicast-packet" || message.type === "multicast-interfaces")) {
    ...
    if (context.bonjourBridge !== null && message.type === "bonjour-peer") {
        ...
        context.status.autoPeers = context.autoIface?.peerInterfaces.length ?? context.status.autoPeers;
        context.pushStatus();
    ...
    if (message.type === "peer-bluetooth-frame") {
        try {
            ...
            context.log(`Rejected BLE invitation frame: ${error instanceof Error ? error.message : String(error)}`);
        }
        return;
    }
    if (context.bleBridge !== null && (message.type === "ble-data" || message.type === "ble-connect" || message.type === "ble-disconnect" || message.type === "ble-error")) {
    ...
    if (context.serialBridge !== null && (message.type === "serial-data" ||
