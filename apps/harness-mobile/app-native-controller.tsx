...
const [status, setStatus] = useState<WorkletStatus>(initialStatus);
  ...
  const [relayNotice, setRelayNotice] = useState<{ appId: string; method: string; kind?: string } | null>(null);
  ...
  const handleWorkletMessage = useCallback((message: WorkletToHostMessage) => {
    ...
    if (message.type === "relay-attribution") { setRelayNotice({ appId: message.appId, method: message.method, ...(message.kind === undefined ? {} : { kind: message.kind }) }); return; }
