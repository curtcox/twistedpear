...
export async function run(sdk, report) {
  ...
  try {
    if (sdk.device === undefined || typeof sdk.device.inventory !== "function") {
      report({
        status: "unavailable",
        details: "device.inventory is not available in this sandbox (upgrade host API).",
    ...
    const diagnostics =
      typeof sdk.device.diagnostics === "function" ? await sdk.device.diagnostics() : [];
    const available = inventory.filter((entry) => entry.availability === "available").length;
    ...
    const notGranted = /CAPABILITY_DENIED|has not been granted|Capability|DEVICE_UNCONFIGURED/i.test(
      message
    );
    const unavailable = /UNKNOWN_METHOD|not available|upgrade host|DEVICE_UNCONFIGURED/i.test(message);
    report({
