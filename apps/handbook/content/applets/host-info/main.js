  ...
  try {
    if (sdk.host === undefined || typeof sdk.host.info !== "function") {
      report({
        status: "unavailable",
        details:
          "host.info is not available in this sandbox (upgrade host API).",
    ...
    const api =
      typeof info.hostApiVersion === "string" ? info.hostApiVersion : "?";
    const roles = info.roles ?? {};
    const interfaces = Array.isArray(info.interfaceTypes)
      ? info.interfaceTypes.join(",")
      : "";
    const grants = Array.isArray(info.grantedCapabilities)
      ? info.grantedCapabilities.length
      : 0;
    ...
    const unavailable = /UNKNOWN_METHOD|not available|upgrade host/i.test(
      message,
    );
    report({
